...
@muladd begin
    ...
    function MPICache(uEltype)
        ...
        if !isbitstype(uEltype)
            throw(
                ArgumentError(
                    "MPICache only supports bitstypes, $uEltype is not a bitstype.",
                ),
            )
        end
            ...
            for (index, interface) in enumerate(mpi_cache.mpi_neighbor_interfaces[d])
                ...
                if cache.mpi_interfaces.remote_sides[interface] == 1 # local element in positive direction
                    @views send_buffer[first:last] .=
                        vec(cache.mpi_interfaces.u[2, :, :, interface])
                else # local element in negative direction
                    @views send_buffer[first:last] .=
                        vec(cache.mpi_interfaces.u[1, :, :, interface])
                end
            end
            ...
            for (index, mortar) in enumerate(mpi_cache.mpi_neighbor_mortars[d])
                ...
                for position in cache.mpi_mortars.local_neighbor_positions[mortar]
                    ...
                    if position == 1 # lower element
                        ...
                        @views send_buffer[first:last] .=
                            vec(cache.mpi_mortars.u_lower[leftright, :, :, mortar])
                    elseif position == 2 # upper element
                        first, last = indices[position]
                        @views send_buffer[first:last] .=
                            vec(cache.mpi_mortars.u_upper[leftright, :, :, mortar])
                    else # large element
                        first_lower, last_lower, first_upper, last_upper = indices[position]
                        @views send_buffer[first_lower:last_lower] .=
                            vec(cache.mpi_mortars.u_lower[leftright, :, :, mortar])
                        @views send_buffer[first_upper:last_upper] .=
                            vec(cache.mpi_mortars.u_upper[leftright, :, :, mortar])
                    end
                end
            end
            ...
            for (index, mortar) in enumerate(mpi_cache.mpi_neighbor_mortars[d])
                ...
                for position = 1:3
                    ...
                    if position == 1 # lower element data has been received
                        first, last = indices[position]
                        @views vec(cache.mpi_mortars.u_lower[leftright, :, :, mortar]) .=
                            recv_buffer[first:last]
                    elseif position == 2 # upper element data has been received
                        first, last = indices[position]
                        @views vec(cache.mpi_mortars.u_upper[leftright, :, :, mortar]) .=
                            recv_buffer[first:last]
                    else # large element data has been received
                        first_lower, last_lower, first_upper, last_upper = indices[position]
                        @views vec(cache.mpi_mortars.u_lower[leftright, :, :, mortar]) .=
                            recv_buffer[first_lower:last_lower]
                        @views vec(cache.mpi_mortars.u_upper[leftright, :, :, mortar]) .=
                            recv_buffer[first_upper:last_upper]
                    end
                end
            end
    ...
    function init_mpi_neighbor_connectivity(
        ...
        for interface_id = 1:nmpiinterfaces(mpi_interfaces)
            ...
            if local_cell_id < remote_cell_id
                global_interface_ids[interface_id] =
                    2 * ndims(tree) * local_cell_id + direction - 1
            else
                global_interface_ids[interface_id] =
                    (2 * ndims(tree) * remote_cell_id + opposite_direction(direction) - 1)
            end
        end
        ...
        for mortar = 1:nmpimortars(mpi_mortars)
            ...
            if 3 in local_neighbor_positions # large element is on this rank
                large_element_id =
                    local_neighbor_ids[findfirst(pos -> pos == 3, local_neighbor_positions)]
            ...
            neighbor_cell_id =
                tree.neighbor_ids[opposite_direction(direction), large_cell_id]
            if direction == 1
        ...
        for (index, d) in enumerate(mpi_neighbor_ranks)
            mpi_neighbor_interfaces[index] =
                interface_ids[findall(x -> (x == d), neighbor_ranks_interface)]
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            if mpi_interfaces.orientations[interface] == 1 # interface in x-direction
                if mpi_interfaces.remote_sides[interface] == 1 # local element in positive direction
                    ...
                    for j in eachnode(dg), v in eachvariable(equations)
                        mpi_interfaces.u[1, v, j, interface] =
                            u[v, nnodes(dg), j, local_element]
                    end
                end
                ...
                if mpi_interfaces.remote_sides[interface] == 1 # local element in positive direction
                    ...
                    for i in eachnode(dg), v in eachvariable(equations)
                        mpi_interfaces.u[1, v, i, interface] =
                            u[v, i, nnodes(dg), local_element]
                    end
                end
            end
        end
                    ...
                    if mpi_mortars.large_sides[mortar] == 1 # -> small elements on right side
                            ...
                            if position == 1
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_lower[2, v, l, mortar] =
                                            u[v, 1, l, element]
                                    end
                                end
                            else # position == 2
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_upper[2, v, l, mortar] =
                                            u[v, 1, l, element]
                                    end
                                end
                            end
                        ...
                        if mpi_mortars.orientations[mortar] == 1
                            ...
                            if position == 1
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_lower[1, v, l, mortar] =
                                ...
                                for l in eachnode(dg)
                                    for v in eachvariable(equations)
                                        mpi_mortars.u_upper[1, v, l, mortar] =
                                            u[v, nnodes(dg), l, element]
                                    end
                                end
                            end
    ...
    @inline function mpi_mortar_fluxes_to_elements!(
        surface_flux_values,
        mesh::ParallelTreeMesh{2},
        equations,
        mortar_l2::LobattoLegendreMortarL2,
        ...
        for (element, position) in zip(local_neighbor_ids, local_neighbor_positions)
            if position in (1, 2) # Current element is small
                ...
                multiply_dimensionwise!(
                    view(surface_flux_values, :, :, direction, element),
