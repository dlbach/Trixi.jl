...
@muladd begin
    ...
    # the RHS etc.
    function create_cache(mesh::TreeMesh{3}, equations, dg::DG, RealT, uEltype)
        ...
        cache =
            (; cache..., create_cache(mesh, equations, dg.volume_integral, dg, uEltype)...)
    ...
    function create_cache(
        ...
        fstar1_L_threaded = A4dp1_x[
            A4dp1_x(undef, nvariables(equations), nnodes(dg) + 1, nnodes(dg), nnodes(dg)) for _ = 1:Threads.nthreads()
        ]
        fstar1_R_threaded = A4dp1_x[
            A4dp1_x(undef, nvariables(equations), nnodes(dg) + 1, nnodes(dg), nnodes(dg)) for _ = 1:Threads.nthreads()
        ]
        fstar2_L_threaded = A4dp1_y[
            A4dp1_y(undef, nvariables(equations), nnodes(dg), nnodes(dg) + 1, nnodes(dg)) for _ = 1:Threads.nthreads()
        ]
        fstar2_R_threaded = A4dp1_y[
            A4dp1_y(undef, nvariables(equations), nnodes(dg), nnodes(dg) + 1, nnodes(dg)) for _ = 1:Threads.nthreads()
        ]
        fstar3_L_threaded = A4dp1_z[
            A4dp1_z(undef, nvariables(equations), nnodes(dg), nnodes(dg), nnodes(dg) + 1) for _ = 1:Threads.nthreads()
        ]
        fstar3_R_threaded = A4dp1_z[
            A4dp1_z(undef, nvariables(equations), nnodes(dg), nnodes(dg), nnodes(dg) + 1) for _ = 1:Threads.nthreads()
        ]
    ...
    function create_cache(
        ...
        A3d = Array{uEltype,3}
        fstar_upper_left_threaded = A3d[
            A3d(undef, nvariables(equations), nnodes(mortar_l2), nnodes(mortar_l2)) for
            _ = 1:Threads.nthreads()
        ]
        fstar_upper_right_threaded = A3d[
            A3d(undef, nvariables(equations), nnodes(mortar_l2), nnodes(mortar_l2)) for
            _ = 1:Threads.nthreads()
        ]
        fstar_lower_left_threaded = A3d[
            A3d(undef, nvariables(equations), nnodes(mortar_l2), nnodes(mortar_l2)) for
            _ = 1:Threads.nthreads()
        ]
        fstar_lower_right_threaded = A3d[
            A3d(undef, nvariables(equations), nnodes(mortar_l2), nnodes(mortar_l2)) for
            _ = 1:Threads.nthreads()
        ]
        fstar_tmp1_threaded = A3d[
            A3d(undef, nvariables(equations), nnodes(mortar_l2), nnodes(mortar_l2)) for
            _ = 1:Threads.nthreads()
        ]

        (;
            fstar_upper_left_threaded,
            fstar_upper_right_threaded,
            fstar_lower_left_threaded,
            fstar_lower_right_threaded,
            fstar_tmp1_threaded,
        )
    end
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                ...
                integral_contribution =
                    integral_contribution + derivative_split[i, ii] * noncons_flux1
            end
            ...
            for jj in eachnode(dg)
                ...
                integral_contribution =
                    integral_contribution + derivative_split[j, jj] * noncons_flux2
            end
            ...
            for kk in eachnode(dg)
                ...
                integral_contribution =
                    integral_contribution + derivative_split[k, kk] * noncons_flux3
            end
    ...
    function calc_volume_integral!(
        ...
        alpha = @trixi_timeit timer() "blending factors" indicator(
            u,
            mesh,
            equations,
            dg,
        ...
        @trixi_timeit timer() "pure DG" @threaded for idx_element in
                                                      eachindex(element_ids_dg)
        ...
        @trixi_timeit timer() "blended DG-FV" @threaded for idx_element in
                                                            eachindex(element_ids_dgfv)
    ...
    @inline function fv_kernel!(
        ...
        @unpack fstar1_L_threaded,
        fstar1_R_threaded,
        fstar2_L_threaded,
        fstar2_R_threaded,
        fstar3_L_threaded,
        fstar3_R_threaded = cache
    ...
    @inline function calcflux_fv!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i = 2:nnodes(dg)
        ...
        for k in eachnode(dg), j = 2:nnodes(dg), i in eachnode(dg)
        ...
        for k = 2:nnodes(dg), j in eachnode(dg), i in eachnode(dg)
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for k in eachnode(dg), j in eachnode(dg), v in eachvariable(equations)
                    interfaces.u[1, v, j, k, interface] =
                        u[v, nnodes(dg), j, k, left_element]
            ...
            elseif orientations[interface] == 2
                ...
                for k in eachnode(dg), i in eachnode(dg), v in eachvariable(equations)
                    interfaces.u[1, v, i, k, interface] =
                        u[v, i, nnodes(dg), k, left_element]
                ...
                for j in eachnode(dg), i in eachnode(dg), v in eachvariable(equations)
                    interfaces.u[1, v, i, j, interface] =
                        u[v, i, j, nnodes(dg), left_element]
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                ...
                for v in eachvariable(equations)
                    ...
                    # the interpretation of global SBP operators coupled discontinuously via
                    # central fluxes/SATs
                    surface_flux_values[v, i, j, left_direction, left_id] =
                        flux[v] + 0.5 * noncons_left[v]
                    surface_flux_values[v, i, j, right_direction, right_id] =
                        flux[v] + 0.5 * noncons_right[v]
                end
            end
        end
    ...
    function calc_boundary_flux!(
        ...
        calc_boundary_flux_by_direction!(
            surface_flux_values,
            t,
    ...
    function calc_boundary_flux_by_direction!(
        surface_flux_values::AbstractArray{<:Any,5},
        t,
        boundary_condition,
        equations,
        surface_integral,
        ...
        @unpack surface_flux = surface_integral
        @unpack u, neighbor_ids, neighbor_sides, node_coordinates, orientations =
            cache.boundaries

        @threaded for boundary = first_boundary:last_boundary
            ...
            if cache.mortars.large_sides[mortar] == 1 # -> small elements on right side
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, j, k, mortar] =
                                u[v, 1, j, k, upper_left_element]
                            cache.mortars.u_upper_right[2, v, j, k, mortar] =
                                u[v, 1, j, k, upper_right_element]
                            cache.mortars.u_lower_left[2, v, j, k, mortar] =
                                u[v, 1, j, k, lower_left_element]
                            cache.mortars.u_lower_right[2, v, j, k, mortar] =
                                u[v, 1, j, k, lower_right_element]
                        end
                    end
                elseif cache.mortars.orientations[mortar] == 2
                    ...
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, i, k, mortar] =
                                u[v, i, 1, k, upper_left_element]
                            cache.mortars.u_upper_right[2, v, i, k, mortar] =
                                u[v, i, 1, k, upper_right_element]
                            cache.mortars.u_lower_left[2, v, i, k, mortar] =
                                u[v, i, 1, k, lower_left_element]
                            cache.mortars.u_lower_right[2, v, i, k, mortar] =
                                u[v, i, 1, k, lower_right_element]
                        end
                    end
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[2, v, i, j, mortar] =
                                u[v, i, j, 1, upper_left_element]
                            cache.mortars.u_upper_right[2, v, i, j, mortar] =
                                u[v, i, j, 1, upper_right_element]
                            cache.mortars.u_lower_left[2, v, i, j, mortar] =
                                u[v, i, j, 1, lower_left_element]
                            cache.mortars.u_lower_right[2, v, i, j, mortar] =
                                u[v, i, j, 1, lower_right_element]
                        end
                    end
                end
                ...
                if cache.mortars.orientations[mortar] == 1
                    ...
                    for k in eachnode(dg), j in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, j, k, mortar] =
                                u[v, nnodes(dg), j, k, upper_left_element]
                            cache.mortars.u_upper_right[1, v, j, k, mortar] =
                                u[v, nnodes(dg), j, k, upper_right_element]
                            cache.mortars.u_lower_left[1, v, j, k, mortar] =
                                u[v, nnodes(dg), j, k, lower_left_element]
                            cache.mortars.u_lower_right[1, v, j, k, mortar] =
                                u[v, nnodes(dg), j, k, lower_right_element]
                        end
                    end
                elseif cache.mortars.orientations[mortar] == 2
                    # L2 mortars in y-direction
                    for k in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, i, k, mortar] =
                                u[v, i, nnodes(dg), k, upper_left_element]
                            cache.mortars.u_upper_right[1, v, i, k, mortar] =
                                u[v, i, nnodes(dg), k, upper_right_element]
                            cache.mortars.u_lower_left[1, v, i, k, mortar] =
                                u[v, i, nnodes(dg), k, lower_left_element]
                            cache.mortars.u_lower_right[1, v, i, k, mortar] =
                                u[v, i, nnodes(dg), k, lower_right_element]
                        end
                    end
                    ...
                    for j in eachnode(dg), i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mortars.u_upper_left[1, v, i, j, mortar] =
                                u[v, i, j, nnodes(dg), upper_left_element]
                            cache.mortars.u_upper_right[1, v, i, j, mortar] =
                                u[v, i, j, nnodes(dg), upper_right_element]
                            cache.mortars.u_lower_left[1, v, i, j, mortar] =
                                u[v, i, j, nnodes(dg), lower_left_element]
                            cache.mortars.u_lower_right[1, v, i, j, mortar] =
                                u[v, i, j, nnodes(dg), lower_right_element]
                        end
                    end
                end
            end
    ...
    @inline function element_solutions_to_mortars!(
        ...
        multiply_dimensionwise!(
            view(mortars.u_lower_left, leftright, :, :, :, mortar),
            mortar_l2.forward_lower,
            mortar_l2.forward_lower,
            u_large,
            fstar_tmp1,
    ...
    function calc_mortar_flux!(
        ...
        @unpack surface_flux = surface_integral
        @unpack u_lower_left, u_lower_right, u_upper_left, u_upper_right, orientations =
            cache.mortars
        @unpack (
            fstar_upper_left_threaded,
            fstar_upper_right_threaded,
        ...
        @threaded for mortar in eachmortar(dg, cache)
            ...
            if large_sides[mortar] == 1 # -> small elements on right side
                for j in eachnode(dg), i in eachnode(dg)
                    ...
                    u_upper_left_ll, u_upper_left_rr =
                        get_surface_node_vars(u_upper_left, equations, dg, i, j, mortar)
                    u_upper_right_ll, u_upper_right_rr =
                        get_surface_node_vars(u_upper_right, equations, dg, i, j, mortar)
                    u_lower_left_ll, u_lower_left_rr =
                        get_surface_node_vars(u_lower_left, equations, dg, i, j, mortar)
                    u_lower_right_ll, u_lower_right_rr =
                        get_surface_node_vars(u_lower_right, equations, dg, i, j, mortar)
                    # Call pointwise nonconservative term
                    noncons_upper_left = nonconservative_flux(
                        u_upper_left_ll,
                        u_upper_left_rr,
                        orientation,
                        equations,
                    )
                    noncons_upper_right = nonconservative_flux(
                        u_upper_right_ll,
                        u_upper_right_rr,
                        orientation,
                        equations,
                    )
                    noncons_lower_left = nonconservative_flux(
                        u_lower_left_ll,
                        u_lower_left_rr,
                        orientation,
                        equations,
                    )
                    noncons_lower_right = nonconservative_flux(
                        u_lower_right_ll,
                        u_lower_right_rr,
                        orientation,
                        equations,
    ...
    @inline function calc_fstar!(
        destination::AbstractArray{<:Any,3},
        equations,
        surface_flux,
        dg::DGSEM,
        u_interfaces,
    ...
    @inline function mortar_fluxes_to_elements!(
        surface_flux_values,
        mesh::TreeMesh{3},
        equations,
        mortar_l2::LobattoLegendreMortarL2,
        ...
        @threaded for element in eachelement(dg, cache)
            for m in eachnode(dg), l in eachnode(dg)
                for v in eachvariable(equations)
                    ...
                    du[v, 1, l, m, element] = (
                        du[v, 1, l, m, element] -
                        surface_flux_values[v, l, m, 1, element] * factor_1
                    )

                    # surface at +x
                    du[v, nnodes(dg), l, m, element] = (
                        du[v, nnodes(dg), l, m, element] +
                        surface_flux_values[v, l, m, 2, element] * factor_2
                    )

                    # surface at -y
                    du[v, l, 1, m, element] = (
                        du[v, l, 1, m, element] -
                        surface_flux_values[v, l, m, 3, element] * factor_1
                    )

                    # surface at +y
                    du[v, l, nnodes(dg), m, element] = (
                        du[v, l, nnodes(dg), m, element] +
                        surface_flux_values[v, l, m, 4, element] * factor_2
                    )

                    # surface at -z
                    du[v, l, m, 1, element] = (
                        du[v, l, m, 1, element] -
                        surface_flux_values[v, l, m, 5, element] * factor_1
                    )

                    # surface at +z
                    du[v, l, m, nnodes(dg), element] = (
                        du[v, l, m, nnodes(dg), element] +
                        surface_flux_values[v, l, m, 6, element] * factor_2
                    )
                end
            end
        end
    ...
    function apply_jacobian!(du, mesh::TreeMesh{3}, equations, dg::DG, cache)
