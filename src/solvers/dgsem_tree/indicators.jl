...
@muladd begin
    ...
    function create_cache(
        typ::Type{IndicatorType},
        semi,
    ) where {IndicatorType<:AbstractIndicator}
    ...
    function IndicatorHennemannGassner(
    ...
    # this method is used when the indicator is constructed as for shock-capturing volume integrals
    function IndicatorLöhner(equations::AbstractEquations, basis; f_wave = 0.2, variable)
        cache = create_cache(IndicatorLöhner, equations, basis)
        IndicatorLöhner{typeof(f_wave),typeof(variable),typeof(cache)}(
            f_wave,
            variable,
            cache,
        )
    ...
    # this method is used when the indicator is constructed as for AMR
    function IndicatorLöhner(semi::AbstractSemidiscretization; f_wave = 0.2, variable)
    ...
    function Base.show(io::IO, ::MIME"text/plain", indicator::IndicatorLöhner)
        ...
        if get(io, :compact, false)
            ...
            setup =
                ["indicator variable" => indicator.variable, "f_wave" => indicator.f_wave]
    ...
    @inline function local_löhner_estimate(
        um::Real,
        u0::Real,
        up::Real,
        ...
        num = abs(up - 2 * u0 + um)
        den =
            abs(up - u0) + abs(u0 - um) + löhner.f_wave * (abs(up) + 2 * abs(u0) + abs(um))
        return num / den
    end
    ...
    # this method is used when the indicator is constructed as for shock-capturing volume integrals
    function IndicatorMax(equations::AbstractEquations, basis; variable)
        cache = create_cache(IndicatorMax, equations, basis)
        IndicatorMax{typeof(variable),typeof(cache)}(variable, cache)
    end

    # this method is used when the indicator is constructed as for AMR
    function IndicatorMax(semi::AbstractSemidiscretization; variable)
    ...
    # this method is used when the indicator is constructed as for shock-capturing volume integrals
    function IndicatorNeuralNetwork(
        equations::AbstractEquations,
        basis;
        indicator_type,
        alpha_max = 0.5,
    ...
    @inline function probability_to_indicator(
