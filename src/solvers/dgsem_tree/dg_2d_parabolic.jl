...
@muladd begin
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                flux_1_node =
                    get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j, element)
                flux_2_node =
                    get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j, element)
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for j in eachnode(dg), v in eachvariable(equations_parabolic)
                    ...
                    interfaces.u[1, v, j, interface] =
                        flux_viscous_x[v, nnodes(dg), j, left_element]
                    interfaces.u[2, v, j, interface] =
                        flux_viscous_x[v, 1, j, right_element]
                end
                ...
                for i in eachnode(dg), v in eachvariable(equations_parabolic)
                    ...
                    interfaces.u[1, v, i, interface] =
                        flux_viscous_y[v, i, nnodes(dg), left_element]
                    interfaces.u[2, v, i, interface] =
                        flux_viscous_y[v, i, 1, right_element]
                end
            end
        end
            ...
            if orientations[boundary] == 1
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for l in eachnode(dg), v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[1, v, l, boundary] =
                            flux_viscous_y[v, l, nnodes(dg), element]
                    end
                else
                    # element in +y direction of boundary
                    for l in eachnode(dg), v in eachvariable(equations_parabolic)
    ...
    function calc_viscous_fluxes!(
        ...
        @threaded for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                ...
                u_node =
                    get_node_vars(u_transformed, equations_parabolic, dg, i, j, element)
                gradients_1_node =
                    get_node_vars(gradients_x, equations_parabolic, dg, i, j, element)
                gradients_2_node =
                    get_node_vars(gradients_y, equations_parabolic, dg, i, j, element)
    ...
    function calc_boundary_flux_by_direction_divergence!(
        ...
        @threaded for boundary = first_boundary:last_boundary
            ...
            for i in eachnode(dg)
                ...
                flux_ll, flux_rr =
                    get_surface_node_vars(u, equations_parabolic, dg, i, boundary)
                if neighbor_sides[boundary] == 1 # Element is on the left, boundary on the right
                    flux_inner = flux_ll
                else # Element is on the right, boundary on the left
                    flux_inner = flux_rr
