...
@muladd begin
    ...
    function create_cache(
        ::Type{IndicatorHennemannGassner},
        ...
        indicator_threaded =
            [A(undef, nnodes(basis), nnodes(basis)) for _ = 1:Threads.nthreads()]
        modal_threaded =
            [A(undef, nnodes(basis), nnodes(basis)) for _ = 1:Threads.nthreads()]
        modal_tmp1_threaded =
            [A(undef, nnodes(basis), nnodes(basis)) for _ = 1:Threads.nthreads()]
    ...
    function create_cache(
        typ::Type{IndicatorHennemannGassner},
        mesh,
        equations::AbstractEquations{2},
        dg::DGSEM,
        cache,
    ...
    @inline function calc_indicator_hennemann_gassner!(
        indicator_hg,
        threshold,
        parameter_s,
        u,
        ...
        @unpack alpha, alpha_tmp, indicator_threaded, modal_threaded, modal_tmp1_threaded =
            indicator_hg.cache
        ...
        for j = 1:nnodes(dg), i = 1:nnodes(dg)
            total_energy += modal[i, j]^2
        end
        total_energy_clip1 = zero(eltype(modal))
        for j = 1:(nnodes(dg)-1), i = 1:(nnodes(dg)-1)
            total_energy_clip1 += modal[i, j]^2
        end
        total_energy_clip2 = zero(eltype(modal))
        for j = 1:(nnodes(dg)-2), i = 1:(nnodes(dg)-2)
    ...
    function (löhner::IndicatorLöhner)(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j in eachnode(dg), i = 2:nnodes(dg)-1
                # x direction
                u0 = indicator[i, j]
                up = indicator[i+1, j]
                um = indicator[i-1, j]
            ...
            for j = 2:nnodes(dg)-1, i in eachnode(dg)
                # y direction
                u0 = indicator[i, j]
                up = indicator[i, j+1]
                um = indicator[i, j-1]
    ...
    function (indicator_max::IndicatorMax)(
        u::AbstractArray{<:Any,4},
        mesh,
        equations,
        dg::DGSEM,
    ...
    function create_cache(
        ::Type{IndicatorNeuralNetwork{NeuralNetworkPerssonPeraire}},
        equations::AbstractEquations{2},
        basis::LobattoLegendreBasis,
    )
        ...
        indicator_threaded = [similar(prototype) for _ = 1:Threads.nthreads()]
        modal_threaded = [similar(prototype) for _ = 1:Threads.nthreads()]
        modal_tmp1_threaded = [similar(prototype) for _ = 1:Threads.nthreads()]
    ...
    function create_cache(
        ::Type{IndicatorNeuralNetwork{NeuralNetworkRayHesthaven}},
        equations::AbstractEquations{2},
        basis::LobattoLegendreBasis,
    )
    ...
    function create_cache(
        ::Type{IndicatorNeuralNetwork{NeuralNetworkCNN}},
        equations::AbstractEquations{2},
        basis::LobattoLegendreBasis,
    )
    ...
    # this method is used when the indicator is constructed as for AMR
    function create_cache(
        typ::Type{<:IndicatorNeuralNetwork},
        mesh,
        equations::AbstractEquations{2},
        dg::DGSEM,
        cache,
    ...
    function (indicator_ann::IndicatorNeuralNetwork{NeuralNetworkPerssonPeraire})(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for j = 1:(nnodes(dg)-3), i = 1:(nnodes(dg)-3)
            ...
            # Scale input data
            network_input =
                network_input / max(maximum(abs, network_input), one(eltype(network_input)))
            probability_troubled_cell = network(network_input)[1]

            # Compute indicator value
            alpha[element] = probability_to_indicator(
    ...
    function (indicator_ann::IndicatorNeuralNetwork{NeuralNetworkRayHesthaven})(
        ...
        network_input,
        neighbor_ids,
        neighbor_mean = indicator_ann.cache #X, network_input
        # TODO: Taal refactor, when to `resize!` stuff changed possibly by AMR?
        #       Shall we implement `resize!(semi::AbstractSemidiscretization, new_size)`
        #       or just `resize!` whenever we call the relevant methods as we do now?
        resize!(alpha, nelements(dg, cache))
    ...
    function (indicator_ann::IndicatorNeuralNetwork{NeuralNetworkCNN})(
        ...
        cnn_nodes,
        vandermonde,
        network_input = indicator_ann.cache
        # TODO: Taal refactor, when to `resize!` stuff changed possibly by AMR?
        #       Shall we implement `resize!(semi::AbstractSemidiscretization, new_size)`
        #       or just `resize!` whenever we call the relevant methods as we do now?
        resize!(alpha, nelements(dg, cache))
