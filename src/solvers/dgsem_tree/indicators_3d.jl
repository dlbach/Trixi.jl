...
@muladd begin
    ...
    function create_cache(
        ...
        indicator_threaded = [
            A(undef, nnodes(basis), nnodes(basis), nnodes(basis)) for
    ...
    @inline function calc_indicator_hennemann_gassner!(
        ...
        modal_threaded,
        modal_tmp1_threaded,
        modal_tmp2_threaded = indicator_hg.cache
        ...
        for k = 1:nnodes(dg), j = 1:nnodes(dg), i = 1:nnodes(dg)
            total_energy += modal[i, j, k]^2
        end
        total_energy_clip1 = zero(eltype(modal))
        for k = 1:(nnodes(dg)-1), j = 1:(nnodes(dg)-1), i = 1:(nnodes(dg)-1)
            total_energy_clip1 += modal[i, j, k]^2
        end
        total_energy_clip2 = zero(eltype(modal))
        for k = 1:(nnodes(dg)-2), j = 1:(nnodes(dg)-2), i = 1:(nnodes(dg)-2)
    ...
    function apply_smoothing!(
        ...
        for mortar in eachmortar(dg, cache)
            ...
            # Apply smoothing
            alpha[lower_left] =
                max(alpha_tmp[lower_left], 0.5 * alpha_tmp[large], alpha[lower_left])
            alpha[lower_right] =
                max(alpha_tmp[lower_right], 0.5 * alpha_tmp[large], alpha[lower_right])
            alpha[upper_left] =
                max(alpha_tmp[upper_left], 0.5 * alpha_tmp[large], alpha[upper_left])
            alpha[upper_right] =
                max(alpha_tmp[upper_right], 0.5 * alpha_tmp[large], alpha[upper_right])
    ...
    function (löhner::IndicatorLöhner)(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for k in eachnode(dg), j in eachnode(dg), i = 2:nnodes(dg)-1
                # x direction
                u0 = indicator[i, j, k]
            ...
            for k in eachnode(dg), j = 2:nnodes(dg)-1, i in eachnode(dg)
                # y direction
                u0 = indicator[i, j, k]
            ...
            for k = 2:nnodes(dg)-1, j in eachnode(dg), i in eachnode(dg)
                # y direction
                u0 = indicator[i, j, k]
