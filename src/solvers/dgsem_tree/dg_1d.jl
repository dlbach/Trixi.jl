...
@muladd begin
    ...
    # the RHS etc.
    function create_cache(mesh::TreeMesh{1}, equations, dg::DG, RealT, uEltype)
    ...
    function create_cache(
        mesh::Union{TreeMesh{1},StructuredMesh{1},P4estMesh{1}},
        equations,
    ...
    function create_cache(
        ...
        fstar1_L_threaded = A2dp1_x[
            A2dp1_x(undef, nvariables(equations), nnodes(dg) + 1) for
            _ = 1:Threads.nthreads()
        ]
        fstar1_R_threaded = A2dp1_x[
            A2dp1_x(undef, nvariables(equations), nnodes(dg) + 1) for
            _ = 1:Threads.nthreads()
        ]
    ...
    @inline function fv_kernel!(
        ...
        for i in eachnode(dg)
            for v in eachvariable(equations)
                du[v, i, element] +=
                    (alpha * (inverse_weights[i] * (fstar1_L[v, i+1] - fstar1_R[v, i])))
    ...
    function calc_interface_flux!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for v in eachvariable(equations)
                ...
                # the interpretation of global SBP operators coupled discontinuously via
                # central fluxes/SATs
                surface_flux_values[v, left_direction, left_id] =
                    flux[v] + 0.5 * noncons_left[v]
                surface_flux_values[v, right_direction, right_id] =
    ...
    function calc_boundary_flux_by_direction!(
        surface_flux_values::AbstractArray{<:Any,3},
        t,
        boundary_condition,
        nonconservative_terms::False,
        equations,
    ...
    function calc_boundary_flux_by_direction!(
        ...
        @threaded for boundary = first_boundary:last_boundary
            ...
            # Copy flux to left and right element storage
            for v in eachvariable(equations)
                surface_flux_values[v, direction, neighbor] =
