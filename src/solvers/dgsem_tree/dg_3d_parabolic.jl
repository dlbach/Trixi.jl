...
@muladd begin
    ...
    function rhs_parabolic!(
        ...
        @trixi_timeit timer() "interface flux" calc_interface_flux!(
            cache_parabolic.elements.surface_flux_values,
            mesh,
            equations_parabolic,
            dg,
            cache_parabolic,
    ...
    function transform_variables!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                ...
                u_transformed_node = gradient_variable_transformation(equations_parabolic)(
                    u_node,
                    equations_parabolic,
                )
                set_node_vars!(
    ...
    function calc_volume_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            ...
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                flux_1_node =
                    get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j, k, element)
                flux_2_node =
                    get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j, k, element)
                flux_3_node =
                    get_node_vars(flux_viscous_z, equations_parabolic, dg, i, j, k, element)

                for ii in eachnode(dg)
                    multiply_add_to_node_vars!(
                        du,
                        derivative_dhat[ii, i],
                        flux_1_node,
                        equations_parabolic,
                        dg,
                        ii,
            ...
            if orientations[interface] == 1
                ...
                for k in eachnode(dg),
                    ...
                    interfaces.u[1, v, j, k, interface] =
                        flux_viscous_x[v, nnodes(dg), j, k, left_element]
                    interfaces.u[2, v, j, k, interface] =
                        flux_viscous_x[v, 1, j, k, right_element]
                end
            elseif orientations[interface] == 2
                ...
                for k in eachnode(dg),
                    ...
                    interfaces.u[1, v, i, k, interface] =
                        flux_viscous_y[v, i, nnodes(dg), k, left_element]
                    interfaces.u[2, v, i, k, interface] =
                        flux_viscous_y[v, i, 1, k, right_element]
                end
                ...
                for j in eachnode(dg),
                    ...
                    interfaces.u[1, v, i, j, interface] =
                        flux_viscous_z[v, i, j, nnodes(dg), left_element]
                    interfaces.u[2, v, i, j, interface] =
                        flux_viscous_z[v, i, j, 1, right_element]
                end
            end
        end
            ...
            if orientations[boundary] == 1
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for k in eachnode(dg),
                        ...
                        boundaries.u[1, v, j, k, boundary] =
                            flux_viscous_x[v, nnodes(dg), j, k, element]
                    end
                    ...
                    for k in eachnode(dg),
                        ...
                        v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[2, v, j, k, boundary] =
                            flux_viscous_x[v, 1, j, k, element]
                    end
                end
            elseif orientations[boundary] == 2
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for k in eachnode(dg),
                        ...
                        v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[1, v, i, k, boundary] =
                            flux_viscous_y[v, i, nnodes(dg), k, element]
                    end
                else
                    # element in +y direction of boundary
                    for k in eachnode(dg),
                        ...
                        v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[2, v, i, k, boundary] =
                            flux_viscous_y[v, i, 1, k, element]
                    end
                end
                ...
                if neighbor_sides[boundary] == 1
                    ...
                    for j in eachnode(dg),
                        ...
                        v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[1, v, i, j, boundary] =
                            flux_viscous_z[v, i, j, nnodes(dg), element]
                    end
                else
                    # element in +z direction of boundary
                    for j in eachnode(dg),
                        ...
                        v in eachvariable(equations_parabolic)
                        # OBS! `boundaries.u` stores the interpolated *fluxes* and *not the solution*!
                        boundaries.u[2, v, i, j, boundary] =
                            flux_viscous_z[v, i, j, 1, element]
                    end
                end
            end
        end
    ...
    function calc_viscous_fluxes!(
        ...
        @threaded for element in eachelement(dg, cache)
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                ...
                u_node =
                    get_node_vars(u_transformed, equations_parabolic, dg, i, j, k, element)
                gradients_1_node =
                    get_node_vars(gradients_x, equations_parabolic, dg, i, j, k, element)
                gradients_2_node =
                    get_node_vars(gradients_y, equations_parabolic, dg, i, j, k, element)
                gradients_3_node =
                    get_node_vars(gradients_z, equations_parabolic, dg, i, j, k, element)

                # Calculate viscous flux and store each component for later use
                flux_viscous_node_x = flux(
                    u_node,
                    (gradients_1_node, gradients_2_node, gradients_3_node),
                    1,
                    equations_parabolic,
                )
                flux_viscous_node_y = flux(
                    u_node,
                    (gradients_1_node, gradients_2_node, gradients_3_node),
                    2,
                    equations_parabolic,
                )
                flux_viscous_node_z = flux(
                    u_node,
                    (gradients_1_node, gradients_2_node, gradients_3_node),
                    3,
                    equations_parabolic,
    ...
    function calc_boundary_flux_gradients!(
        cache,
        t,
    ...
    function calc_boundary_flux_divergence!(
        cache,
        t,
        boundary_conditions_parabolic::BoundaryConditionPeriodic,
        mesh::TreeMesh{3},
    ...
    function calc_boundary_flux_gradients!(
        ...
        calc_boundary_flux_by_direction_gradient!(
            surface_flux_values,
            t,
    ...
    function calc_boundary_flux_by_direction_gradient!(
        ...
        @threaded for boundary = first_boundary:last_boundary
            ...
            for j in eachnode(dg), i in eachnode(dg)
                # Get boundary flux
                u_ll, u_rr =
                    get_surface_node_vars(u, equations_parabolic, dg, i, j, boundary)
        ...
        @trixi_timeit timer() "interface flux" begin
            ...
            @threaded for interface in eachinterface(dg, cache_parabolic)
                ...
                for j in eachnode(dg), i in eachnode(dg)
                    ...
                    u_ll, u_rr = get_surface_node_vars(
                        cache_parabolic.interfaces.u,
                        equations_parabolic,
                        dg,
                        i,
    ...
    # the RHS etc.
    function create_cache_parabolic(
        mesh::TreeMesh{3},
    ...
    function apply_jacobian_parabolic!(
        du,
        mesh::TreeMesh{3},
