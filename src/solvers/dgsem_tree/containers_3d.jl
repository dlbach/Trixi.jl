...
@muladd begin
    ...
    function Base.resize!(elements::ElementContainer3D, capacity)
        ...
        elements.node_coordinates = unsafe_wrap(
            Array,
    ...
    function ElementContainer3D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
        ...
        _surface_flux_values =
            fill(nan_uEltype, n_variables * n_nodes * n_nodes * 2 * 3 * capacity)
        ...
        for element in eachelement(elements)
            ...
            for k in eachnode(basis), j in eachnode(basis), i in eachnode(basis)
                ...
                elements.node_coordinates[3, i, j, k, element] = (
                    mesh.tree.coordinates[3, cell_id] +
                    jacobian * (nodes[k] - reference_offset)
                )
            end
        end
    ...
    function Base.resize!(interfaces::InterfaceContainer3D, capacity)
        ...
        interfaces.u =
            unsafe_wrap(Array, pointer(_u), (2, n_variables, n_nodes, n_nodes, capacity))
    ...
    function InterfaceContainer3D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
        ...
        u = unsafe_wrap(Array, pointer(_u), (2, n_variables, n_nodes, n_nodes, capacity))
    ...
    function init_interfaces(cell_ids, mesh::TreeMesh3D, elements::ElementContainer3D)
        # Initialize container
    ...
    function Base.resize!(boundaries::BoundaryContainer3D, capacity)
        ...
        boundaries.node_coordinates =
            unsafe_wrap(Array, pointer(_node_coordinates), (3, n_nodes, n_nodes, capacity))
    ...
    function BoundaryContainer3D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {RealT<:Real,uEltype<:Real}
    ...
    function init_boundaries(cell_ids, mesh::TreeMesh3D, elements::ElementContainer3D)
        # Initialize container
    ...
    function Base.resize!(mortars::L2MortarContainer3D, capacity)
        ...
        @unpack _u_upper_left,
        _u_upper_right,
        _u_lower_left,
        _u_lower_right,
        _neighbor_ids,
        large_sides,
        orientations = mortars

        resize!(_u_upper_left, 2 * n_variables * n_nodes * n_nodes * capacity)
        mortars.u_upper_left = unsafe_wrap(
            Array,
            pointer(_u_upper_left),
            (2, n_variables, n_nodes, n_nodes, capacity),
        )

        resize!(_u_upper_right, 2 * n_variables * n_nodes * n_nodes * capacity)
        mortars.u_upper_right = unsafe_wrap(
            Array,
            pointer(_u_upper_right),
            (2, n_variables, n_nodes, n_nodes, capacity),
        )

        resize!(_u_lower_left, 2 * n_variables * n_nodes * n_nodes * capacity)
        mortars.u_lower_left = unsafe_wrap(
            Array,
            pointer(_u_lower_left),
            (2, n_variables, n_nodes, n_nodes, capacity),
        )

        resize!(_u_lower_right, 2 * n_variables * n_nodes * n_nodes * capacity)
        mortars.u_lower_right = unsafe_wrap(
            Array,
            pointer(_u_lower_right),
            (2, n_variables, n_nodes, n_nodes, capacity),
        )

        resize!(_neighbor_ids, 5 * capacity)
        mortars.neighbor_ids = unsafe_wrap(Array, pointer(_neighbor_ids), (5, capacity))
    ...
    function L2MortarContainer3D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
        ...
        neighbor_ids = unsafe_wrap(Array, pointer(_neighbor_ids), (5, capacity))
        ...
        for element in eachelement(elements)
            ...
            for direction in eachdirection(mesh.tree)
                ...
                if direction == 1 # -x
                    ...
                    mortars.neighbor_ids[3, count] =
                        c2e[mesh.tree.child_ids[6, neighbor_cell_id]]
                    mortars.neighbor_ids[4, count] =
                        c2e[mesh.tree.child_ids[8, neighbor_cell_id]]
                elseif direction == 2 # +x
                    ...
                    mortars.neighbor_ids[3, count] =
                        c2e[mesh.tree.child_ids[5, neighbor_cell_id]]
                    mortars.neighbor_ids[4, count] =
                        c2e[mesh.tree.child_ids[7, neighbor_cell_id]]
