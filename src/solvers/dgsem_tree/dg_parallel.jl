...
@muladd begin
    ...
    function init_mpi_data_structures(
        ...
        for index = 1:length(mpi_neighbor_interfaces)
            mpi_send_buffers[index] = Vector{uEltype}(
                undef,
                length(mpi_neighbor_interfaces[index]) * data_size +
                length(mpi_neighbor_mortars[index]) * n_small_elements * 2 * data_size,
            )
            mpi_recv_buffers[index] = Vector{uEltype}(
                undef,
                length(mpi_neighbor_interfaces[index]) * data_size +
                length(mpi_neighbor_mortars[index]) * n_small_elements * 2 * data_size,
            )
