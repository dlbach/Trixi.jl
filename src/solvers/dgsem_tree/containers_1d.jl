...
@muladd begin
    ...
    function Base.resize!(elements::ElementContainer1D, capacity)
        ...
        elements.node_coordinates =
            unsafe_wrap(Array, pointer(_node_coordinates), (1, n_nodes, capacity))
    ...
    function ElementContainer1D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ...
    function Base.resize!(interfaces::InterfaceContainer1D, capacity)
        ...
        interfaces.u = unsafe_wrap(Array, pointer(_u), (2, n_variables, capacity))
    ...
    function InterfaceContainer1D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
        ...
        u = unsafe_wrap(Array, pointer(_u), (2, n_variables, capacity))
    ...
    function init_interfaces(cell_ids, mesh::TreeMesh1D, elements::ElementContainer1D)
        # Initialize container
            ...
            for direction in eachdirection(mesh.tree)
                ...
                if has_neighbor(mesh.tree, cell_id, direction)
                    ...
                    if has_children(mesh.tree, neighbor_cell_id) # Cell has small neighbor
                        interfaces.neighbor_ids[2, count] =
    ...
    function Base.resize!(boundaries::BoundaryContainer1D, capacity)
        ...
        resize!(_u, 2 * n_variables * capacity)
        boundaries.u = unsafe_wrap(Array, pointer(_u), (2, n_variables, capacity))

        resize!(_node_coordinates, 1 * capacity)
        boundaries.node_coordinates =
            unsafe_wrap(Array, pointer(_node_coordinates), (1, capacity))
    ...
    function BoundaryContainer1D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {RealT<:Real,uEltype<:Real}
        ...
        node_coordinates = unsafe_wrap(Array, pointer(_node_coordinates), (1, capacity))
    ...
    function init_boundaries(cell_ids, mesh::TreeMesh1D, elements::ElementContainer1D)
        # Initialize container
