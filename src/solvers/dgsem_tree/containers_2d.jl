...
@muladd begin
    ...
    function Base.resize!(elements::ElementContainer2D, capacity)
        ...
        @unpack _node_coordinates, _surface_flux_values, inverse_jacobian, cell_ids =
            elements
        ...
        elements.node_coordinates =
            unsafe_wrap(Array, pointer(_node_coordinates), (2, n_nodes, n_nodes, capacity))

        resize!(_surface_flux_values, n_variables * n_nodes * 2 * 2 * capacity)
        elements.surface_flux_values = unsafe_wrap(
            Array,
    ...
    function ElementContainer2D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
        ...
        for element in eachelement(elements)
            ...
            for j in eachnode(basis), i in eachnode(basis)
                elements.node_coordinates[1, i, j, element] = (
                    mesh.tree.coordinates[1, cell_id] +
                    jacobian * (nodes[i] - reference_offset)
                )
                elements.node_coordinates[2, i, j, element] = (
                    mesh.tree.coordinates[2, cell_id] +
                    jacobian * (nodes[j] - reference_offset)
                )
            end
        end
    ...
    function Base.resize!(interfaces::InterfaceContainer2D, capacity)
        ...
        interfaces.u = unsafe_wrap(Array, pointer(_u), (2, n_variables, n_nodes, capacity))
    ...
    function InterfaceContainer2D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
    ...
    function init_interfaces(cell_ids, mesh::TreeMesh2D, elements::ElementContainer2D)
        # Initialize container
    ...
    function init_interfaces!(interfaces, elements, mesh::TreeMesh2D)
        ...
        @assert count == ninterfaces(interfaces) (
            "Actual interface count ($count) does not match " *
    ...
    function Base.resize!(boundaries::BoundaryContainer2D, capacity)
        ...
        @unpack _u, _node_coordinates, neighbor_ids, orientations, neighbor_sides =
            boundaries

        resize!(_u, 2 * n_variables * n_nodes * capacity)
        boundaries.u = unsafe_wrap(Array, pointer(_u), (2, n_variables, n_nodes, capacity))

        resize!(_node_coordinates, 2 * n_nodes * capacity)
        boundaries.node_coordinates =
            unsafe_wrap(Array, pointer(_node_coordinates), (2, n_nodes, capacity))
    ...
    function BoundaryContainer2D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {RealT<:Real,uEltype<:Real}
    ...
    function init_boundaries(cell_ids, mesh::TreeMesh2D, elements::ElementContainer2D)
        # Initialize container
    ...
    function init_boundaries!(boundaries, elements, mesh::TreeMesh2D)
        ...
        @assert count == nboundaries(boundaries) (
            "Actual boundaries count ($count) does not match " *
    ...
    function Base.resize!(mortars::L2MortarContainer2D, capacity)
        ...
        @unpack _u_upper, _u_lower, _neighbor_ids, large_sides, orientations = mortars

        resize!(_u_upper, 2 * n_variables * n_nodes * capacity)
        mortars.u_upper =
            unsafe_wrap(Array, pointer(_u_upper), (2, n_variables, n_nodes, capacity))

        resize!(_u_lower, 2 * n_variables * n_nodes * capacity)
        mortars.u_lower =
            unsafe_wrap(Array, pointer(_u_lower), (2, n_variables, n_nodes, capacity))

        resize!(_neighbor_ids, 3 * capacity)
        mortars.neighbor_ids = unsafe_wrap(Array, pointer(_neighbor_ids), (3, capacity))
    ...
    function L2MortarContainer2D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
        ...
        u_upper = unsafe_wrap(Array, pointer(_u_upper), (2, n_variables, n_nodes, capacity))

        _u_lower = fill(nan, 2 * n_variables * n_nodes * capacity)
        u_lower = unsafe_wrap(Array, pointer(_u_lower), (2, n_variables, n_nodes, capacity))

        _neighbor_ids = fill(typemin(Int), 3 * capacity)
        neighbor_ids = unsafe_wrap(Array, pointer(_neighbor_ids), (3, capacity))
    ...
    function init_mortars!(mortars, elements, mesh::TreeMesh2D)
        ...
        for element in eachelement(elements)
            ...
            for direction in eachdirection(mesh.tree)
                ...
                if direction == 1
                    mortars.neighbor_ids[1, count] =
                        c2e[mesh.tree.child_ids[2, neighbor_cell_id]]
                    mortars.neighbor_ids[2, count] =
                        c2e[mesh.tree.child_ids[4, neighbor_cell_id]]
                elseif direction == 2
                    mortars.neighbor_ids[1, count] =
                        c2e[mesh.tree.child_ids[1, neighbor_cell_id]]
                    mortars.neighbor_ids[2, count] =
                        c2e[mesh.tree.child_ids[3, neighbor_cell_id]]
                elseif direction == 3
                    mortars.neighbor_ids[1, count] =
                        c2e[mesh.tree.child_ids[3, neighbor_cell_id]]
                    mortars.neighbor_ids[2, count] =
        ...
        @assert count == nmortars(mortars) (
            "Actual mortar count ($count) does not match " *
    ...
    function Base.resize!(mpi_interfaces::MPIInterfaceContainer2D, capacity)
        ...
        resize!(_u, 2 * n_variables * n_nodes * capacity)
        mpi_interfaces.u =
            unsafe_wrap(Array, pointer(_u), (2, n_variables, n_nodes, capacity))
    ...
    function MPIInterfaceContainer2D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
    ...
    # Return number of interfaces
    nmpiinterfaces(mpi_interfaces::MPIInterfaceContainer2D) =
        length(mpi_interfaces.orientations)


    # Create MPI interface container and initialize MPI interface data in `elements`.
    function init_mpi_interfaces(cell_ids, mesh::TreeMesh2D, elements::ElementContainer2D)
        # Initialize container
    ...
    function init_mpi_interfaces!(mpi_interfaces, elements, mesh::TreeMesh2D)
        ...
        @assert count == nmpiinterfaces(mpi_interfaces) (
            "Actual interface count ($count) does not match " *
    ...
    function MPIL2MortarContainer2D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
    ...
    function init_mpi_mortars!(mpi_mortars, elements, mesh::TreeMesh2D)
        ...
        for element in eachelement(elements)
            ...
            for direction in eachdirection(mesh.tree)
                ...
                if iseven(direction)
                    mpi_mortars.large_sides[count] =
                        is_own_cell(mesh.tree, large_cell_id) ? 1 : 2
                else
                    mpi_mortars.large_sides[count] =
                        is_own_cell(mesh.tree, large_cell_id) ? 2 : 1
                end
