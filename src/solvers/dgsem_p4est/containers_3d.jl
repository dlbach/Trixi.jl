...
@muladd begin
    ...
    function init_elements!(elements, mesh::P4estMesh{3}, basis::LobattoLegendreBasis)
        @unpack node_coordinates, jacobian_matrix, contravariant_vectors, inverse_jacobian =
            elements

        calc_node_coordinates!(node_coordinates, mesh, basis)

        for element = 1:ncells(mesh)
    ...
    function calc_node_coordinates!(
        ...
        for tree in eachindex(trees)
            ...
            for i in eachindex(quadrants)
                ...
                nodes_out_x =
                    2 * (quad_length * 1 / 2 * (nodes .+ 1) .+ quad.x / p4est_root_len) .- 1
                nodes_out_y =
                    2 * (quad_length * 1 / 2 * (nodes .+ 1) .+ quad.y / p4est_root_len) .- 1
                nodes_out_z =
                    2 * (quad_length * 1 / 2 * (nodes .+ 1) .+ quad.z / p4est_root_len) .- 1
    ...
    @inline function init_interface_node_indices!(
        interfaces::P4estInterfaceContainer{3},
        faces,
        orientation,
        interface_id,
    )
        ...
        for side = 1:2
            ...
            if side == 1
                ...
                surface_index1, surface_index2 =
                    orientation_to_indices_p4est(faces[2], faces[1], orientation)
            end

            if faces[side] == 0
                # Index face in negative x-direction
                interfaces.node_indices[side, interface_id] =
                    (:begin, surface_index1, surface_index2)
            elseif faces[side] == 1
                # Index face in positive x-direction
                interfaces.node_indices[side, interface_id] =
                    (:end, surface_index1, surface_index2)
            elseif faces[side] == 2
                # Index face in negative y-direction
                interfaces.node_indices[side, interface_id] =
                    (surface_index1, :begin, surface_index2)
            elseif faces[side] == 3
                # Index face in positive y-direction
                interfaces.node_indices[side, interface_id] =
                    (surface_index1, :end, surface_index2)
            elseif faces[side] == 4
                # Index face in negative z-direction
                interfaces.node_indices[side, interface_id] =
                    (surface_index1, surface_index2, :begin)
            else # faces[side] == 5
                # Index face in positive z-direction
                interfaces.node_indices[side, interface_id] =
                    (surface_index1, surface_index2, :end)
            end
        end
    ...
    @inline function init_boundary_node_indices!(
        boundaries::P4estBoundaryContainer{3},
        face,
        boundary_id,
    )
        if face == 0
    ...
    @inline function init_mortar_node_indices!(
        mortars::P4estMortarContainer{3},
        faces,
        orientation,
        mortar_id,
    )
        for side = 1:2
            ...
            if faces[side] == 0
                # Index face in negative x-direction
                mortars.node_indices[side, mortar_id] =
                    (:begin, surface_index1, surface_index2)
            elseif faces[side] == 1
                # Index face in positive x-direction
                mortars.node_indices[side, mortar_id] =
