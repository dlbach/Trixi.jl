...
@muladd begin
    ...
    @inline nmpiinterfaces(interfaces::P4estMPIInterfaceContainer) =
    ...
    function init_mpi_interfaces(mesh::ParallelP4estMesh, equations, basis, elements)
        ...
        _u = Vector{uEltype}(
            undef,
            2 * nvariables(equations) * nnodes(basis)^(NDIMS - 1) * n_mpi_interfaces,
        )
        u = unsafe_wrap(
            Array,
            pointer(_u),
    ...
    mutable struct P4estMPIMortarContainer{
        NDIMS,
        uEltype<:Real,
        RealT<:Real,
        NDIMSP1,
    ...
    @inline nmpimortars(mpi_mortars::P4estMPIMortarContainer) =
    ...
    function Base.resize!(mpi_mortars::P4estMPIMortarContainer, capacity)
        ...
        mpi_mortars.normal_directions = unsafe_wrap(
            Array,
    ...
    function init_mpi_mortars(mesh::ParallelP4estMesh, equations, basis, elements)
        ...
        _normal_directions = Vector{RealT}(
            undef,
            NDIMS * nnodes(basis)^(NDIMS - 1) * 2^(NDIMS - 1) * n_mpi_mortars,
        )
        normal_directions = unsafe_wrap(
            Array,
            pointer(_normal_directions),
        ...
        mpi_mortars =
            P4estMPIMortarContainer{NDIMS,uEltype,RealT,NDIMS + 1,NDIMS + 2,NDIMS + 3}(
                u,
                local_neighbor_ids,
    ...
    mutable struct ParallelInitSurfacesIterFaceUserData{
        Interfaces,
        Mortars,
        Boundaries,
        MPIInterfaces,
    ...
    function ParallelInitSurfacesIterFaceUserData(
        interfaces,
        mortars,
        boundaries,
        mpi_interfaces,
    ...
    function init_surfaces_iter_face_parallel(info, user_data)
        # Unpack user_data
        data =
            unsafe_pointer_to_objref(Ptr{ParallelInitSurfacesIterFaceUserData}(user_data))
    ...
    # 2D
    cfunction(::typeof(init_surfaces_iter_face_parallel), ::Val{2}) = @cfunction(
        init_surfaces_iter_face_parallel,
        Cvoid,
        (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(init_surfaces_iter_face_parallel), ::Val{3}) = @cfunction(
        init_surfaces_iter_face_parallel,
        Cvoid,
    ...
    function init_surfaces_iter_face_inner(
        info,
    ...
    function init_mpi_interfaces_iter_face_inner(info, sides, user_data)
        ...
        init_mpi_interface_node_indices!(
            mpi_interfaces,
            faces,
            local_side,
    ...
    function init_mpi_mortars_iter_face_inner(info, sides, user_data)
        ...
        local_small_quad_positions =
            findall(sides[hanging_side].is.hanging.is_ghost .== false)
        ...
        tree_small_quad_ids =
            map(p -> sides[hanging_side].is.hanging.quadid[p], local_small_quad_positions)
    ...
    # 2D
    cfunction(::typeof(count_surfaces_iter_face_parallel), ::Val{2}) = @cfunction(
        count_surfaces_iter_face_parallel,
        Cvoid,
        (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(count_surfaces_iter_face_parallel), ::Val{3}) = @cfunction(
        count_surfaces_iter_face_parallel,
        Cvoid,
        (Ptr{p8est_iter_face_info_t}, Ptr{Cvoid})
    )
