...
@muladd begin
    ...
    function P4estMPICache(uEltype)
        ...
        if !isbitstype(uEltype)
            throw(
                ArgumentError(
                    "P4estMPICache only supports bitstypes, $uEltype is not a bitstype.",
                ),
            )
        end
    ...
    function start_mpi_send!(mpi_cache::P4estMPICache, mesh, equations, dg, cache)
        ...
        for d = 1:length(mpi_cache.mpi_neighbor_ranks)
            ...
            for (index, interface) in enumerate(mpi_cache.mpi_neighbor_interfaces[d])
                ...
                @views send_buffer[first:last] .=
                    vec(cache.mpi_interfaces.u[local_side, .., interface])
            end
            ...
            mortars_data_size =
                length(mpi_cache.mpi_neighbor_mortars[d]) * n_small_elements * 2 * data_size
            # `NaN |> eltype(...)` ensures that the NaN's are of the appropriate floating point type
            send_buffer[interfaces_data_size+1:interfaces_data_size+mortars_data_size] .=
                NaN |> eltype(mpi_cache)

            for (index, mortar) in enumerate(mpi_cache.mpi_neighbor_mortars[d])
                index_base =
                    interfaces_data_size + (index - 1) * n_small_elements * 2 * data_size
                ...
                for position in cache.mpi_mortars.local_neighbor_positions[mortar]
                    ...
                    if position > n_small_elements # large element
                        @views send_buffer[first:last] .=
                            vec(cache.mpi_mortars.u[2, :, :, .., mortar])
                    else # small element
                        @views send_buffer[first:last] .=
                            vec(cache.mpi_mortars.u[1, :, position, .., mortar])
                    end
                end
            end
        end
        ...
        for (index, d) in enumerate(mpi_cache.mpi_neighbor_ranks)
            mpi_cache.mpi_send_requests[index] =
                MPI.Isend(mpi_cache.mpi_send_buffers[index], d, mpi_rank(), mpi_comm())
        end
    ...
    function start_mpi_receive!(mpi_cache::P4estMPICache)
        for (index, d) in enumerate(mpi_cache.mpi_neighbor_ranks)
            mpi_cache.mpi_recv_requests[index] =
                MPI.Irecv!(mpi_cache.mpi_recv_buffers[index], d, d, mpi_comm())
        end
        ...
        while d !== nothing
            ...
            for (index, interface) in enumerate(mpi_cache.mpi_neighbor_interfaces[d])
                ...
                if cache.mpi_interfaces.local_sides[interface] == 1 # local element on primary side
                    @views vec(cache.mpi_interfaces.u[2, .., interface]) .=
                        recv_buffer[first:last]
                else # local element at secondary side
                    @views vec(cache.mpi_interfaces.u[1, .., interface]) .=
                        recv_buffer[first:last]
                end
            end
            ...
            for (index, mortar) in enumerate(mpi_cache.mpi_neighbor_mortars[d])
                ...
                for position = 1:n_positions
                    ...
                    if position == n_positions # large element
                        @views vec(cache.mpi_mortars.u[2, :, :, .., mortar]) .=
                            recv_buffer[first:last]
                    else # small element
                        @views vec(cache.mpi_mortars.u[1, :, position, .., mortar]) .=
                            recv_buffer[first:last]
                    end
                end
            end
    ...
    # size `2 * data_size`.
    @inline function buffer_mortar_indices(
        ...
        return (
            ...
            (index_base + 1, index_base + 1 * data_size),
            # first, last for local element in position 2 (small element)
            (index_base + 1 * data_size + 1, index_base + 2 * data_size),
            # first, last for local element in position 3 (large element)
            (index_base + 2 * data_size + 1, index_base + 4 * data_size),
        )
    end
    ...
    @inline function buffer_mortar_indices(
        ...
        return (
            ...
            (index_base + 2 * data_size + 1, index_base + 3 * data_size),
            # first, last for local element in position 4 (small element)
            (index_base + 3 * data_size + 1, index_base + 4 * data_size),
            # first, last for local element in position 5 (large element)
            (index_base + 4 * data_size + 1, index_base + 8 * data_size),
        )
    end
    ...
    function init_mpi_cache!(
        ...
        n_elements_by_rank =
            vcat(
                ...
                n_elements_global,
            ) |> diff # diff sufficient due to 0-based quad indices
        n_elements_by_rank = OffsetArray(n_elements_by_rank, 0:(mpi_nranks()-1))
        # Account for 1-based indexing in Julia
        first_element_global_id =
            Int(
                unsafe_load(unsafe_load(mesh.p4est).global_first_quadrant, mpi_rank() + 1),
            ) + 1
    ...
    function init_mpi_neighbor_connectivity(
        mpi_interfaces,
        mpi_mortars,
        mesh::ParallelP4estMesh,
    )
        ...
        user_data =
            InitNeighborRankConnectivityIterFaceUserData(mpi_interfaces, mpi_mortars, mesh)

        iterate_p4est(
            mesh.p4est,
            user_data;
            ghost_layer = mesh.ghost,
            iter_face_c = iter_face_c,
        )

        # Build proper connectivity data structures from information gathered by iterating over p4est
        @unpack global_interface_ids,
        neighbor_ranks_interface,
        global_mortar_ids,
        neighbor_ranks_mortar = user_data

        mpi_neighbor_ranks =
            vcat(neighbor_ranks_interface, neighbor_ranks_mortar...) |> sort |> unique
        ...
        for (index, d) in enumerate(mpi_neighbor_ranks)
            mpi_neighbor_interfaces[index] =
                interface_ids[findall(==(d), neighbor_ranks_interface)]
            mpi_neighbor_mortars[index] =
                mortar_ids[findall(x -> (d in x), neighbor_ranks_mortar)]
        end
    ...
    mutable struct InitNeighborRankConnectivityIterFaceUserData{
        MPIInterfaces,
        MPIMortars,
        Mesh,
    }
    ...
    function init_neighbor_rank_connectivity_iter_face(info, user_data)
        data = unsafe_pointer_to_objref(
            Ptr{InitNeighborRankConnectivityIterFaceUserData}(user_data),
        )
    ...
    # 2D
    cfunction(::typeof(init_neighbor_rank_connectivity_iter_face), ::Val{2}) = @cfunction(
        init_neighbor_rank_connectivity_iter_face,
        Cvoid,
        (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(init_neighbor_rank_connectivity_iter_face), ::Val{3}) = @cfunction(
        init_neighbor_rank_connectivity_iter_face,
        Cvoid,
    ...
    function init_neighbor_rank_connectivity_iter_face_inner(info, user_data)
        ...
        if unsafe_load(info).sides.elem_count == 2 # MPI interfaces/mortars have two neighboring elements
            ...
            if sides[1].is_hanging == false && sides[2].is_hanging == false # No hanging nodes for MPI interfaces
                ...
                neighbor_rank =
                    findfirst(
                        r -> proc_offsets[r] <= ghost_id < proc_offsets[r+1],
                        1:mpi_nranks(),
                    ) - 1 # MPI ranks are 0-based
                ...
                if local_side == 1
                    ...
                    primary_quad_id =
                        offset + unsafe_load(sides[1].is.full.quad.p.piggy3.local_num)
                end
                ...
                trees = (
                    unsafe_load_tree(mesh.p4est, sides[1].treeid + 1),
                ...
                remote_small_quad_positions =
                    findall(sides[hanging_side].is.hanging.is_ghost .== true)
                proc_offsets = unsafe_wrap(
                    Array,
                    unsafe_load(unsafe_load(info).ghost_layer).proc_offsets,
                    mpi_nranks() + 1,
                ...
                ghost_ids = map(
                    pos -> sides[hanging_side].is.hanging.quadid[pos],
                    remote_small_quad_positions,
                )
                ...
                if sides[full_side].is.full.is_ghost == true
                    ...
                    large_quad_id =
                        offset +
                        trees[full_side].quadrants_offset +
                ...
                global_mortar_ids[mortar_id] =
                    2 * ndims(mesh) * large_quad_id + sides[full_side].face
    ...
    function exchange_normal_directions!(
        ...
        for index = 1:length(mpi_neighbor_mortars)
            send_buffers[index] = Vector{RealT}(
                undef,
                length(mpi_neighbor_mortars[index]) * n_small_elements * data_size,
            )
            send_buffers[index] .= NaN |> RealT
            recv_buffers[index] = Vector{RealT}(
                undef,
                length(mpi_neighbor_mortars[index]) * n_small_elements * data_size,
            )
        ...
        # Fill send buffers
        for d = 1:length(mpi_neighbor_ranks)
            ...
            for (index, mortar) in enumerate(mpi_neighbor_mortars[d])
                ...
                for position in mpi_mortars.local_neighbor_positions[mortar]
                    if position <= n_small_elements # element is small
                        first, last = indices[position]
                        @views send_buffer[first:last] .=
                            vec(mpi_mortars.normal_directions[:, .., position, mortar])
                    end
                end
            end
        end
        ...
        while d !== nothing
            ...
            for (index, mortar) in enumerate(mpi_neighbor_mortars[d])
                index_base = (index - 1) * n_small_elements * data_size
                indices = buffer_mortar_indices(mesh, index_base, data_size)
                for position = 1:n_small_elements
                    # Skip if received data for `position` is NaN as no real data has been sent for the
                    # corresponding element
                    if isnan(recv_buffer[Base.first(indices[position])])
                        continue
                    ...
                    first, last = indices[position]
                    @views vec(mpi_mortars.normal_directions[:, .., position, mortar]) .=
                        recv_buffer[first:last]
                end
            end
    ...
    @inline function get_normal_direction(mpi_mortars::P4estMPIMortarContainer, indices...)
        SVector(
            ntuple(
                @inline(dim -> mpi_mortars.normal_directions[dim, indices...]),
