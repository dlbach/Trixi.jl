...
@muladd begin
    ...
    mutable struct P4estElementContainer{
        NDIMS,
        RealT<:Real,
        uEltype<:Real,
        NDIMSP1,
    ...
    @inline nelements(elements::P4estElementContainer) =
        size(elements.node_coordinates, ndims(elements) + 2)
    @inline Base.ndims(::P4estElementContainer{NDIMS}) where {NDIMS} = NDIMS
    @inline Base.eltype(
        ::P4estElementContainer{NDIMS,RealT,uEltype},
    ) where {NDIMS,RealT,uEltype} = uEltype
    ...
    function Base.resize!(elements::P4estElementContainer, capacity)
        @unpack _node_coordinates,
        _jacobian_matrix,
        _contravariant_vectors,
        _inverse_jacobian,
        _surface_flux_values = elements
        ...
        elements.jacobian_matrix = unsafe_wrap(
            Array,
        ...
        elements.contravariant_vectors = unsafe_wrap(
            Array,
        ...
        elements.inverse_jacobian = unsafe_wrap(
            Array,
    ...
    @inline ninterfaces(interfaces::P4estInterfaceContainer) =
    ...
    function init_interfaces(mesh::P4estMesh, equations, basis, elements)
        ...
        _u = Vector{uEltype}(
            undef,
            2 * nvariables(equations) * nnodes(basis)^(NDIMS - 1) * n_interfaces,
        )
        u = unsafe_wrap(
            Array,
            pointer(_u),
    ...
    @inline nboundaries(boundaries::P4estBoundaryContainer) =
    ...
    function init_boundaries(mesh::P4estMesh, equations, basis, elements)
        ...
        _u = Vector{uEltype}(
            undef,
            nvariables(equations) * nnodes(basis)^(NDIMS - 1) * n_boundaries,
        )
        u = unsafe_wrap(
            Array,
            pointer(_u),
        ...
        boundaries = P4estBoundaryContainer{NDIMS,uEltype,NDIMS + 1}(
            u,
            neighbor_ids,
            node_indices,
            names,
    ...
    function Base.resize!(mortars::P4estMortarContainer, capacity)
        ...
        mortars.neighbor_ids =
            unsafe_wrap(Array, pointer(_neighbor_ids), (2^(n_dims - 1) + 1, capacity))
    ...
    function init_mortars(mesh::P4estMesh, equations, basis, elements)
        ...
        neighbor_ids =
            unsafe_wrap(Array, pointer(_neighbor_ids), (2^(NDIMS - 1) + 1, n_mortars))
        ...
        mortars = P4estMortarContainer{NDIMS,uEltype,NDIMS + 1,NDIMS + 3}(
            u,
            neighbor_ids,
            node_indices,
            _u,
    ...
    # 2D
    cfunction(::typeof(init_surfaces_iter_face), ::Val{2}) = @cfunction(
        init_surfaces_iter_face,
        Cvoid,
        (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(init_surfaces_iter_face), ::Val{3}) = @cfunction(
        init_surfaces_iter_face,
        Cvoid,
    ...
    function init_interfaces_iter_face_inner(info, sides, user_data)
        ...
        offsets = SVector(trees[1].quadrants_offset, trees[2].quadrants_offset)
    ...
    # 2D
    cfunction(::typeof(count_surfaces_iter_face), ::Val{2}) = @cfunction(
        count_surfaces_iter_face,
        Cvoid,
        (Ptr{p4est_iter_face_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(count_surfaces_iter_face), ::Val{3}) = @cfunction(
        count_surfaces_iter_face,
        Cvoid,
        (Ptr{p8est_iter_face_info_t}, Ptr{Cvoid})
    )
