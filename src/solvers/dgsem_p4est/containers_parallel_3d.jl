...
@muladd begin
    ...
    @inline function init_mpi_interface_node_indices!(
        mpi_interfaces::P4estMPIInterfaceContainer{3},
        faces,
        local_side,
        orientation,
        mpi_interface_id,
        ...
        if faces[local_side] == 0
            ...
            mpi_interfaces.node_indices[mpi_interface_id] =
    ...
    @inline function init_mortar_node_indices!(
        mortars::P4estMPIMortarContainer{3},
        faces,
        orientation,
        mortar_id,
    )
    ...
    function init_normal_directions!(
        mpi_mortars::P4estMPIMortarContainer{3},
        basis,
        elements,
    )
        ...
        @threaded for mortar = 1:nmpimortars(mpi_mortars)
            ...
            for (element, position) in
                ...
                for j in eachnode(basis)
                    for i in eachnode(basis)
                        ...
                        @views mpi_mortars.normal_directions[:, i, j, position, mortar] .=
                            normal_direction
