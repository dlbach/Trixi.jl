...
@muladd begin
    ...
    function prolong2mpiinterfaces!(
        ...
        @threaded for interface in eachmpiinterface(dg, cache)
            ...
            i_element_start, i_element_step =
                index_to_start_step_2d(local_indices[1], index_range)
            j_element_start, j_element_step =
                index_to_start_step_2d(local_indices[2], index_range)

            i_element = i_element_start
            j_element = j_element_start
            for i in eachnode(dg)
                for v in eachvariable(equations)
                    mpi_interfaces.u[local_side, v, i, interface] =
                        u[v, i_element, j_element, local_element]
                end
    ...
    @inline function calc_mpi_interface_flux!(
        ...
        u_ll, u_rr =
            get_surface_node_vars(u, equations, dg, interface_node_index, interface_index)
        ...
        @threaded for mortar in eachmpimortar(dg, cache)
            ...
            for (element, position) in zip(local_neighbor_ids, local_neighbor_positions)
                if position == 3 # -> large element
                    ...
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            cache.mpi_mortars.u[1, v, position, i, mortar] =
        ...
        @threaded for mortar in eachmpimortar(dg, cache)
            ...
            for position = 1:2
                ...
                for node in eachnode(dg)
                    ...
                    normal_direction =
                        get_normal_direction(cache.mpi_mortars, node, position, mortar)
        ...
        for (element, position) in
            ...
            if position == 3 # -> large element
                ...
                if :i_backward in large_indices
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            surface_flux_values[v, end+1-i, large_direction, element] =
                                u_buffer[v, i]
                        end
                    end
                else
                    for i in eachnode(dg)
                        for v in eachvariable(equations)
                            surface_flux_values[v, i, large_direction, element] =
                                u_buffer[v, i]
                        end
                    end
                end
