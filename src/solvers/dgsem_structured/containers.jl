...
@muladd begin
    ...
    function init_elements(
        mesh::StructuredMesh{NDIMS,RealT},
        ...
        node_coordinates = Array{RealT,NDIMS + 2}(
            undef,
            NDIMS,
        ...
        jacobian_matrix = Array{RealT,NDIMS + 3}(
            undef,
            NDIMS,
            NDIMS,
            ntuple(_ -> nnodes(basis), NDIMS)...,
        ...
        inverse_jacobian =
            Array{RealT,NDIMS + 1}(undef, ntuple(_ -> nnodes(basis), NDIMS)..., nelements)
        surface_flux_values = Array{uEltype,NDIMS + 2}(
            undef,
    ...
    Base.eltype(::ElementContainer{NDIMS,RealT,uEltype}) where {NDIMS,RealT,uEltype} =
        uEltype
