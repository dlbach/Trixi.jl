...
@muladd begin
    ...
    function init_elements!(elements, mesh::StructuredMesh{3}, basis::LobattoLegendreBasis)
        ...
        for cell_z = 1:size(mesh, 3), cell_y = 1:size(mesh, 2), cell_x = 1:size(mesh, 1)
    ...
    function calc_node_coordinates!(
        ...
        for k in eachnode(basis), j in eachnode(basis), i in eachnode(basis)
            ...
            node_coordinates[:, i, j, k, element] .= mapping(
    ...
    function calc_jacobian_matrix!(
        jacobian_matrix::AbstractArray{<:Any,6},
        element,
        node_coordinates,
        basis,
    )
    ...
    function calc_contravariant_vectors!(
        contravariant_vectors::AbstractArray{<:Any,6},
        element,
        jacobian_matrix,
        node_coordinates,
        basis::LobattoLegendreBasis,
    ...
    function calc_inverse_jacobian!(
        inverse_jacobian::AbstractArray{<:Any,4},
        element,
        jacobian_matrix,
        basis,
    )
    ...
    function initialize_left_neighbor_connectivity!(
        left_neighbors,
        mesh::StructuredMesh{3},
        linear_indices,
    )
        # Neighbors in x-direction
        for cell_z = 1:size(mesh, 3), cell_y = 1:size(mesh, 2)
            # Inner elements
            for cell_x = 2:size(mesh, 1)
                element = linear_indices[cell_x, cell_y, cell_z]
            ...
            if isperiodic(mesh, 1)
                # Periodic boundary
                left_neighbors[1, linear_indices[1, cell_y, cell_z]] =
                    linear_indices[end, cell_y, cell_z]
            else
        ...
        for cell_z = 1:size(mesh, 3), cell_x = 1:size(mesh, 1)
            # Inner elements
            for cell_y = 2:size(mesh, 2)
                element = linear_indices[cell_x, cell_y, cell_z]
            ...
            if isperiodic(mesh, 2)
                # Periodic boundary
                left_neighbors[2, linear_indices[cell_x, 1, cell_z]] =
                    linear_indices[cell_x, end, cell_z]
            else
        ...
        for cell_y = 1:size(mesh, 2), cell_x = 1:size(mesh, 1)
            # Inner elements
            for cell_z = 2:size(mesh, 3)
                element = linear_indices[cell_x, cell_y, cell_z]
            ...
            if isperiodic(mesh, 3)
                # Periodic boundary
                left_neighbors[3, linear_indices[cell_x, cell_y, 1]] =
                    linear_indices[cell_x, cell_y, end]
            else
