...
@muladd begin
    ...
    @inline function weak_form_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            Ja11, Ja12, Ja13 =
                get_contravariant_vector(1, contravariant_vectors, i, j, k, element)
            ...
            Ja21, Ja22, Ja23 =
                get_contravariant_vector(2, contravariant_vectors, i, j, k, element)
            ...
            Ja31, Ja32, Ja33 =
                get_contravariant_vector(3, contravariant_vectors, i, j, k, element)
    ...
    @inline function flux_differencing_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii = (i+1):nnodes(dg)
                ...
                Ja1_node_ii =
                    get_contravariant_vector(1, contravariant_vectors, ii, j, k, element)
            ...
            for jj = (j+1):nnodes(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja2_node_jj =
                    get_contravariant_vector(2, contravariant_vectors, i, jj, k, element)
            ...
            for kk = (k+1):nnodes(dg)
                ...
                # pull the contravariant vectors and compute the average
                Ja3_node_kk =
                    get_contravariant_vector(3, contravariant_vectors, i, j, kk, element)
    ...
    @inline function flux_differencing_kernel!(
        ...
        for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                ...
                # averaged contravariant vector
                fluxtilde1 =
                    nonconservative_flux(u_node, u_node_ii, Ja1_node, Ja1_avg, equations)
                integral_contribution =
                    integral_contribution + derivative_split[i, ii] * fluxtilde1
            end
            ...
            for jj in eachnode(dg)
                ...
                # averaged contravariant vector
                fluxtilde2 =
                    nonconservative_flux(u_node, u_node_jj, Ja2_node, Ja2_avg, equations)
                integral_contribution =
                    integral_contribution + derivative_split[j, jj] * fluxtilde2
            end
            ...
            for kk in eachnode(dg)
                ...
                # averaged contravariant vector
                fluxtilde3 =
                    nonconservative_flux(u_node, u_node_kk, Ja3_node, Ja3_avg, equations)
                integral_contribution =
                    integral_contribution + derivative_split[k, kk] * fluxtilde3
            end
    ...
    @inline function calcflux_fv!(
        ...
        for k in eachnode(dg), j in eachnode(dg)
            normal_direction =
                get_contravariant_vector(1, contravariant_vectors, 1, j, k, element)
            ...
            for i = 2:nnodes(dg)
                ...
                for m = 1:nnodes(dg)
                    normal_direction +=
                        weights[i-1] *
                        derivative_matrix[i-1, m] *
                        get_contravariant_vector(1, contravariant_vectors, m, j, k, element)
                end
        ...
        for k in eachnode(dg), i in eachnode(dg)
            normal_direction =
                get_contravariant_vector(2, contravariant_vectors, i, 1, k, element)
            ...
            for j = 2:nnodes(dg)
                ...
                for m = 1:nnodes(dg)
                    normal_direction +=
                        weights[j-1] *
                        derivative_matrix[j-1, m] *
                        get_contravariant_vector(2, contravariant_vectors, i, m, k, element)
                end
        ...
        for j in eachnode(dg), i in eachnode(dg)
            normal_direction =
                get_contravariant_vector(3, contravariant_vectors, i, j, 1, element)
            ...
            for k = 2:nnodes(dg)
                ...
                for m = 1:nnodes(dg)
                    normal_direction +=
                        weights[k-1] *
                        derivative_matrix[k-1, m] *
                        get_contravariant_vector(3, contravariant_vectors, i, j, m, element)
                end
    ...
    @inline function calc_interface_flux!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            if orientation == 1
                ...
                normal_direction =
                    sign_jacobian * get_contravariant_vector(
                        1,
    ...
    @inline function calc_interface_flux!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            # from the flux above
            noncons_left =
                sign_jacobian * nonconservative_flux(
                    u_ll,
                    u_rr,
                    normal_direction,
                    normal_direction,
            ...
            for v in eachvariable(equations)
                ...
                # the interpretation of global SBP operators coupled discontinuously via
                # central fluxes/SATs
                surface_flux_values[v, i, j, right_direction, left_element] =
                    flux[v] + 0.5 * noncons_left[v]
                surface_flux_values[v, i, j, left_direction, right_element] =
