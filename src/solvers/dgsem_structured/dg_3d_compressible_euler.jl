...
@inline function flux_differencing_kernel!(
    ...
    du_permuted = StrideArray{eltype(u_cons)}(
        undef,
        (StaticInt(nnodes(dg)^2), StaticInt(nnodes(dg)), StaticInt(nvariables(equations))),
    )
    ...
    contravariant_vectors_x = StrideArray{eltype(contravariant_vectors)}(
        undef,
        (StaticInt(nnodes(dg)^2), StaticInt(nnodes(dg)), StaticInt(ndims(mesh))),
    )
    ...
    GC.@preserve u_prim begin
        ...
        # We must also permute the contravariant vectors.
        contravariant_vectors_z = StrideArray{eltype(contravariant_vectors)}(
            undef,
            (StaticInt(nnodes(dg)^2), StaticInt(nnodes(dg)), StaticInt(ndims(mesh))),
        )
        ...
        @turbo for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
            ij = i + nnodes(dg) * (j - 1)
            contravariant_vectors_z[ij, k, 1] =
                contravariant_vectors[1, 3, i, j, k, element]
            contravariant_vectors_z[ij, k, 2] =
                contravariant_vectors[2, 3, i, j, k, element]
            contravariant_vectors_z[ij, k, 3] =
                contravariant_vectors[3, 3, i, j, k, element]
        end

        for k in eachnode(dg), kk = (k+1):nnodes(dg)
    ...
    GC.@preserve u_prim begin
        ...
        for k in eachnode(dg), kk = (k+1):nnodes(dg)
            @turbo for ij in Base.OneTo(nnodes(dg)^2)
                ...
                regular_path2 = (log_y2 - log_x2) / y2_minus_x2
                inv_rho_p_mean =
                    p_ll * p_rr * ifelse(z2 < 1.0e-4, special_path2, regular_path2)
                ...
                f5 = (
                    f1 * (
                        velocity_square_avg +
                        inv_rho_p_mean * equations.inv_gamma_minus_one
                    ) + 0.5 * (p_ll * v_dot_n_rr + p_rr * v_dot_n_ll)
                )
