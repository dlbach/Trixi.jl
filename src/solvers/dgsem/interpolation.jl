...
# implemented below.
function multiply_dimensionwise_naive(
...
function multiply_dimensionwise_naive(
    ...
    n_vars = size(data_in, 1)
    data_out =
        zeros(promote_type(eltype(data_in), eltype(matrix)), n_vars, size_out, size_out)

    for j = 1:size_out, i = 1:size_out
        for jj = 1:size_in, ii = 1:size_in
            for v = 1:n_vars
...
function multiply_dimensionwise_naive(
    ...
    n_vars = size(data_in, 1)
    data_out = zeros(
        promote_type(eltype(data_in), eltype(matrix)),
        n_vars,
        size_out,
        size_out,
        size_out,
    ...
    for k = 1:size_out, j = 1:size_out, i = 1:size_out
        for kk = 1:size_in, jj = 1:size_in, ii = 1:size_in
            for v = 1:n_vars
                data_out[v, i, j, k] +=
                    matrix[i, ii] * matrix[j, jj] * matrix[k, kk] * data_in[v, ii, jj, kk]
            end
        end
    end
...
# 1D version
function multiply_dimensionwise!(
...
function add_multiply_dimensionwise!(
    data_out::AbstractArray{<:Any,3},
    matrix1::AbstractMatrix,
    matrix2::AbstractMatrix,
    data_in::AbstractArray{<:Any,3},
...
function multiply_dimensionwise!(
    ...
    @turbo for k in axes(data_out, 4),
        j in axes(data_out, 3),
        i in axes(data_out, 2),
