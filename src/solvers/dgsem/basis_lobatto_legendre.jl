...
@muladd begin
    ...
    struct LobattoLegendreMortarL2{
        RealT<:Real,
        NNODES,
    ...
    @inline nnodes(mortar::LobattoLegendreMortarL2{RealT,NNODES}) where {RealT,NNODES} =
        NNODES
    ...
    struct LobattoLegendreAnalyzer{
        RealT<:Real,
        NNODES,
        VectorT<:AbstractVector{RealT},
    ...
    @inline nnodes(analyzer::LobattoLegendreAnalyzer{RealT,NNODES}) where {RealT,NNODES} =
        NNODES
    """
    ...
    struct LobattoLegendreAdaptorL2{
        RealT<:Real,
        NNODES,
        ForwardMatrix<:AbstractMatrix{RealT},
        ReverseMatrix<:AbstractMatrix{RealT},
    ...
    @inline nnodes(adaptor::LobattoLegendreAdaptorL2{RealT,NNODES}) where {RealT,NNODES} =
        NNODES
    ...
    function polynomial_interpolation_matrix(
        nodes_in,
        nodes_out,
        ...
        vandermonde = Matrix{promote_type(eltype(nodes_in), eltype(nodes_out))}(
            undef,
            n_nodes_out,
            n_nodes_in,
        )
