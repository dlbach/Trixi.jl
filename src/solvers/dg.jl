...
@muladd begin
    ...
    function VolumeIntegralShockCapturingHG(
        indicator;
    ...
    @inline ndofs(mesh::MeshesDGSEM, dg::DG, cache) =
    ...
    @inline ndofsglobal(mesh, dg::DG, cache) =
    ...
    @inline nelementsglobal(dg::DG, cache) =
    ...
    @inline function get_surface_node_vars(u, equations, solver::DG, indices...)
        ...
        # `Val(...)` below.
        u_ll =
            SVector(ntuple(@inline(v -> u[1, v, indices...]), Val(nvariables(equations))))
        u_rr =
            SVector(ntuple(@inline(v -> u[2, v, indices...]), Val(nvariables(equations))))
        return u_ll, u_rr
    end
    ...
    @inline function wrap_array(
        ...
        if LoopVectorization.check_args(u_ode)
            ...
            unsafe_wrap(
                Array{eltype(u_ode),ndims(mesh) + 2},
                pointer(u_ode),
                (
    ...
    @inline function wrap_array_native(
        u_ode::AbstractVector,
        mesh::AbstractMesh,
        equations,
        dg::DG,
