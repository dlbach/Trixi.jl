...
@muladd begin
    ...
    function create_cache(
        ...
        f_threaded = [similar(prototype) for _ = 1:Threads.nthreads()]
    ...
    function create_cache(
        ...
        u_node = SVector{nvariables(equations),uEltype}(
            ntuple(_ -> zero(uEltype), Val{nvariables(equations)}()),
        )
    ...
    function calc_volume_integral!(
        ...
        if nvariables(equations) == 1
            ...
            u_vectors = reshape(
                reinterpret(SVector{nvariables(equations),eltype(u)}, u),
                nnodes(dg),
                nnodes(dg),
                nelements(dg, cache),
            )
            du_vectors = reshape(
                reinterpret(SVector{nvariables(equations),eltype(du)}, du),
                nnodes(dg),
                nnodes(dg),
                nelements(dg, cache),
            )
    ...
    function calc_interface_flux!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                flux_minus_rr =
                    splitting(u_rr, Val{:minus}(), orientations[interface], equations)
                flux_plus_ll =
                    splitting(u_ll, Val{:plus}(), orientations[interface], equations)
        ...
        for element in eachelement(dg, cache)
            ...
            for j in eachnode(analyzer), i in eachnode(analyzer)
                u_exact = initial_condition(
                    get_node_coords(node_coordinates, equations, dg, i, j, element),
                    t,
                    equations,
                )
                diff =
