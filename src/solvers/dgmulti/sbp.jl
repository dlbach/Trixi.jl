...
function StartUpDG.RefElemData(
    element_type::Line,
    ...
    V1 =
        StartUpDG.vandermonde(element_type, 1, r) /
...
function StartUpDG.RefElemData(
    ...
    rf, sf, wf, nrJ, nsJ =
        StartUpDG.init_face_data(element_type, quad_rule_face = (nodes_1d, weights_1d))
    if D isa AbstractPeriodicDerivativeOperator
        # we do not need any face stuff for periodic operators
        Vf = spzeros(length(wf), length(wq))
    else
    ...
    V1 =
        StartUpDG.vandermonde(element_type, 1, r, s) /
...
function StartUpDG.RefElemData(
    ...
    V1 =
        StartUpDG.vandermonde(element_type, 1, r, s, t) /
...
function StartUpDG.inverse_trace_constant(
    rd::RefElemData{NDIMS,ElementType,ApproximationType},
) where {
    NDIMS,
...
const DGMultiPeriodicFDSBP{NDIMS,ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
    SurfaceIntegral,
    ...
    NDIMS,
    ElemType,
    ApproxType<:SummationByPartsOperators.AbstractPeriodicDerivativeOperator,
    SurfaceIntegral,
    VolumeIntegral,
}

const DGMultiFluxDiffPeriodicFDSBP{NDIMS,ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
    SurfaceIntegral,
    ...
    ElemType,
    ApproxType<:SummationByPartsOperators.AbstractPeriodicDerivativeOperator,
    SurfaceIntegral<:SurfaceIntegralWeakForm,
    VolumeIntegral<:VolumeIntegralFluxDifferencing,
}
...
function DGMultiMesh(
    ...
    md = MeshData(
        StartUpDG.VertexMappedMesh(mesh_type, VXYZ, EToV),
        FToF,
        xyz,
        xyzf,
        xyzq,
    ...
    boundary_faces = []
    return DGMultiMesh{NDIMS,rd.element_type,typeof(md),typeof(boundary_faces)}(
        md,
        boundary_faces,
    )
end
...
@muladd begin
    ...
    function create_cache(
        mesh::DGMultiMesh,
        equations,
        dg::DGMultiFluxDiffPeriodicFDSBP,
        RealT,
        uEltype,
    )
            ...
            for dim in eachdim(mesh)
                ...
                @threaded for i in row_ids
                    ...
                    for id in nzrange(A_base, i)
                        ...
                        A_ij = vals[id]
                        AF_ij =
                            2 * A_ij * volume_flux(u_i, u_j, normal_direction, equations)
                        du_i = du_i + AF_ij
                    end
                    du[i] = du_i
                end
