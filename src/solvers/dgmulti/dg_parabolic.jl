...
function create_cache_parabolic(
    ...
    gradients = SVector{ndims(mesh)}(
        ntuple(
            _ -> similar(u_transformed, (dg.basis.Nq, mesh.md.num_elements)),
            ndims(mesh),
        ),
    )
    ...
    local_u_values_threaded =
        [similar(u_transformed, dg.basis.Nq) for _ = 1:Threads.nthreads()]
    local_flux_viscous_threaded = [
        SVector{ndims(mesh)}(ntuple(_ -> similar(u_transformed, dg.basis.Nq), ndims(mesh))) for _ = 1:Threads.nthreads()
    ]
    local_flux_face_values_threaded =
        [similar(scalar_flux_face_values[:, 1]) for _ = 1:Threads.nthreads()]
    ...
    return (;
        ...
        gradient_lift_matrix,
        projection_face_interpolation_matrix,
        divergence_lift_matrix,
        dxidxhatj,
        J,
        invJ, # geometric terms
        u_face_values,
        gradients_face_values,
...
function transform_variables!(
    ...
    @threaded for i in eachindex(u)
        u_transformed[i] =
            gradient_variable_transformation(equations_parabolic)(u[i], equations_parabolic)
    end
end

# TODO: reuse entropy projection computations for DGMultiFluxDiff{<:Polynomial} (including `GaussSBP` solvers)
function calc_gradient_surface_integral!(
    gradients,
    u,
    scalar_flux_face_values,
    mesh,
    ...
    @threaded for e in eachelement(mesh, dg)
        ...
        for dim in eachdim(mesh)
            for i in eachindex(local_flux_values)
                ...
                local_flux_values[i] =
                    scalar_flux_face_values[i, e] * mesh.md.nxyzJ[dim][i, e]
            end
            apply_to_each_field(
                mul_by_accum!(gradient_lift_matrix),
                view(gradients[dim], :, e),
                local_flux_values,
            )
        end
    end
...
function calc_gradient_volume_integral!(
    gradients,
    u,
    mesh::DGMultiMesh,
    equations::AbstractEquationsParabolic,
    ...
    @threaded for e in eachelement(mesh, dg)
        for i in eachdim(mesh), j in eachdim(mesh)
            ...
            apply_to_each_field(
                mul_by_accum!(strong_differentiation_matrices[j], dxidxhatj),
                view(gradients[i], :, e),
                view(u, :, e),
            )
        end
...
function calc_gradient_volume_integral!(
    ...
    (; strong_differentiation_matrices, dxidxhatj, local_flux_viscous_threaded) =
        cache_parabolic
    ...
    @threaded for e in eachelement(mesh, dg)
        ...
        for i in eachdim(mesh)
            apply_to_each_field(
                mul_by!(strong_differentiation_matrices[i]),
                local_reference_gradients[i],
                view(u, :, e),
            )
        end
        ...
        for i in eachdim(mesh), j in eachdim(mesh)
            for node in eachindex(local_reference_gradients[j])
                gradients[i][node, e] =
                    gradients[i][node, e] +
                    dxidxhatj[i, j][node, e] * local_reference_gradients[j][node]
            end
        end
    end
end
...
function calc_gradient!(
    ...
    calc_gradient_surface_integral!(
        gradients,
        u,
        scalar_flux_face_values,
        mesh,
...
function invert_jacobian_gradient!(
    gradients,
    mesh::DGMultiMesh,
    equations,
    dg::DGMulti,
...
function calc_boundary_flux!(
    ...
    calc_single_boundary_flux!(
        flux,
        u,
        t,
        operator_type,
        first(boundary_conditions),
...
function calc_single_boundary_flux!(
    flux_face_values,
    u_face_values,
    t,
    operator_type,
    ...
    for f in mesh.boundary_faces[boundary_key]
        for i in Base.OneTo(num_pts_per_face)
            ...
            flux_face_values[fid, e] = boundary_condition(
...
function calc_viscous_penalty!(
    ...
    @threaded for face_node_index in each_face_node_global(mesh, dg)
        ...
        scalar_flux_face_values[idM] =
            scalar_flux_face_values[idM] + penalty(uP, uM, equations, parabolic_scheme)
    end
    return nothing
end

function calc_divergence_volume_integral!(
    du,
    u,
    flux_viscous,
    mesh::DGMultiMesh,
    ...
    @threaded for e in eachelement(mesh, dg)
        for i in eachdim(mesh), j in eachdim(mesh)
            ...
            apply_to_each_field(
                mul_by_accum!(weak_differentiation_matrices[j], dxidxhatj),
                view(du, :, e),
                view(flux_viscous[i], :, e),
            )
        end
    end
end
...
function calc_divergence_volume_integral!(
    ...
    (; weak_differentiation_matrices, dxidxhatj, local_flux_viscous_threaded) =
        cache_parabolic
        ...
        for i in eachdim(mesh)
            ...
            for j in eachdim(mesh)
                for node in eachindex(local_viscous_flux)
                    local_viscous_flux[node] =
                        local_viscous_flux[node] +
                        dxidxhatj[j, i][node, e] * flux_viscous[j][node, e]
                end
            end

            # differentiate with respect to reference coordinates
            apply_to_each_field(
                mul_by_accum!(weak_differentiation_matrices[i]),
                view(du, :, e),
                local_viscous_flux,
            )
        end
...
function calc_divergence!(
    ...
    calc_divergence_volume_integral!(
        du,
        u,
        flux_viscous,
        mesh,
    ...
    @threaded for face_node_index in each_face_node_global(mesh, dg, cache, cache_parabolic)
        ...
        for dim in eachdim(mesh)
            ...
            flux_face_value =
                flux_face_value + 0.5 * (fP + fM) * nxyzJ[dim][face_node_index]
        end
    ...
    apply_to_each_field(
        mul_by_accum!(cache_parabolic.divergence_lift_matrix),
        du,
        scalar_flux_face_values,
    )
