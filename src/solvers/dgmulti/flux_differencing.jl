...
@muladd begin
        ...
        for i in row_ids
            ...
            for id in nzrange(A_base, i)
                ...
                if j > i
                    ...
                    normal_direction =
                        0.5 *
                        (getindex.(normal_directions, i) + getindex.(normal_directions, j))
    ...
    @inline function build_lazy_physical_derivative(
        ...
        if orientation == 1
            return LazyMatrixLinearCombo(
                Qrst_skew,
                operator_scaling .* (rxJ[1, element], sxJ[1, element], txJ[1, element]),
            )
        elseif orientation == 2
            return LazyMatrixLinearCombo(
                Qrst_skew,
                operator_scaling .* (ryJ[1, element], syJ[1, element], tyJ[1, element]),
            )
        else # if orientation == 3
            return LazyMatrixLinearCombo(
                Qrst_skew,
                operator_scaling .* (rzJ[1, element], szJ[1, element], tzJ[1, element]),
            )
        end
    end
    ...
    @inline function get_contravariant_vector(
        element,
        orientation,
    ...
    function compute_flux_differencing_SBP_matrices(
    ...
    function create_cache(
        ...
        fluxdiff_local_threaded =
            [zeros(SVector{nvars,uEltype}, rd.Nq) for _ = 1:Threads.nthreads()]

        return (;
            md,
            Qrst_skew,
            dxidxhatj = md.rstxyzJ,
            invJ = inv.(md.J),
            lift_scalings,
            inv_wq = inv.(rd.wq),
            u_values,
    ...
    function create_cache(mesh::DGMultiMesh, equations, dg::DGMultiFluxDiff, RealT, uEltype)
        ...
        nvars = nvariables(equations)
        entropy_var_values =
            allocate_nested_array(uEltype, nvars, (rd.Nq, md.num_elements), dg)

        # storage for all quadrature points (concatenated volume / face quadrature points)
        num_quad_points_total = rd.Nq + rd.Nfq
        entropy_projected_u_values = allocate_nested_array(
            uEltype,
            nvars,
            (num_quad_points_total, md.num_elements),
            dg,
        )
        projected_entropy_var_values = allocate_nested_array(
            uEltype,
            nvars,
            (num_quad_points_total, md.num_elements),
            dg,
        ...
        fluxdiff_local_threaded = [
            zeros(SVector{nvars,uEltype}, num_quad_points_total) for
            _ = 1:Threads.nthreads()
        ]
        rhs_local_threaded = [
            allocate_nested_array(uEltype, nvars, (num_quad_points_total,), dg) for
            _ = 1:Threads.nthreads()
        ]
        ...
        return (;
            ...
            Ph,
            invJ = inv.(J),
            dxidxhatj = interpolated_geometric_terms,
            entropy_var_values,
    ...
    @inline function cons2entropy!(
        entropy_var_values::StructArray,
    ...
    @inline function entropy2cons!(
        entropy_projected_u_values::StructArray,
        projected_entropy_var_values::StructArray,
        equations,
    )
        @threaded for i in eachindex(projected_entropy_var_values)
            entropy_projected_u_values[i] =
                entropy2cons(projected_entropy_var_values[i], equations)
        end
    end
    ...
    @inline has_sparse_operators(::Union{Tri,Tet}, approx_type::AT) where {AT<:SBP} =
        False()
    ...
    @inline function local_flux_differencing!(
        fluxdiff_local,
        u_local,
        element_index,
        has_nonconservative_terms::False,
        volume_flux,
        has_sparse_operators::False,
        mesh,
        equations,
        dg,
        cache,
    ...
    @inline function local_flux_differencing!(
        ...
        u_local,
        element_index,
        has_nonconservative_terms::True,
        volume_flux,
        has_sparse_operators::False,
        mesh,
        equations,
        ...
        for dim in eachdim(mesh)
            ...
            half_Qi_skew =
                build_lazy_physical_derivative(element_index, dim, mesh, dg, cache, 0.5)
    ...
    function calc_sources!(
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            for i in each_quad_node(mesh, dg, cache)
                du[i, e] +=
                    source_terms(u[i, e], SVector(getindex.(md.xyzq, i, e)), t, equations)
            end
        end
    end
