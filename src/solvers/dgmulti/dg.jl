...
@muladd begin
    ...
    mul_by!(A::LinearAlgebra.AdjOrTrans{T,S}) where {T,S<:AbstractSparseMatrix} =
        @inline (out, x) -> mul!(out, A, x)

    #  out <- out + α * A * x
    mul_by_accum!(A, α) = @inline (out, x) -> matmul!(out, A, x, α, One())
    mul_by_accum!(A::AbstractSparseMatrix, α) =
    ...
    @inline apply_to_each_field(f::F, args::Vararg{Any,N}) where {F,N} =
    ...
    @inline ndofs(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    """
    @inline eachelement(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    """
    @inline each_face_node(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    """
    @inline each_quad_node(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    """
    @inline each_dof_global(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    In particular, not the quadrature nodes themselves are returned.
    """
    @inline each_quad_node_global(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    """
    @inline each_face_node_global(mesh::DGMultiMesh, dg::DGMulti, other_args...) =
    ...
    function digest_boundary_conditions(
        boundary_conditions::NamedTuple{Keys,ValueTypes},
        mesh::DGMultiMesh,
        dg::DGMulti,
        cache,
    ) where {Keys,ValueTypes<:NTuple{N,Any}} where {N}
    ...
    function allocate_nested_array(uEltype, nvars, array_dimensions, dg)
        ...
        return StructArray{SVector{nvars,uEltype}}(
            ntuple(_ -> zeros(uEltype, array_dimensions...), nvars),
        )
    end
    ...
    function create_cache(
        ...
        flux_threaded = [
            [allocate_nested_array(uEltype, nvars, (rd.Nq,), dg) for _ = 1:NDIMS] for
            _ = 1:Threads.nthreads()
        ]
    ...
    function compute_coefficients!(
        ...
        @threaded for i in each_quad_node_global(mesh, dg, cache)
            u_values[i] = initial_condition(SVector(getindex.(md.xyzq, i)), t, equations)
        end
    ...
    function calc_volume_integral!(
        ...
        rd = dg.basis
        md = mesh.md
        @unpack weak_differentiation_matrices, dxidxhatj, u_values, local_values_threaded =
            cache
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            ...
            for i in eachdim(mesh)
                ...
                for j in eachdim(mesh)
                    apply_to_each_field(
                        mul_by_accum!(
                            weak_differentiation_matrices[j],
                            dxidxhatj[i, j][1, e],
                        ),
                        view(du, :, e),
                        flux_values,
            ...
            for j in eachdim(mesh)
                ...
                for i in eachdim(mesh)
                    for ii in eachindex(rotated_flux_values)
                        ...
                        rotated_flux_values[ii] =
                            rotated_flux_values[ii] + dxidxhatj_node * flux_i_node
                    end
                end

                # apply weak differentiation matrices to rotated fluxes
                apply_to_each_field(
                    mul_by_accum!(weak_differentiation_matrices[j]),
                    view(du, :, e),
                    rotated_flux_values,
                )
            end
    ...
    function calc_interface_flux!(
        ...
        @threaded for face_node_index in each_face_node_global(mesh, dg, cache)
            ...
            if idM != idP
                ...
                nonconservative_part =
                    flux_nonconservative(uM, uP, normal, normal, equations)
                # The factor 0.5 is necessary for the nonconservative fluxes based on the
                # interpretation of global SBP operators.
                flux_face_values[idM] =
                    (conservative_part + 0.5 * nonconservative_part) * Jf[idM]
            end
        end
    end
    ...
    function calc_single_boundary_flux!(
        ...
        for f in mesh.boundary_faces[boundary_key]
            for i in Base.OneTo(num_pts_per_face)
                ...
                cons_flux_at_face_node = boundary_condition(
                    u_face_values[i, f],
                    face_normal,
                    face_coordinates,
                    t,
                ...
                noncons_flux_at_face_node = nonconservative_flux(
                    u_face_values[i, f],
                    u_face_values[i, f],
                    face_normal,
                    face_normal,
                ...
                flux_face_values[i, f] =
                    (cons_flux_at_face_node + 0.5 * noncons_flux_at_face_node) * Jf[i, f]
    ...
    calc_sources!(du, u, t, source_terms::Nothing, mesh, equations, dg::DGMulti, cache) =
        nothing
    calc_sources!(
        du,
        u,
        ...
        mesh,
        equations,
        dg::DGMultiFluxDiffSBP,
        cache,
    ) = nothing

    # uses quadrature + projection to compute source terms.
    function calc_sources!(du, u, t, source_terms, mesh, equations, dg::DGMulti, cache)
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            ...
            for i in each_quad_node(mesh, dg, cache)
                source_values[i] =
                    source_terms(u_e[i], SVector(getindex.(md.xyzq, i, e)), t, equations)
            end
