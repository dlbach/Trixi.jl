...
struct TensorProductGaussFaceOperator{
    NDIMS,
    OperatorType<:AbstractGaussOperator,
    Tmat,
    Tweights,
    Tfweights,
    Tindices,
...
function TensorProductGaussFaceOperator(
    ...
    for i = 1:nnodes_1d # loop over nodes in one face
    ...
    return TensorProductGaussFaceOperator{2,T_op,Tm,Tw,Tf,Ti}(
        interp_matrix_gauss_to_face_1d,
        inv.(wq1D),
        rd.wf,
...
function TensorProductGaussFaceOperator(
    ...
    face_indices_tensor_product =
        zeros(Int, 2, nnodes_1d, nnodes_1d, ndims(rd.element_type))
    for j = 1:nnodes_1d, i = 1:nnodes_1d # loop over nodes in one face
    ...
    Tf = typeof(rd.wf)
    Ti = typeof(face_indices_tensor_product)
    return TensorProductGaussFaceOperator{3,T_op,Tm,Tw,Tf,Ti}(
        interp_matrix_gauss_to_face_1d,
        inv.(wq1D),
        rd.wf,
        face_indices_tensor_product,
...
@inline function tensor_product_gauss_face_operator!(
    out::AbstractMatrix,
...
@muladd begin
    ...
    @inline function tensor_product_gauss_face_operator!(
        ...
        @turbo for i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d)      # loop over "line" of volume nodes
                out[index_left] =
                    out[index_left] + interp_matrix_gauss_to_face_1d[1, jj] * x[jj, i]
                out[index_right] =
                    out[index_right] + interp_matrix_gauss_to_face_1d[2, jj] * x[jj, i]
            end
        end
        ...
        @turbo for i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d)               # loop over "line" of volume nodes
                out[index_left] =
                    out[index_left] + interp_matrix_gauss_to_face_1d[1, jj] * x[i, jj]
                out[index_right] =
                    out[index_right] + interp_matrix_gauss_to_face_1d[2, jj] * x[i, jj]
            end
        end
    end
    ...
    @inline function tensor_product_gauss_face_operator!(
        ...
        @turbo for j in Base.OneTo(nnodes_1d), i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                out[index_left] =
                    out[index_left] + interp_matrix_gauss_to_face_1d[1, jj] * x[jj, i, j]
                out[index_right] =
                    out[index_right] + interp_matrix_gauss_to_face_1d[2, jj] * x[jj, i, j]
            end
        end
        ...
        @turbo for j in Base.OneTo(nnodes_1d), i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                out[index_left] =
                    out[index_left] + interp_matrix_gauss_to_face_1d[1, jj] * x[i, jj, j]
                out[index_right] =
                    out[index_right] + interp_matrix_gauss_to_face_1d[2, jj] * x[i, jj, j]
            end
        end
        ...
        @turbo for i in Base.OneTo(nnodes_1d), j in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                ...
                out[index_left] =
                    out[index_left] + interp_matrix_gauss_to_face_1d[1, jj] * x[j, i, jj]
                out[index_right] =
                    out[index_right] + interp_matrix_gauss_to_face_1d[2, jj] * x[j, i, jj]
            end
        end
    end
    ...
    @inline function tensor_product_gauss_face_operator!(
        ...
        @turbo for i in Base.OneTo(nnodes_1d) # loop over face nodes
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over a line of volume nodes
                out[jj, i] =
                    out[jj, i] + interp_matrix_gauss_to_face_1d[1, jj] * x[index_left]
                out[jj, i] =
                    out[jj, i] + interp_matrix_gauss_to_face_1d[2, jj] * x[index_right]
            end
        end
        ...
        @turbo for i in Base.OneTo(nnodes_1d)
            ...
            for jj in Base.OneTo(nnodes_1d)
                out[i, jj] =
                    out[i, jj] + interp_matrix_gauss_to_face_1d[1, jj] * x[index_left]
                out[i, jj] =
                    out[i, jj] + interp_matrix_gauss_to_face_1d[2, jj] * x[index_right]
            end
        end
    ...
    @inline function tensor_product_gauss_face_operator!(
        ...
        @turbo for j in Base.OneTo(nnodes_1d), i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                out[jj, i, j] =
                    out[jj, i, j] + interp_matrix_gauss_to_face_1d[1, jj] * x[index_left]
                out[jj, i, j] =
                    out[jj, i, j] + interp_matrix_gauss_to_face_1d[2, jj] * x[index_right]
            end
        end
        ...
        @turbo for j in Base.OneTo(nnodes_1d), i in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                out[i, jj, j] =
                    out[i, jj, j] + interp_matrix_gauss_to_face_1d[1, jj] * x[index_left]
                out[i, jj, j] =
                    out[i, jj, j] + interp_matrix_gauss_to_face_1d[2, jj] * x[index_right]
            end
        end
        ...
        @turbo for i in Base.OneTo(nnodes_1d), j in Base.OneTo(nnodes_1d) # loop over nodes in a face
            ...
            for jj in Base.OneTo(nnodes_1d) # loop over "line" of volume nodes
                ...
                # This is due to way we define face nodes for Hex() types in StartUpDG.jl.
                out[j, i, jj] =
                    out[j, i, jj] + interp_matrix_gauss_to_face_1d[1, jj] * x[index_left]
                out[j, i, jj] =
                    out[j, i, jj] + interp_matrix_gauss_to_face_1d[2, jj] * x[index_right]
            end
        end

        # apply inv(M)
        @turbo for k in Base.OneTo(nnodes_1d),
            j in Base.OneTo(nnodes_1d),
            i in Base.OneTo(nnodes_1d)

            out[i, j, k] =
                out[i, j, k] *
                inv_volume_weights_1d[i] *
                inv_volume_weights_1d[j] *
                inv_volume_weights_1d[k]
        end
    end
    ...
    function create_cache(
        mesh::DGMultiMesh,
        equations,
        dg::DGMultiFluxDiff{<:GaussSBP,<:Union{Quad,Hex}},
        RealT,
        uEltype,
    )
        ...
        interp_matrix_lobatto_to_gauss =
            SimpleKronecker(NDIMS, interp_matrix_lobatto_to_gauss_1D, uEltype)
        interp_matrix_gauss_to_lobatto =
            SimpleKronecker(NDIMS, interp_matrix_gauss_to_lobatto_1D, uEltype)
        ...
        projection_matrix_gauss_to_face =
            TensorProductGaussFaceOperator(Projection{Static.False()}(), dg)
        ...
        return (;
            ...
            create_cache(mesh, equations, dg.volume_integral, dg, RealT, uEltype)...,
        ) # add cache specialized on the volume integral
    end
    ...
    function entropy_projection!(
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            apply_to_each_field(
                mul_by!(interp_matrix_lobatto_to_gauss),
                view(u_values, :, e),
                view(u, :, e),
            )
        end
        ...
        apply_to_each_field(
            mul_by!(interp_matrix_gauss_to_face),
            entropy_var_face_values,
            entropy_var_values,
        )

        # directly copy over volume values (no entropy projection required)
        entropy_projected_volume_values =
            view(entropy_projected_u_values, volume_indices, :)
    ...
    function project_rhs_to_gauss_nodes!(
        du,
        rhs_local,
        element,
        mesh::DGMultiMesh,
        ...
        apply_to_each_field(
            mul_by!(cache.projection_matrix_gauss_to_face),
            rhs_volume_local,
            local_face_flux,
        )
        ...
        for i in eachindex(rhs_volume_local)
            du_local =
                rhs_volume_local[i] + local_volume_flux[i] * cache.inv_gauss_weights[i]
    ...
    function invert_jacobian_and_interpolate!(
        du,
        mesh::DGMultiMesh,
        equations,
        dg::DGMultiFluxDiff{<:GaussSBP},
        ...
        @threaded for e in eachelement(mesh, dg, cache)
            ...
            apply_to_each_field(
                mul_by!(interp_matrix_gauss_to_lobatto),
                view(du, :, e),
                rhs_volume_local,
            )
        end
    ...
    function rhs!(
        ...
        @trixi_timeit timer() "entropy_projection!" entropy_projection!(
            cache,
            u,
            mesh,
            equations,
        ...
        @trixi_timeit timer() "Jacobian" invert_jacobian_and_interpolate!(
            du,
            mesh,
            equations,
            dg,
