...
function create_cache(
    ...
    sparse_hybridized_SBP_operators =
        map((Q, B) -> 0.5 * [Q-Q' E'*B; -B*E zeros(size(B))], Qrst, Brst)
    ...
    sparsity_pattern =
        sum(map(A -> abs.(A)', sparse_hybridized_SBP_operators)) .> 100 * eps()
...
function create_cache(
    ...
    A = Vector{real(basis)}
    indicator_threaded = [A(undef, nnodes(basis)) for _ = 1:Threads.nthreads()]
    modal_threaded = [A(undef, nnodes(basis)) for _ = 1:Threads.nthreads()]
...
function (indicator_hg::IndicatorHennemannGassner)(
    u,
    mesh::DGMultiMesh,
    equations,
    dg::DGMulti{NDIMS},
    ...
    (; alpha, alpha_tmp, indicator_threaded, modal_threaded, inverse_vandermonde) =
        indicator_hg.cache
...
function pure_and_blended_element_ids!(
    element_ids_dg,
    element_ids_dgfv,
    alpha,
    mesh::DGMultiMesh,
...
function calc_volume_integral!(
    ...
    alpha =
        @trixi_timeit timer() "blending factors" indicator(u, mesh, equations, dg, cache)
    ...
    @trixi_timeit timer() "blended DG-FV" @threaded for idx_element in
        ...
        low_order_flux_differencing_kernel!(
            du,
            u,
            element,
            mesh,
...
function get_contravariant_matrix(element, mesh::DGMultiMesh{2,<:Affine}, cache)
    (; dxidxhatj) = cache
    return SMatrix{2,2}(
        dxidxhatj[1, 1][1, element],
        dxidxhatj[2, 1][1, element],
        dxidxhatj[1, 2][1, element],
        dxidxhatj[2, 2][1, element],
    )
end
...
function get_contravariant_matrix(element, mesh::DGMultiMesh{3,<:Affine}, cache)
    ...
    return SMatrix{3,3}(
        ...
        dxidxhatj[1, 3][1, element],
        dxidxhatj[2, 3][1, element],
...
function get_contravariant_matrix(i, element, mesh::DGMultiMesh{2}, cache)
    (; dxidxhatj) = cache
    return SMatrix{2,2}(
        dxidxhatj[1, 1][i, element],
        dxidxhatj[2, 1][i, element],
        dxidxhatj[1, 2][i, element],
        dxidxhatj[2, 2][i, element],
    )
end
...
function get_contravariant_matrix(i, element, mesh::DGMultiMesh{3}, cache)
    ...
    return SMatrix{3,3}(
        ...
        dxidxhatj[1, 3][i, element],
        dxidxhatj[2, 3][i, element],
...
get_avg_contravariant_matrix(i, j, element, mesh::DGMultiMesh, cache) =
    0.5 * (
        get_contravariant_matrix(i, element, mesh, cache) +
...
function low_order_flux_differencing_kernel!(
    du,
    u,
    element,
    mesh::DGMultiMesh,
