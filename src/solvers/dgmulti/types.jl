...
const DGMulti{NDIMS,ElemType,ApproxType,SurfaceIntegral,VolumeIntegral} = DG{
    <:RefElemData{NDIMS,ElemType,ApproxType},
    Mortar,
    SurfaceIntegral,
    VolumeIntegral,
} where {Mortar}
...
const DGMultiWeakForm{ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
...
const DGMultiFluxDiff{ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
    <:SurfaceIntegralWeakForm,
    <:Union{VolumeIntegralFluxDifferencing,VolumeIntegralShockCapturingHG},
} where {NDIMS}

const DGMultiFluxDiffSBP{ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
    <:SurfaceIntegralWeakForm,
    <:Union{VolumeIntegralFluxDifferencing,VolumeIntegralShockCapturingHG},
} where {NDIMS,ApproxType<:Union{SBP,AbstractDerivativeOperator}}

const DGMultiSBP{ApproxType,ElemType} = DGMulti{
    NDIMS,
    ElemType,
    ApproxType,
    SurfaceIntegral,
    ...
    NDIMS,
    ElemType,
    ApproxType<:Union{SBP,AbstractDerivativeOperator},
    SurfaceIntegral,
    VolumeIntegral,
}
...
@muladd begin
    ...
    Base.summary(io::IO, dg::DG) where {DG<:DGMulti} =
    ...
    DGMulti(basis::RefElemData; volume_integral, surface_integral) =
        DG(basis, nothing, surface_integral, volume_integral) #= mortar =#
    ...
    function DGMultiMesh(
        ...
        return DGMultiMesh{
            NDIMS,
            typeof(geometric_term_type),
            typeof(md),
            typeof(boundary_faces),
        }(
            md,
            boundary_faces,
        )
    ...
    GeometricTermsType(mesh_type, dg::DGMulti) =
    ...
    function DGMultiMesh(
        ...
        md = StartUpDG.make_periodic(md, periodicity)
        boundary_faces =
            StartUpDG.tag_boundary_faces(triangulateIO, dg.basis, md, boundary_dict)
    ...
    function DGMultiMesh(
        ...
        vertex_coordinates, EToV =
            StartUpDG.uniform_mesh(dg.basis.element_type, cells_per_dimension...)
        domain_lengths = coordinates_max .- coordinates_min
        for i = 1:NDIMS
            @. vertex_coordinates[i] =
                0.5 * (vertex_coordinates[i] + 1) * domain_lengths[i] + coordinates_min[i]
        end
    ...
    function DGMultiMesh(
        ...
        md = MeshData(vertex_coordinates, EToV, dg.basis)
        md =
            NDIMS == 1 ? StartUpDG.make_periodic(md, periodicity...) :
    ...
    function DGMultiMesh(
        ...
        md = StartUpDG.make_periodic(md, periodicity)
        boundary_faces = Dict(
            Pair.(keys(md.mesh_type.boundary_faces), values(md.mesh_type.boundary_faces)),
        )
    ...
    struct LazyMatrixLinearCombo{Tcoeffs,N,Tv,TA<:AbstractMatrix{Tv}} <: AbstractMatrix{Tv}
        ...
        function LazyMatrixLinearCombo(matrices, coeffs)
            ...
            new{
                typeof(first(coeffs)),
                length(matrices),
                eltype(first(matrices)),
                typeof(first(matrices)),
            }(
                matrices,
                coeffs,
            )
    ...
    function SimpleKronecker(NDIMS, A, eltype_A = eltype(A))
        ...
        tmp_storage = [
            zeros(eltype_A, ntuple(_ -> size(A, 2), NDIMS)...) for _ = 1:Threads.nthreads()
        ]
    ...
    function LinearAlgebra.mul!(b_in, A_kronecker::SimpleKronecker{2}, x_in)
        ...
        @turbo thread = true for j = 1:n, i = 1:n
            tmp = zero(eltype(x))
            for ii = 1:n
    ...
    function LinearAlgebra.mul!(b_in, A_kronecker::SimpleKronecker{3}, x_in)
        ...
        @turbo thread = true for k = 1:n, j = 1:n, i = 1:n
            tmp = zero(eltype(x))
            for ii = 1:n
...
@deprecate DGMultiMesh(dg::DGMulti{NDIMS}; cells_per_dimension, kwargs...) where {NDIMS} DGMultiMesh(
    dg,
    cells_per_dimension;
    kwargs...,
)
...
@deprecate DGMultiMesh(
    vertex_coordinates,
    EToV,
    dg::DGMulti{NDIMS};
    kwargs...,
) where {NDIMS} DGMultiMesh(dg, vertex_coordinates, EToV; kwargs...)
@deprecate DGMultiMesh(
    triangulateIO,
    dg::DGMulti{2,Tri},
    boundary_dict::Dict{Symbol,Int};
