...
@muladd begin
    ...
    function UnstructuredElementContainer2D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
        ...
        return UnstructuredElementContainer2D{RealT,uEltype}(
            node_coordinates,
            jacobian_matrix,
            inverse_jacobian,
    ...
    @inline nelements(elements::UnstructuredElementContainer2D) =
    ...
    """
    @inline eachelement(elements::UnstructuredElementContainer2D) =
        Base.OneTo(nelements(elements))

    @inline nvariables(elements::UnstructuredElementContainer2D) =
        size(elements.surface_flux_values, 1)
    @inline nnodes(elements::UnstructuredElementContainer2D) =
        size(elements.surface_flux_values, 2)

    Base.real(elements::UnstructuredElementContainer2D) = eltype(elements.node_coordinates)
    Base.eltype(elements::UnstructuredElementContainer2D) =
    ...
    function init_element!(elements, element, nodes, corners_or_surface_curves)
        ...
        calc_contravariant_vectors!(
            elements.contravariant_vectors,
            element,
            elements.jacobian_matrix,
        )

        calc_normal_directions!(
            elements.normal_directions,
            element,
            nodes,
            corners_or_surface_curves,
        )
    ...
    function UnstructuredInterfaceContainer2D{uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {uEltype<:Real}
    ...
    @inline ninterfaces(interfaces::UnstructuredInterfaceContainer2D) =
        ...
        for j = 1:n_surfaces
            if edge_information[4, j] > 0
                ...
                if boundary_names[primary_side, primary_element] === :Bottom
                    secondary_element =
                        primary_element + (n_elements - convert(Int, sqrt(n_elements)))
                    secondary_side = 3
                ...
                elseif boundary_names[primary_side, primary_element] === :Left
                    secondary_element =
                        primary_element + (convert(Int, sqrt(n_elements)) - 1)
                    secondary_side = 2
    ...
    # information in the unstructured mesh constructor
    function UnstructuredBoundaryContainer2D{RealT,uEltype}(
        capacity::Integer,
        n_variables,
        n_nodes,
    ) where {RealT<:Real,uEltype<:Real}
    ...
    @inline nboundaries(boundaries::UnstructuredBoundaryContainer2D) =
        length(boundaries.name)


    function init_boundaries(
        mesh::UnstructuredMesh2D,
        elements::UnstructuredElementContainer2D,
    )
    ...
    function init_boundaries!(
        ...
        for j = 1:n_surfaces
            if edge_information[4, j] == 0
                ...
                if primary_side == 1
                    boundaries.node_coordinates[:, :, bndy_count] .=
                        enc[:, :, 1, primary_element]
                elseif primary_side == 2
                    boundaries.node_coordinates[:, :, bndy_count] .=
                        enc[:, end, :, primary_element]
