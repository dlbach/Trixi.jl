...
@muladd begin
    ...
    # the RHS etc.
    function create_cache(mesh::UnstructuredMesh2D, equations, dg::DG, RealT, uEltype)
        ...
        if !isapprox(max_discrete_metric_identities(dg, cache), 0, atol = 1e-12)
            error(
                "metric terms fail free-stream preservation check with maximum error $(max_discrete_metric_identities(dg, cache))",
            )
        end
            ...
            elseif secondary_side == 2
                for i in eachnode(dg), v in eachvariable(equations)
                    interfaces.u[2, v, i, interface] =
                        u[v, nnodes(dg), i, secondary_element]
                end
            elseif secondary_side == 3
                for i in eachnode(dg), v in eachvariable(equations)
                    interfaces.u[2, v, i, interface] =
                        u[v, i, nnodes(dg), secondary_element]
                end
    ...
    function calc_interface_flux!(
        ...
        @unpack u, start_index, index_increment, element_ids, element_side_ids =
            cache.interfaces
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for primary_index in eachnode(dg)
                ...
                u_ll = get_one_sided_surface_node_vars(
                    u,
                    equations,
                    dg,
                    1,
                ...
                u_rr = get_one_sided_surface_node_vars(
                    u,
                    equations,
                    dg,
                    2,
                ...
                outward_direction = get_surface_normal(
                    normal_directions,
                    primary_index,
                    primary_side,
                    primary_element,
    ...
    function calc_interface_flux!(
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for primary_index in eachnode(dg)
                ...
                noncons_primary = nonconservative_flux(
                    u_ll,
                    u_rr,
                    outward_direction,
                    outward_direction,
                ...
                noncons_secondary = nonconservative_flux(
                    u_rr,
                    u_ll,
                    outward_direction,
                    outward_direction,
    ...
    function calc_boundary_flux_by_type!(
        cache,
        t,
        BCs::NTuple{N,Any},
    ...
    @inline function calc_boundary_flux!(
        ...
        # pull the outward pointing (normal) directional vector
        outward_direction =
            get_surface_normal(normal_directions, node_index, side_index, element_index)
    ...
    @inline function calc_boundary_flux!(
        ...
        noncons_flux = nonconservative_flux(
            u_inner,
            u_inner,
            outward_direction,
            outward_direction,
        ...
        for v in eachvariable(equations)
            ...
            # the interpretation of global SBP operators coupled discontinuously via
            # central fluxes/SATs
            surface_flux_values[v, node_index, side_index, element_index] =
    ...
    function calc_surface_integral!(
        ...
        @threaded for element in eachelement(dg, cache)
            for l in eachnode(dg), v in eachvariable(equations)
                ...
                du[v, 1, l, element] +=
                    (surface_flux_values[v, l, 4, element] * boundary_interpolation[1, 1])
                ...
                du[v, l, 1, element] +=
                    (surface_flux_values[v, l, 1, element] * boundary_interpolation[1, 1])
    ...
    function max_discrete_metric_identities(dg::DGSEM, cache)
        ...
        for i = 1:ndims_, element in eachelement(dg, cache)
            ...
            @views mul!(
                metric_id_dy,
                contravariant_vectors[i, 2, :, :, element],
                derivative_matrix',
            )
