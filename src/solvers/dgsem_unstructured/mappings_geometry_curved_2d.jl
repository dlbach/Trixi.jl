...
@muladd begin
    ...
    function transfinite_quad_map(xi, eta, surface_curves::AbstractVector{<:CurvedSurface})
        ...
        x = (
            ...
            0.25 * (
                (1.0 - xi) * ((1.0 - eta) * x_corner1 + (1.0 + eta) * x_corner4) +
        ...
        y = (
            ...
            0.25 * (
                (1.0 - xi) * ((1.0 - eta) * y_corner1 + (1.0 + eta) * y_corner4) +
    ...
    function transfinite_quad_map_metrics(
        xi,
        eta,
        ...
        X_xi = (
            0.5 * (x2 - x4 + (1.0 - eta) * x1_prime + (1.0 + eta) * x3_prime) -
            0.25 * (
                (1.0 - eta) * (x_corner2 - x_corner1) +
                (1.0 + eta) * (x_corner3 - x_corner4)
            )
        )

        X_eta = (
            0.5 * ((1.0 - xi) * x4_prime + (1.0 + xi) * x2_prime + x3 - x1) -
            0.25 * (
                (1.0 - xi) * (x_corner4 - x_corner1) + (1.0 + xi) * (x_corner3 - x_corner2)
            )
        )

        Y_xi = (
            0.5 * (y2 - y4 + (1.0 - eta) * y1_prime + (1.0 + eta) * y3_prime) -
            0.25 * (
                (1.0 - eta) * (y_corner2 - y_corner1) +
                (1.0 + eta) * (y_corner3 - y_corner4)
            )
        )

        Y_eta = (
            0.5 * ((1.0 - xi) * y4_prime + (1.0 + xi) * y2_prime + y3 - y1) -
            0.25 * (
                (1.0 - xi) * (y_corner4 - y_corner1) + (1.0 + xi) * (y_corner3 - y_corner2)
            )
        )
    ...
    function calc_node_coordinates!(
        ...
        for j in eachindex(nodes), i in eachindex(nodes)
            node_coordinates[:, i, j, element] .=
                transfinite_quad_map(nodes[i], nodes[j], surface_curves)
        end
    ...
    function calc_metric_terms!(
        ...
        for j in eachindex(nodes), i in eachindex(nodes)
            ...
            ) = transfinite_quad_map_metrics(nodes[i], nodes[j], surface_curves)
        end
    ...
    function calc_normal_directions!(
        ...
        for j in eachindex(nodes)
            ...
            X_xi, X_eta, Y_xi, Y_eta =
                transfinite_quad_map_metrics(1.0, nodes[j], surface_curves)
        ...
        for i in eachindex(nodes)
            # side 1
            X_xi, X_eta, Y_xi, Y_eta =
                transfinite_quad_map_metrics(nodes[i], -1.0, surface_curves)
            Jtemp = X_xi * Y_eta - X_eta * Y_xi
