...
@muladd begin
    ...
    function UnstructuredSortedBoundaryTypes(boundary_conditions::Dict, cache)
        ...
        container = UnstructuredSortedBoundaryTypes{
            n_boundary_types,
    ...
    function initialize!(
        boundary_types_container::UnstructuredSortedBoundaryTypes{N},
        cache,
    ) where {N}
        ...
        if mpi_isparallel()
            ...
            if mpi_isroot()
                ...
                MPI.Gatherv!(
                    send_buffer,
                    MPI.VBuffer(recv_buffer, recv_buffer_length),
                    mpi_root(),
                    mpi_comm(),
                )
                all_names =
                    unique(Symbol.(split(String(recv_buffer), "\0"; keepempty = false)))
