...
const TrixiODESolution = Union{
    ODESolution{
        ...
        P<:ODEProblem{
            ...
            P_,
            F_,
        } where {uType_,tType_,isinplace,P_<:AbstractSemidiscretization,F_},
    },
    TimeIntegratorSolution,
}
...
@muladd begin
    ...
    struct PlotData2DTriangulated{
        DataType,
        NodeType,
        FaceNodeType,
        FaceDataType,
        VariableNames,
        PlottingTriangulation,
    } <: AbstractPlotData{2}
    ...
    ```
    """
    PlotData2D(u_ode, semi; kwargs...) = PlotData2D(
        wrap_array_native(u_ode, semi),
        mesh_equations_solver_cache(semi)...;
        kwargs...,
    )

    # Redirect `PlotDataTriangulated2D` constructor.
    PlotData2DTriangulated(u_ode, semi; kwargs...) = PlotData2DTriangulated(
        wrap_array_native(u_ode, semi),
        mesh_equations_solver_cache(semi)...;
        kwargs...,
    )
    ...
    PlotData2D(u, mesh::TreeMesh, equations, solver, cache; kwargs...) =
    ...
    function PlotData2DCartesian(
        ...
        unstructured_data =
            get_unstructured_data(u, solution_variables_, mesh, equations, solver, cache)
        x, y, data, mesh_vertices_x, mesh_vertices_y = get_data_2d(
            center_level_0,
            length_level_0,
            leaf_cell_ids,
            coordinates,
    ...
    function PlotData2D(
        ...
        u_structarray =
            StructArray{eltype(u)}(ntuple(_ -> zeros(eltype(first(u)), size(u)), nvars))
    ...
    function PlotData2D(
        ...
        x_face, y_face, face_data =
            mesh_plotting_wireframe(u, mesh, equations, dg, cache; nvisnodes = nvisnodes)

        return PlotData2DTriangulated(
            x_plot,
            y_plot,
            u_plot,
            t,
    ...
    function PlotData2DTriangulated(
        u,
        mesh,
        equations,
        dg::DGSEM,
        ...
        u_extracted = StructArray{SVector{nvars,uEltype}}(
            ntuple(_ -> similar(x, (n_nodes_2d, n_elements)), nvars),
        )
        ...
        uplot = StructArray{SVector{nvars,uEltype}}(
            map(x -> plotting_interp_matrix * x, StructArrays.components(u_extracted)),
        )

        xfp, yfp, ufp = mesh_plotting_wireframe(
            u_extracted,
            mesh,
            equations,
            dg,
    ...
    function ScalarPlotData2D(
        ...
        x_face, y_face, face_data = mesh_plotting_wireframe(
            ScalarData(u),
            mesh,
            equations,
            dg,
    ...
    !!! warning "Experimental implementation"
        This is an experimental feature and may change in future releases.
    """
    PlotData1D(u_ode, semi; kwargs...) = PlotData1D(
        wrap_array_native(u_ode, semi),
        mesh_equations_solver_cache(semi)...;
        kwargs...,
    )
    ...
    function PlotData1D(
        ...
        if ndims(mesh) == 1
            x, data, mesh_vertices_x =
                get_data_1d(original_nodes, unstructured_data, nvisnodes)
            orientation_x = 1
            ...
            if n_nodes == 1
                ...
                if n_visnodes != 2
                    throw(
                        ArgumentError(
                            "This number of visualization nodes is currently not supported for finite volume approximations.",
                        ),
                    )
                end
                left_boundary = mesh.tree.center_level_0[1] - mesh.tree.length_level_0 / 2
                dx_2 = zero(left_boundary)
                for i = 1:div(length(x), 2)
        ...
        elseif ndims(mesh) == 2
            if curve !== nothing
                x, data, mesh_vertices_x = unstructured_2d_to_1d_curve(
                    original_nodes,
                    unstructured_data,
                    nvisnodes,
                    curve,
                ...
                x, data, mesh_vertices_x = unstructured_2d_to_1d(
                    original_nodes,
                    unstructured_data,
                    nvisnodes,
                    slice,
            ...
            if curve !== nothing
                x, data, mesh_vertices_x = unstructured_3d_to_1d_curve(
                    original_nodes,
                    unstructured_data,
                    nvisnodes,
                    curve,
                ...
                x, data, mesh_vertices_x = unstructured_3d_to_1d(
                    original_nodes,
                    unstructured_data,
                    nvisnodes,
                    slice,
    ...
    function PlotData1D(
        ...
        elseif ndims(mesh) == 2
            ...
            x, data, mesh_vertices_x =
                unstructured_2d_to_1d_curve(pd, curve, slice, point, nvisnodes)
        else # ndims(mesh) == 3
    ...
    function PlotData1D(
        ...
        if u isa StructArray
            ...
            data = map(
                x -> vcat(dg.basis.Vp * x, fill(NaN, 1, size(u, 2))),
            ...
            x_plot = vec(x)
            data_plot =
                permutedims(reinterpret(reshape, eltype(eltype(data)), vec(data)), (2, 1))
        end
    ...
    !!! warning "Experimental implementation"
        This is an experimental feature and may change in future releases.
    """
    PlotData1D(sol::TrixiODESolution; kwargs...) =
