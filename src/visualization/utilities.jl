...
@muladd begin
    ...
    function reference_plotting_triangulation(
        reference_plotting_coordinates,
        tol = 50 * eps(),
    )
    ...
    function global_plotting_triangulation_triplot(xyz_plot, u_plot, t)
        ...
        for e = 1:num_elements
            for i = 1:num_reference_plotting_triangles
                tp[:, i+(e-1)*num_reference_plotting_triangles] .=
                    @views t[i, :] .+ (e - 1) * num_plotting_points
    ...
    # dispatch on semi
    mesh_plotting_wireframe(u, semi) =
    ...
    function mesh_plotting_wireframe(
        u::StructArray,
        mesh,
        equations,
        dg::DGMulti,
        ...
        vandermonde_matrix_1D =
            StartUpDG.vandermonde(Line(), N, StartUpDG.nodes(Line(), num_face_points - 1))
        ...
        xf, yf =
            map(x -> reshape(view(x, Fmask, :), num_face_points, num_faces_total), md.xyz)
        uf = similar(u, size(xf))
        apply_to_each_field(
            (out, x) -> out .= reshape(view(x, Fmask, :), num_face_points, num_faces_total),
            uf,
            u,
        )
    ...
    function mesh_plotting_wireframe(
        ...
        x = reshape(
            view(cache.elements.node_coordinates, 1, :, :, :),
            n_nodes_2d,
            n_elements,
        )
        y = reshape(
            view(cache.elements.node_coordinates, 2, :, :, :),
            n_nodes_2d,
            n_elements,
        )
        ...
        plotting_interp_matrix1D =
            face_plotting_interpolation_matrix(dg; nvisnodes = nvisnodes)
        ...
        reshape_and_interpolate(x) =
            plotting_interp_matrix1D *
            face_first_reshape(x, nnodes(dg), n_nodes_2d, n_elements)
        xfp, yfp = map(reshape_and_interpolate, (x, y))
        ufp = StructArray{SVector{nvars,uEltype}}(
            map(reshape_and_interpolate, StructArrays.components(u)),
        )
    ...
    function mesh_plotting_wireframe(
        ...
        num_faces_total = num_faces(rd.element_type) * md.num_elements
        xf, yf, uf = map(
            x -> reshape(view(x, Fmask, :), num_face_points, num_faces_total),
            (md.xyz..., u.data),
        )
    ...
    adapt_to_mesh_level!(sol::TrixiODESolution, level) =
    ...
    function get_data_2d(
        ...
        if max_level > max_supported_level
            error(
                "Maximum refinement level $max_level is higher than " *
        ...
        nvisnodes_per_level =
            [2^(max_level - level) * nvisnodes_at_max_level for level = 0:max_level]
        ...
        if ndims == 3
            (unstructured_data, coordinates, levels, center_level_0) =
                unstructured_3d_to_2d(
                    unstructured_data,
                    coordinates,
                    levels,
                    length_level_0,
        ...
        xs =
            collect(range(-1, 1, length = resolution + 1)) .* length_level_0 / 2 .+
            center_level_0[1]
        ys =
            collect(range(-1, 1, length = resolution + 1)) .* length_level_0 / 2 .+
            center_level_0[2]
        ...
        if grid_lines
            mesh_vertices_x, mesh_vertices_y =
                calc_vertices(coordinates, levels, length_level_0)
        else
    ...
    function get_data_1d(original_nodes, unstructured_data, nvisnodes)
        ...
        interpolated_nodes =
            Array{eltype(original_nodes),2}(undef, max_nvisnodes, n_elements)
        interpolated_data =
            Array{eltype(unstructured_data),3}(undef, max_nvisnodes, n_elements, n_vars)

        for j = 1:n_elements
            # Interpolate on an equidistant grid.
            interpolated_nodes[:, j] .= range(
                original_nodes[1, 1, j],
        ...
        for v = 1:n_vars
            ...
            for element = 1:n_elements
                multiply_scalar_dimensionwise!(
                    @view(interpolated_data[:, element, v]),
                    vandermonde,
        ...
        return vec(interpolated_nodes),
        reshape(interpolated_data, :, n_vars),
    ...
    function get_unstructured_data(u, solution_variables, mesh, equations, solver, cache)
        ...
        if solution_variables === cons2cons
            ...
            n_vars =
                length(solution_variables(get_node_vars(u, equations, solver), equations))
            ...
            for idx in axes(reshaped_u, 2)
                reshaped_r[:, idx] = solution_variables(
                    get_node_vars(reshaped_u, equations, solver, idx),
                    equations,
                )
            end
        end

        unstructured_data = Array{eltype(raw_data)}(
            undef,
            ntuple((d) -> nnodes(solver), ndims(equations))...,
            nelements(solver, cache),
            n_vars,
        )
        for variable = 1:n_vars
    ...
    function cell2node(cell_centered_data)
        ...
        node_centered_data = [
            Matrix{Float64}(undef, resolution_out, resolution_out) for
            _ = 1:length(cell_centered_data)
        ]
        ...
        for (cell_data, node_data) in zip(cell_centered_data, node_centered_data)
            ...
            for j = 1:resolution_out
                for i = 1:resolution_out
                    node_data[i, j] =
                        (tmp[i, j] + tmp[i+1, j] + tmp[i, j+1] + tmp[i+1, j+1]) / 4
                end
            end
        end
    ...
    # Note: This is a low-level function that is not considered as part of Trixi.jl's interface and may
    #       thus be changed in future releases.
    function unstructured_3d_to_2d(
        unstructured_data,
        coordinates,
        levels,
        length_level_0,
        ...
        if point[slice_dimension] < lower_limit || point[slice_dimension] > upper_limit
            error(
                string(
                    "Slice plane is outside of domain.",
        ...
        for element_id = 1:n_elements
            ...
            if !(
                (
                    min_coordinate[slice_dimension] <= point[slice_dimension] &&
            ...
            if haskey(vandermonde_to_2d, normalized_intercept)
                ...
                vandermonde =
                    polynomial_interpolation_matrix(nodes_in, [normalized_intercept])
    ...
    function unstructured_2d_to_1d(
        original_nodes,
        unstructured_data,
        nvisnodes,
        slice,
        ...
        if point[slice_dimension] < lower_limit || point[slice_dimension] > upper_limit
            error(
                string(
                    "Slice axis is outside of domain. ",
                    " point[$slice_dimension]=$(point[slice_dimension]) must be between $lower_limit and $upper_limit",
                ),
            )
        end
        ...
        for element_id = 1:n_elements
            ...
            for v = 1:n_variables
                for node = 1:n_nodes_in
                    new_unstructured_data[node, new_id, v] =
                        (vandermonde*unstructured_data[node, :, element_id, v])[1]
                end
            end
        ...
        return get_data_1d(
            reshape(new_nodes[:, 1:new_id], 1, n_nodes_in, new_id),
            new_unstructured_data[:, 1:new_id, :],
            nvisnodes,
        )
    end
    ...
    function calc_arc_length(coordinates)
        ...
        for i = 1:n_points-1
            arc_length[i+1] =
                arc_length[i] + sqrt(sum((coordinates[:, i] - coordinates[:, i+1]) .^ 2))
        end
        return arc_length
    end

    # Convert 2d unstructured data to 1d data at given curve.
    function unstructured_2d_to_1d_curve(
        original_nodes,
        unstructured_data,
        nvisnodes,
        curve,
        ...
        for element = 1:n_points_curve
            @assert (prod(
                vcat(curve[:, n_points_curve] .>= min, curve[:, n_points_curve] .<= max),
            )) "Some coordinates from `curve` are outside of the domain.."
        end
        ...
        for element = 1:n_points_curve
            ...
            normalized_coordinates =
                (curve[:, element] - min_coordinate) / element_length[1] * 2 .- 1

            # Interpolate to a single point in each element.
            vandermonde_x =
                polynomial_interpolation_matrix(nodes_in, normalized_coordinates[1])
            vandermonde_y =
                polynomial_interpolation_matrix(nodes_in, normalized_coordinates[2])
            for v = 1:n_variables
                for i = 1:n_nodes
                    temp_data[i, element, v] =
                        (vandermonde_y*unstructured_data[i, :, element_ids[element], v])[1]
                end
    ...
    function unstructured_2d_to_1d_curve(pd, input_curve, slice, point, nvisnodes)
        ...
        for point = 1:n_points_curve
            ...
            for v = 1:n_variables
                ...
                data_on_curve[point, v] = triangle_interpolation(
                    x_coordinates_triangle,
    ...
    function unstructured_3d_to_1d_curve(
        original_nodes,
        unstructured_data,
        nvisnodes,
        curve,
        ...
        for element = 1:n_points_curve
            ...
            max_coordinate =
                original_nodes[:, n_nodes, n_nodes, n_nodes, element_ids[element]]
            element_length = max_coordinate - min_coordinate
            ...
            vandermonde_y =
                polynomial_interpolation_matrix(nodes_in, normalized_coordinates[2])
            vandermonde_z =
                polynomial_interpolation_matrix(nodes_in, normalized_coordinates[3])
            for v = 1:n_variables
                for i = 1:n_nodes
                    for ii = 1:n_nodes
                        temp_data[i, ii, element, v] = (vandermonde_z*unstructured_data[
                            i,
                            ii,
                            :,
                            element_ids[element],
                    ...
                    temp_data[i, n_nodes+1, element, v] =
                        (vandermonde_y*temp_data[i, 1:n_nodes, element, v])[1]
                end
                data_on_curve[element, v] =
                    (vandermonde_x*temp_data[:, n_nodes+1, element, v])[1]
            end
        end
    ...
    function tetrahedron_interpolation(
        x_coordinates_in,
        y_coordinates_in,
        z_coordinates_in,
        values_in,
        ...
        c = A \ values_in
        return c[1] * coordinate_out[1] +
               c[2] * coordinate_out[2] +
               c[3] * coordinate_out[3] +
               c[4]
    end
    ...
    function distances_from_single_point(nodes, point)
        ...
        for element = 1:n_elements
            for x = 1:n_nodes
                for y = 1:n_nodes
                    for z = 1:n_nodes
                        distances[x, y, z, element] =
                            norm(shifted_data[:, x, y, z, element])
                    end
                end
            end
        end
    ...
    function get_value_at_point(point, nodes, data)
        ...
        @views coordinates_tetrahedron[:, 1] =
            nodes[:, index[1], index[2], index[3], index[4]]
        ...
        for i = 1:4
            ...
            while true
                ...
                @views coordinates_tetrahedron[:, i] =
                    nodes[:, index[1], index[2], index[3], closest_element]
                @views value_tetrahedron[:, i] =
                    data[:, index[1], index[2], index[3], closest_element]
        ...
        for v = 1:n_variables
            value_at_point[v] = tetrahedron_interpolation(
                coordinates_tetrahedron[1, :],
    ...
    function unstructured_3d_to_1d(
        original_nodes,
        unstructured_data,
        nvisnodes,
        slice,
        ...
        for element_id = 1:n_elements
            ...
            max_coordinate =
                original_nodes[:, n_nodes_in, n_nodes_in, n_nodes_in, element_id]
            element_length = max_coordinate - min_coordinate

            # Test if the element is on the slice axis. If not just continue with the next element.
            if !(
                (
                    prod(min_coordinate[other_dimensions] .<= point[other_dimensions]) &&
            ...
            normalized_intercept =
                ...
                element_length[1] * 2 .- 1
            vandermonde_i =
                polynomial_interpolation_matrix(nodes_in, normalized_intercept[1])
            vandermonde_ii =
                polynomial_interpolation_matrix(nodes_in, normalized_intercept[2])

            # Interpolate to each node of new 1D element.
            for v = 1:n_variables
                for i = 1:n_nodes_in
                    for ii = 1:n_nodes_in
                        temp_unstructured_data[i, ii, new_id, v] =
                            (vandermonde_ii*unstructured_data[ii, :, i, element_id, v])[1]
                    end
                    new_unstructured_data[i, new_id, v] =
                        (vandermonde_i*temp_unstructured_data[i, :, new_id, v])[1]
                end
            end
    ...
    function unstructured2structured(
        ...
        lower_left_index =
            element2index(normalized_coordinates, levels, resolution, nvisnodes_per_level)

        # Create output data structure
        structured = [Matrix{Float64}(undef, resolution, resolution) for _ = 1:n_variables]
        ...
        for v = 1:n_variables
            ...
            for element_id = 1:n_elements
                ...
                structured[v][first[1]:last[1], first[2]:last[2]] .= (reshape(
                    multiply_dimensionwise(vandermonde, reshaped_data[:, :, :, element_id]),
                    n_nodes_out,
                    n_nodes_out,
                ))
            end
    ...
    function calc_vertices(node_coordinates, mesh)
        ...
        for cell_y in axes(mesh, 2)
            ...
            for cell_x in axes(mesh, 1)
                for node = 1:(n_nodes-1)
                    x[i, line_index] =
                        node_coordinates[1, node, end, linear_indices[cell_x, cell_y]]
                    y[i, line_index] =
                        node_coordinates[2, node, end, linear_indices[cell_x, cell_y]]
        ...
        for cell_x in axes(mesh, 1)
            ...
            for cell_y in axes(mesh, 2)
                for node = 1:(n_nodes-1)
                    x[i, line_index] =
                        node_coordinates[1, end, node, linear_indices[cell_x, cell_y]]
                    y[i, line_index] =
                        node_coordinates[2, end, node, linear_indices[cell_x, cell_y]]
    ...
    function face_plotting_interpolation_matrix(
        dg::DGSEM;
    ...
    function plotting_interpolation_matrix(
        dg::DGSEM;
        nvisnodes = 2 * length(dg.basis.nodes),
    )
    ...
    function get_ids_by_coordinates!(ids, coordinates, pd)
        if length(ids) != 2 * size(coordinates, 2)
            throw(
                DimensionMismatch(
                    "storage length for element ids does not match the number of coordinates",
                ),
            )
        end
    ...
    function is_in_triangle(point, x, y)
        ...
        b = SVector(x[2], y[2])
        c = SVector(x[3], y[3])
        return is_on_same_side(point, a, b, c) &&
               is_on_same_side(point, b, c, a) &&
    ...
    function triangle_interpolation(
        x_coordinates_in,
        y_coordinates_in,
        values_in,
        coordinate_out,
