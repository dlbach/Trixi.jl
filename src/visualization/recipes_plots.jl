...
@muladd begin
    ...
    RecipesBase.@recipe function f(
        pm::PlotMesh{<:PlotData2DCartesian{<:Any,<:AbstractVector{<:AbstractVector}}},
    )
        @unpack plot_data = pm
        @unpack x, y, mesh_vertices_x, mesh_vertices_y = plot_data
    ...
    RecipesBase.@recipe function f(pds::PlotDataSeries{<:PlotData2DTriangulated})
        ...
        return DGTriPseudocolor(
            global_plotting_triangulation_triplot((x, y), data_field, t)...,
        )
    end
    ...
    RecipesBase.@recipe function f(pd::PlotData2DTriangulated{<:ScalarData})
        ...
        # `data.data` instead.
        return DGTriPseudocolor(
            global_plotting_triangulation_triplot((x, y), data.data, t)...,
        )
    end
