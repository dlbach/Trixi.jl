...
@muladd begin
    ...
    function convergence_test(mod::Module, elixir::AbstractString, iterations; kwargs...)
        ...
        errorsmatrix = Dict(
            kind => transpose(reshape(error, (nvariables, iterations))) for
            (kind, error) in errors
        )
        ...
        eocs = Dict(
            kind => log.(error[2:end, :] ./ error[1:end-1, :]) ./ log(1 / 2) for
            (kind, error) in errorsmatrix
        )
