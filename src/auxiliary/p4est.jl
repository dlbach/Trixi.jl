...
@muladd begin
    ...
    # 2D
    refine_p4est!(p4est::Ptr{p4est_t}, recursive, refine_fn_c, init_fn_c) =
        p4est_refine(p4est, recursive, refine_fn_c, init_fn_c)
    # 3D
    refine_p4est!(p8est::Ptr{p8est_t}, recursive, refine_fn_c, init_fn_c) =
        p8est_refine(p8est, recursive, refine_fn_c, init_fn_c)


    # Refine `p4est` if coarsen_fn_c returns 1
    # 2D
    coarsen_p4est!(p4est::Ptr{p4est_t}, recursive, coarsen_fn_c, init_fn_c) =
        p4est_coarsen(p4est, recursive, coarsen_fn_c, init_fn_c)
    # 3D
    coarsen_p4est!(p8est::Ptr{p8est_t}, recursive, coarsen_fn_c, init_fn_c) =
    ...
    # 2D
    function iterate_p4est(
        p4est::Ptr{p4est_t},
        user_data;
        ghost_layer = C_NULL,
        iter_volume_c = C_NULL,
        iter_face_c = C_NULL,
    )
        if user_data === C_NULL
