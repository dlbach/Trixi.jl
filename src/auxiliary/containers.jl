...
@muladd begin
    ...
    function copy_data!(
        ...
        if destination <= first || destination > last
            ...
            for i = 0:(count-1), j = 1:block_size
            ...
            for i = (count-1):-1:0, j = 1:block_size
                target[block_size*(destination+i-1)+j] = source[block_size*(first+i-1)+j]
            end
        end
    ...
    function move!(c::AbstractContainer, first::Int, last::Int, destination::Int)
        ...
        count = last - first + 1
        first_invalid =
            (first <= destination + count - 1 <= last) ? destination + count : first
    ...
    move!(c::AbstractContainer, from::Int, destination::Int) =
