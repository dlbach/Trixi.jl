...
@muladd begin
    ...
    struct SemidiscretizationEulerAcoustics{SemiAcoustics,SemiEuler,Cache} <:
        ...
        function SemidiscretizationEulerAcoustics{SemiAcoustics,SemiEuler,Cache}(
            ...
            semi_euler,
            cache,
        ) where {SemiAcoustics,SemiEuler,Cache}
    ...
    function SemidiscretizationEulerAcoustics(
        semi_acoustics::SemiAcoustics,
        semi_euler::SemiEuler;
        ...
        Mesh,
        SemiAcoustics<:SemidiscretizationHyperbolic{
            Mesh,
        ...
        return SemidiscretizationEulerAcoustics{
            typeof(semi_acoustics),
            typeof(semi_euler),
            typeof(cache),
        }(
            semi_acoustics,
    ...
    function create_cache(
        ::Type{SemidiscretizationEulerAcoustics},
        source_region,
        weights,
        mesh,
        ...
        acoustic_source_terms = zeros(
            eltype(cache.elements),
            (ndims(equations), nnodes(dg), nnodes(dg), length(coupled_element_ids)),
        )

        acoustic_source_weights = precompute_weights(
            source_region,
            weights,
            coupled_element_ids,
            equations,
    ...
    function precompute_weights(
        ...
        acoustic_source_weights = zeros(
            eltype(cache.elements),
            (nnodes(dg), nnodes(dg), length(coupled_element_ids)),
        )

        @threaded for k = 1:length(coupled_element_ids)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                ...
                acoustic_source_weights[i, j, k] =
                    source_region(x) ? weights(x) : zero(weights(x))
            end
        end
    ...
    function add_acoustic_source_terms!(
        du_acoustics,
        acoustic_source_terms,
        source_weights,
        coupled_element_ids,
        ...
        @threaded for k = 1:length(coupled_element_ids)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                du_acoustics[1, i, j, element] +=
                    source_weights[i, j, k] * acoustic_source_terms[1, i, j, k]
                du_acoustics[2, i, j, element] +=
                    source_weights[i, j, k] * acoustic_source_terms[2, i, j, k]
            end
        end
