...
@muladd begin
    ...
    """
    calc_error_norms(u_ode, t, analyzer, semi::AbstractSemidiscretization, cache_analysis) =
    ...
    and a vector `b`.
    """
    function linear_structure(semi::AbstractSemidiscretization; t0 = zero(real(semi)))
        # allocate memory
        u_ode = allocate_coefficients(mesh_equations_solver_cache(semi)...)
    ...
    function jacobian_ad_forward(semi::AbstractSemidiscretization, t0, _u0_ode::StructArray)
        u0_ode_plain =
            similar(_u0_ode, eltype(eltype(_u0_ode)), (size(_u0_ode)..., nvariables(semi)))
    ...
    function _jacobian_ad_forward_structarrays(semi, t0, u0_ode_plain, du_ode_plain, config)
        ...
        J = ForwardDiff.jacobian(
            ...
            u0_ode_plain,
            config,
        ) do du_ode_plain, u_ode_plain
            u_ode = StructArray{SVector{nvariables(semi),eltype(config)}}(
                ntuple(v -> view(u_ode_plain, :, :, v), nvariables(semi)),
            )
            du_ode = StructArray{SVector{nvariables(semi),eltype(config)}}(
                ntuple(v -> view(du_ode_plain, :, :, v), nvariables(semi)),
            )
            Trixi.rhs!(du_ode, u_ode, new_semi, t0)
        end
