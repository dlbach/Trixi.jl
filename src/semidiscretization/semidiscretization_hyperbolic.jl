...
@muladd begin
    ...
    struct SemidiscretizationHyperbolic{
        ...
        function SemidiscretizationHyperbolic{
            Mesh,
            Equations,
            InitialCondition,
            BoundaryConditions,
    ...
    function SemidiscretizationHyperbolic(
        ...
        cache =
            (; create_cache(mesh, equations, solver, RealT, uEltype)..., initial_cache...)
        _boundary_conditions =
            digest_boundary_conditions(boundary_conditions, mesh, solver, cache)
    ...
    function digest_boundary_conditions(
        ...
        (;
            x_neg = boundary_conditions,
            x_pos = boundary_conditions,
            y_neg = boundary_conditions,
    ...
    function digest_boundary_conditions(
        ...
        (;
            ...
            y_neg = boundary_conditions,
            y_pos = boundary_conditions,
            z_neg = boundary_conditions,
            z_pos = boundary_conditions,
        )
    end
    ...
    function digest_boundary_conditions(
        ...
        (;
            x_neg = boundary_conditions[1],
            x_pos = boundary_conditions[2],
            y_neg = boundary_conditions[3],
            y_pos = boundary_conditions[4],
        )
    end
    ...
    function digest_boundary_conditions(
        ...
        (;
            ...
            y_neg = boundary_conditions[3],
            y_pos = boundary_conditions[4],
            z_neg = boundary_conditions[5],
            z_pos = boundary_conditions[6],
        )
    end
    ...
    function digest_boundary_conditions(
        ...
        solver,
        cache,
    ) where {Keys,ValueTypes<:NTuple{2,Any}}
    ...
    function digest_boundary_conditions(
        ...
        solver,
        cache,
    ) where {Keys,ValueTypes<:NTuple{4,Any}}
    ...
    function digest_boundary_conditions(
        ...
        solver,
        cache,
    ) where {Keys,ValueTypes<:NTuple{6,Any}}
    ...
    function digest_boundary_conditions(
        boundary_conditions::AbstractArray,
        mesh,
        solver,
        cache,
    )
        throw(
            ArgumentError(
                "Please use a (named) tuple instead of an (abstract) array to supply multiple boundary conditions (to improve performance).",
            ),
        )
    end
    ...
    const SemiHypMeshBCSolver{Mesh,BoundaryConditions,Solver} =
        SemidiscretizationHyperbolic{
            ...
            SourceTerms,
            Solver,
        } where {Equations,InitialCondition,SourceTerms}

    # generic fallback: print the type of semi.boundary_condition.
    print_boundary_conditions(io, semi::SemiHypMeshBCSolver) =
        summary_line(io, "boundary conditions", typeof(semi.boundary_conditions))

    function print_boundary_conditions(
        io,
        semi::SemiHypMeshBCSolver{<:Any,<:UnstructuredSortedBoundaryTypes},
    )
    ...
    function print_boundary_conditions(
        ...
        semi::SemiHypMeshBCSolver{
            <:Union{TreeMesh,StructuredMesh},
            <:Union{Tuple,NamedTuple,AbstractArray},
        },
    )
