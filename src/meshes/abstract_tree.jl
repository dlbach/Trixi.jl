...
@muladd begin
    ...
    n_children(t::AbstractTree, cell_id::Int) =
    ...
    has_neighbor(t::AbstractTree, cell_id::Int, direction::Int) =
    ...
    @inline function set_cell_coordinates!(
        t::AbstractTree{NDIMS},
        coords,
        cell,
    ) where {NDIMS}
        for d = 1:NDIMS
    ...
    function init_children!(t::AbstractTree, cell_id, max_level)
        ...
        if t.levels[cell_id] >= max_level
            ...
            n_offspring = 0
            for child = 1:n_children_per_cell(t)
    ...
    function refine!(
        ...
        refined = @trixi_timeit timer() "refine_unbalanced!" refine_unbalanced!(
            t,
            cell_ids,
        ...
        refined_original_cells =
            @views(-t.original_cell_ids[1:length(t)][t.original_cell_ids[1:length(t)].<0])
    ...
    function refine_box!(
        ...
        cells = filter_leaf_cells(t) do cell_id
            return (
                all(coordinates_min .< cell_coordinates(t, cell_id)) &&
    ...
    function refine_box!(t::AbstractTree{1}, coordinates_min::Real, coordinates_max::Real)
        return refine_box!(
            t,
            [convert(Float64, coordinates_min)],
    ...
    function refine_sphere!(
        t::AbstractTree{NDIMS},
        center::SVector{NDIMS},
        radius,
    ) where {NDIMS}
            ...
            for child = 1:n_children_per_cell(t)
                if has_child(t, coarse_cell_id, child)
                    if !is_leaf(t, t.child_ids[child, coarse_cell_id])
                        error(
                            "cell $coarse_cell_id has child cell at position $child that is not a leaf cell",
                        )
                    end
                end
            end
    ...
    function coarsen_box!(
        t::AbstractTree{NDIMS},
        coordinates_min::AbstractArray{Float64},
        coordinates_max::AbstractArray{Float64},
    ) where {NDIMS}
        for dim = 1:NDIMS
            @assert coordinates_min[dim] < coordinates_max[dim] "Minimum coordinates are not minimum."
        end
    ...
    function child_coordinates(
        ::AbstractTree{NDIMS},
        parent_coordinates,
        parent_length::Number,
        child::Int,
    ) where {NDIMS}
        ...
        return SVector(
            ntuple(
                d -> parent_coordinates[d] + child_sign(child, d) * child_length / 2,
                Val(NDIMS),
            ),
        )
    end
            ...
            for direction in eachdirection(t)
                if has_neighbor(t, cell_id, direction)
                    t.neighbor_ids[
                        ...
                        t.neighbor_ids[direction, cell_id],
                    ] = 0
                end
            end
        end
