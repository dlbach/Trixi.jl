...
@muladd begin
    ...
    mutable struct TreeMesh{NDIMS,TreeType<:AbstractTree{NDIMS}} <: AbstractMesh{NDIMS}
        ...
        function TreeMesh{NDIMS,TreeType}(
            n_cells_max::Integer,
        ) where {NDIMS,TreeType<:AbstractTree{NDIMS}}
            # Create mesh
            m = new()
    ...
    TreeMesh(::Type{TreeType}, args...) where {NDIMS,TreeType<:AbstractTree{NDIMS}} =
        TreeMesh{NDIMS,TreeType}(args...)

    # Constructor accepting a single number as center (as opposed to an array) for 1D
    function TreeMesh{1,TreeType}(
        n::Int,
        center::Real,
        len::Real,
        periodicity = true,
    ) where {TreeType<:AbstractTree{1}}
        # TODO: Taal refactor, allow other RealT for the mesh, not just Float64
    ...
    function TreeMesh{NDIMS,TreeType}(
        ...
        # TODO: Taal refactor, allow other RealT for the mesh, not just Float64
        TreeMesh{NDIMS,TreeType}(
            n_cells_max,
            SVector{NDIMS,Float64}(domain_center),
            convert(Float64, domain_length),
            periodicity,
        )
    end

    function TreeMesh(
        coordinates_min::NTuple{NDIMS,Real},
        ...
        if !(n_cells_max isa Integer && n_cells_max > 0)
            throw(
                ArgumentError(
                    "`n_cells_max` must be a positive integer (provided `n_cells_max = $n_cells_max`)",
                ),
            )
        end
        if !(initial_refinement_level isa Integer && initial_refinement_level >= 0)
            throw(
                ArgumentError(
                    "`initial_refinement_level` must be a non-negative integer (provided `initial_refinement_level = $initial_refinement_level`)",
                ),
            )
        end
        ...
        # Create mesh
        mesh = @trixi_timeit timer() "creation" TreeMesh{NDIMS,TreeType}(
            n_cells_max,
            domain_center,
            domain_length,
            periodicity,
    ...
    function initialize!(
        ...
        @trixi_timeit timer() "initial refinement" refine_uniformly!(
            mesh.tree,
            initial_refinement_level,
        )
    ...
    function Base.show(
        io::IO,
        ::MIME"text/plain",
        mesh::TreeMesh{NDIMS,TreeType},
    ) where {NDIMS,TreeType}
        if get(io, :compact, false)
            ...
            summary_box(
                io,
                "TreeMesh{" * string(NDIMS) * ", " * string(TreeType) * "}",
                setup,
            )
        end
    end
