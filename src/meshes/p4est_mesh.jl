...
@muladd begin
    ...
    function P4estMesh(
        ...
        @assert count(i -> i !== nothing, (mapping, faces, coordinates_min)) == 1 "Exactly one of mapping, faces and coordinates_min/max must be specified"
        ...
        nodes = basis.nodes
        tree_node_coordinates = Array{RealT,NDIMS + 2}(
            undef,
            NDIMS,
            ntuple(_ -> length(nodes), NDIMS)...,
            prod(trees_per_dimension),
        )
        calc_tree_node_coordinates!(
            tree_node_coordinates,
            nodes,
            mapping,
            trees_per_dimension,
    ...
    # 2D version
    function structured_boundary_names!(
        boundary_names,
        trees_per_dimension::NTuple{2},
        periodicity,
    )
        ...
        if !periodicity[1]
            for cell_y = 1:trees_per_dimension[2]
        ...
        if !periodicity[2]
            for cell_x = 1:trees_per_dimension[1]
    ...
    function structured_boundary_names!(
        ...
        # Boundaries in x-direction
        if !periodicity[1]
            for cell_z = 1:trees_per_dimension[3], cell_y = 1:trees_per_dimension[2]
        ...
        # Boundaries in y-direction
        if !periodicity[2]
            for cell_z = 1:trees_per_dimension[3], cell_x = 1:trees_per_dimension[1]
        ...
        if !periodicity[3]
            for cell_y = 1:trees_per_dimension[2], cell_x = 1:trees_per_dimension[1]
    ...
    function P4estMesh{NDIMS}(
        ...
        if header == " File created by HOHQMesh"
            ...
            p4est, tree_node_coordinates, nodes, boundary_names =
                p4est_mesh_from_hohqmesh_abaqus(
                    meshfile,
    ...
    # [`HOHQMesh.jl`](https://github.com/trixi-framework/HOHQMesh.jl).
    function p4est_mesh_from_hohqmesh_abaqus(
        meshfile,
        initial_refinement_level,
        n_dimensions,
        RealT,
        ...
        tree_node_coordinates = Array{RealT,n_dimensions + 2}(
            undef,
            n_dimensions,
        ...
        file_idx = calc_tree_node_coordinates!(
            tree_node_coordinates,
            file_lines,
            nodes,
            vertices,
    ...
    function p4est_mesh_from_standard_abaqus(
        meshfile,
        mapping,
        polydeg,
        initial_refinement_level,
        ...
        vertices = unsafe_wrap(Array, connectivity_obj.vertices, (3, n_vertices))
        tree_to_vertex =
            unsafe_wrap(Array, connectivity_obj.tree_to_vertex, (2^n_dimensions, n_trees))
    ...
    function P4estMeshCubedSphere(
        ...
        tree_node_coordinates =
            Array{RealT,5}(undef, 3, ntuple(_ -> length(nodes), 3)..., n_trees)
        calc_tree_node_coordinates!(
            tree_node_coordinates,
            nodes,
            trees_per_face_dimension,
    ...
    function connectivity_structured(n_cells_x, n_cells_y, periodicity)
        ...
        for cell_y = 1:n_cells_y, cell_x = 1:n_cells_x
        ...
        connectivity = p4est_connectivity_new_copy(
            n_vertices,
            n_trees,
            n_corners,
            vertices,
    ...
    function connectivity_structured(n_cells_x, n_cells_y, n_cells_z, periodicity)
        ...
        for cell_z = 1:n_cells_z, cell_y = 1:n_cells_y, cell_x = 1:n_cells_x
        ...
        connectivity = p8est_connectivity_new_copy(
            n_vertices,
            n_trees,
            n_corners,
            n_edges,
    ...
    function connectivity_cubed_sphere(trees_per_face_dimension, layers)
        ...
        linear_indices =
            LinearIndices((trees_per_face_dimension, trees_per_face_dimension, layers, 6))
        ...
        for direction = 1:6
            for cell_z = 1:n_cells_z, cell_y = 1:n_cells_y, cell_x = 1:n_cells_x
                ...
                if cell_x > 1 # Connect to tree at the same face
                    tree_to_tree[1, tree] =
                        linear_indices[cell_x-1, cell_y, cell_z, direction] - 1
                    tree_to_face[1, tree] = 1
                ...
                elseif direction == 5 # This is the -z face
                    ...
                    target = 1
                    tree_to_tree[1, tree] =
                        linear_indices[end-cell_y+1, end, cell_z, target] - 1
                ...
                elseif direction == 5 # This is the -z face
                    target = 1
                    tree_to_tree[2, tree] =
                        linear_indices[end-cell_y+1, 1, cell_z, target] - 1
                ...
                if cell_y > 1 # Connect to tree at the same face
                    tree_to_tree[3, tree] =
                        linear_indices[cell_x, cell_y-1, cell_z, direction] - 1
                    tree_to_face[3, tree] = 3
                elseif direction == 1
                    target = 5
                    tree_to_tree[3, tree] =
                        linear_indices[end, end-cell_x+1, cell_z, target] - 1
                ...
                elseif direction == 3
                    target = 5
                    tree_to_tree[3, tree] =
                        linear_indices[end-cell_x+1, 1, cell_z, target] - 1
                ...
                elseif direction == 1
                    target = 6
                    tree_to_tree[4, tree] =
                        linear_indices[1, end-cell_x+1, cell_z, target] - 1
                ...
                elseif direction == 4
                    target = 6
                    tree_to_tree[4, tree] =
                        linear_indices[end-cell_x+1, end, cell_z, target] - 1
                ...
                if cell_z > 1
                    tree_to_tree[5, tree] =
                        linear_indices[cell_x, cell_y, cell_z-1, direction] - 1
                    tree_to_face[5, tree] = 5
                else # Non-periodic boundary, tree and face point to themselves (zero-based indexing)
                    tree_to_tree[5, tree] = tree - 1
                    tree_to_face[5, tree] = 4
    ...
    # 2D version
    function calc_tree_node_coordinates!(
        ...
        for cell_y = 1:trees_per_dimension[2], cell_x = 1:trees_per_dimension[1]
            ...
            for j in eachindex(nodes), i in eachindex(nodes)
                ...
                node_coordinates[:, i, j, tree_id] .= mapping(
                    cell_x_offset + dx / 2 * nodes[i],
    ...
    # 3D version
    function calc_tree_node_coordinates!(
        node_coordinates::AbstractArray{<:Any,5},
        nodes,
        mapping,
        trees_per_dimension,
    )
        ...
        for cell_z = 1:trees_per_dimension[3],
            cell_y = 1:trees_per_dimension[2],
            cell_x = 1:trees_per_dimension[1]
            ...
            for k in eachindex(nodes), j in eachindex(nodes), i in eachindex(nodes)
                # node_coordinates are the mapped reference node coordinates
                node_coordinates[:, i, j, k, tree_id] .= mapping(
                    cell_x_offset + dx / 2 * nodes[i],
                    cell_y_offset + dy / 2 * nodes[j],
    ...
    # 2D version
    function calc_tree_node_coordinates!(
        node_coordinates::AbstractArray{RealT,4},
        nodes,
        mapping,
        vertices,
        tree_to_vertex,
        ...
        for tree = 1:size(tree_to_vertex, 2)
    ...
    function map_node_coordinates!(node_coordinates::AbstractArray{<:Any,4}, mapping)
        for tree in axes(node_coordinates, 4),
            ...
            node_coordinates[:, i, j, tree] .=
                mapping(node_coordinates[1, i, j, tree], node_coordinates[2, i, j, tree])
        end

        return node_coordinates
    end

    function map_node_coordinates!(
        node_coordinates::AbstractArray{<:Any,4},
        mapping::Nothing,
    )
        return node_coordinates
    ...
    # 3D version
    function calc_tree_node_coordinates!(
        node_coordinates::AbstractArray{RealT,5},
        nodes,
        mapping,
        vertices,
        tree_to_vertex,
    ...
    function map_node_coordinates!(node_coordinates::AbstractArray{<:Any,5}, mapping)
        for tree in axes(node_coordinates, 5),
            ...
            node_coordinates[:, i, j, k, tree] .= mapping(
                node_coordinates[1, i, j, k, tree],
            ...
            if sum(curved_check) == 0
                ...
                for i = 1:4
                    if curved_check[i] == 0
                        ...
                        for k = 1:nnodes
                            curve_values[k, 1] = linear_interpolate(
                                nodes[k],
                                quad_vertices_flipped[m1, 1],
                                quad_vertices_flipped[m2, 1],
                            )
                            curve_values[k, 2] = linear_interpolate(
                                nodes[k],
                    ...
                    surface_curves[i] =
                        CurvedSurfaceT(nodes, bary_weights, copy(curve_values))
            ...
            if sum(curved_check) == 0
                ...
                for face = 1:6
                    if curved_check[face] == 0
                        ...
                        for q = 1:nnodes, p = 1:nnodes
                            @views bilinear_interpolation!(
                                curve_values[:, p, q],
                                face_vertices,
                                nodes[p],
                                nodes[q],
    ...
    # 2D
    cfunction(::typeof(init_fn), ::Val{2}) = @cfunction(
        init_fn,
        Cvoid,
        (Ptr{p4est_t}, Ptr{p4est_topidx_t}, Ptr{p4est_quadrant_t})
    )
    # 3D
    cfunction(::typeof(init_fn), ::Val{3}) = @cfunction(
        init_fn,
        Cvoid,
    ...
    # 2D
    cfunction(::typeof(refine_fn), ::Val{2}) = @cfunction(
        refine_fn,
        Cint,
        (Ptr{p4est_t}, Ptr{p4est_topidx_t}, Ptr{p4est_quadrant_t})
    )
    # 3D
    cfunction(::typeof(refine_fn), ::Val{3}) = @cfunction(
        refine_fn,
        Cint,
        (Ptr{p8est_t}, Ptr{p4est_topidx_t}, Ptr{p8est_quadrant_t})
    )
    ...
    function refine!(mesh::P4estMesh)
        ...
        # Refine marked cells
        @trixi_timeit timer() "refine" refine_p4est!(
            mesh.p4est,
            false,
            refine_fn_c,
            init_fn_c,
    ...
    # 2D
    unsafe_wrap_quadrants(quadrants_ptr, ::Ptr{p4est_t}) =
        unsafe_wrap(Array, quadrants_ptr, 4)
    # 3D
    unsafe_wrap_quadrants(quadrants_ptr, ::Ptr{p8est_t}) =
        unsafe_wrap(Array, quadrants_ptr, 8)

    # 2D
    cfunction(::typeof(coarsen_fn), ::Val{2}) = @cfunction(
        coarsen_fn,
        Cint,
        (Ptr{p4est_t}, Ptr{p4est_topidx_t}, Ptr{Ptr{p4est_quadrant_t}})
    )
    # 3D
    cfunction(::typeof(coarsen_fn), ::Val{3}) = @cfunction(
        coarsen_fn,
        Cint,
    ...
    function coarsen!(mesh::P4estMesh)
        ...
        @trixi_timeit timer() "coarsen!" coarsen_p4est!(
            mesh.p4est,
            false,
            coarsen_fn_c,
            init_fn_c,
    ...
    # 2D
    cfunction(::typeof(save_original_id_iter_volume), ::Val{2}) = @cfunction(
        save_original_id_iter_volume,
        Cvoid,
        (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(save_original_id_iter_volume), ::Val{3}) = @cfunction(
        save_original_id_iter_volume,
        Cvoid,
    ...
    # 2D
    cfunction(::typeof(collect_changed_iter_volume), ::Val{2}) = @cfunction(
        collect_changed_iter_volume,
        Cvoid,
        (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(collect_changed_iter_volume), ::Val{3}) = @cfunction(
        collect_changed_iter_volume,
        Cvoid,
        (Ptr{p8est_iter_volume_info_t}, Ptr{Cvoid})
    )
    ...
    # 2D
    cfunction(::typeof(collect_new_iter_volume), ::Val{2}) = @cfunction(
        collect_new_iter_volume,
        Cvoid,
        (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(collect_new_iter_volume), ::Val{3}) = @cfunction(
        collect_new_iter_volume,
        Cvoid,
        (Ptr{p8est_iter_volume_info_t}, Ptr{Cvoid})
    )
