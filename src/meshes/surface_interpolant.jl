...
@muladd begin
    ...
    function evaluate_at(s, boundary_curve::CurvedSurface)
        ...
        x_coordinate_at_s_on_boundary_curve =
            lagrange_interpolation(s, nodes, view(coordinates, :, 1), barycentric_weights)
        y_coordinate_at_s_on_boundary_curve =
    ...
    function derivative_at(s, boundary_curve::CurvedSurface)
        ...
        x_coordinate_at_s_on_boundary_curve_prime = lagrange_interpolation_derivative(
            s,
            nodes,
            view(coordinates, :, 1),
            barycentric_weights,
        )
        y_coordinate_at_s_on_boundary_curve_prime = lagrange_interpolation_derivative(
            s,
            nodes,
            view(coordinates, :, 2),
            barycentric_weights,
