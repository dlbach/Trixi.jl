...
@muladd begin
    ...
    function calc_node_coordinates!(
        ...
        for k in eachindex(nodes), j in eachindex(nodes), i in eachindex(nodes)
            node_coordinates[:, i, j, k, element] .=
                straight_side_hex_map(nodes[i], nodes[j], nodes[k], corners)
        end
    ...
    function transfinite_hex_map(xi, eta, zeta, face_curves::AbstractVector{<:CurvedFace})
        ...
        for j = 1:3
            ...
            coordinate[j] = (
                0.5 * (
                    face_values[j, 6] * (1 - xi) +
                    face_values[j, 4] * (1 + xi) +
    ...
    function calc_node_coordinates!(
        ...
        for k in eachindex(nodes), j in eachindex(nodes), i in eachindex(nodes)
            node_coordinates[:, i, j, k, element] .=
                transfinite_hex_map(nodes[i], nodes[j], nodes[k], face_curves)
        end
