...
@muladd begin
    ...
    function StructuredMesh(
        ...
        return StructuredMesh{NDIMS,RealT}(
    ...
    linear_interpolate(s, left_value, right_value) =
    ...
    function coordinates2mapping(coordinates_min::NTuple{2}, coordinates_max::NTuple{2})
        mapping(xi, eta) = SVector(
            linear_interpolate(xi, coordinates_min[1], coordinates_max[1]),
    ...
    function transfinite_mapping(faces::NTuple{6,Any})
        mapping(x, y, z) = (
            ...
            correction_term_3d(x, y, z, faces) + trilinear_mapping(x, y, z, faces)
        )
    end
    ...
    function validate_faces(faces::NTuple{6,Any})
        @assert (faces[1](-1, -1) ≈ faces[3](-1, -1) ≈ faces[5](-1, -1)) "faces[1](-1, -1), faces[3](-1, -1) and faces[5](-1, -1) need to match at (-1, -1, -1) corner"

        @assert (faces[2](-1, -1) ≈ faces[3](1, -1) ≈ faces[5](1, -1)) "faces[2](-1, -1), faces[3](1, -1) and faces[5](1, -1) need to match at (1, -1, -1) corner"

        @assert (faces[1](1, -1) ≈ faces[4](-1, -1) ≈ faces[5](-1, 1)) "faces[1](1, -1), faces[4](-1, -1) and faces[5](-1, 1) need to match at (-1, 1, -1) corner"

        @assert (faces[2](1, -1) ≈ faces[4](1, -1) ≈ faces[5](1, 1)) "faces[2](1, -1), faces[4](1, -1) and faces[5](1, 1) need to match at (1, 1, -1) corner"

        @assert (faces[1](-1, 1) ≈ faces[3](-1, 1) ≈ faces[6](-1, -1)) "faces[1](-1, 1), faces[3](-1, 1) and faces[6](-1, -1) need to match at (-1, -1, 1) corner"

        @assert (faces[2](-1, 1) ≈ faces[3](1, 1) ≈ faces[6](1, -1)) "faces[2](-1, 1), faces[3](1, 1) and faces[6](1, -1) need to match at (1, -1, 1) corner"

        @assert (faces[1](1, 1) ≈ faces[4](-1, 1) ≈ faces[6](-1, 1)) "faces[1](1, 1), faces[4](-1, 1) and faces[6](-1, 1) need to match at (-1, 1, 1) corner"

        @assert (faces[2](1, 1) ≈ faces[4](1, 1) ≈ faces[6](1, 1)) "faces[2](1, 1), faces[4](1, 1) and faces[6](1, 1) need to match at (1, 1, 1) corner"
    end
