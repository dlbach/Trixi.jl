...
@muladd begin
    ...
    function evaluate_at(s, boundary_face::CurvedFace)
        ...
        x_coordinate_at_s_on_boundary_face = lagrange_interpolation_2d(
            s,
            nodes,
            view(coordinates, 1, :, :),
            barycentric_weights,
        )
        y_coordinate_at_s_on_boundary_face = lagrange_interpolation_2d(
            s,
            nodes,
            view(coordinates, 2, :, :),
            barycentric_weights,
        )
        z_coordinate_at_s_on_boundary_face = lagrange_interpolation_2d(
            s,
            nodes,
            view(coordinates, 3, :, :),
            barycentric_weights,
    ...
    function lagrange_interpolation_2d(x, nodes, function_values, barycentric_weights)
        ...
        for j in eachindex(nodes)
            f_intermediate[j] = lagrange_interpolation(
                x[2],
                nodes,
        ...
        point_value =
            lagrange_interpolation(x[1], nodes, f_intermediate, barycentric_weights)
