...
@muladd begin
    ...
    function save_mesh_file(mesh::TreeMesh, output_directory, timestep, mpi_parallel::False)
    ...
    function load_mesh_serial(mesh_file::AbstractString; n_cells_max, RealT)
        ndims, mesh_type = h5open(mesh_file, "r") do file
            return read(attributes(file)["ndims"]), read(attributes(file)["mesh_type"])
        end
        ...
        elseif mesh_type == "StructuredMesh"
            size_, mapping_as_string = h5open(mesh_file, "r") do file
                return read(attributes(file)["size"]), read(attributes(file)["mapping"])
            end
        ...
        elseif mesh_type == "P4estMesh"
            p4est_filename, tree_node_coordinates, nodes, boundary_names_ =
    ...
    function load_mesh!(mesh::ParallelTreeMesh, mesh_file::AbstractString)
        ...
        if mpi_isroot()
            h5open(mesh_file, "r") do file
                ...
                @views MPI.Bcast!(
                    mesh.tree.neighbor_ids[:, 1:n_cells],
                    mpi_root(),
                    mpi_comm(),
                )
                @views MPI.Bcast!(mesh.tree.levels[1:n_cells], mpi_root(), mpi_comm())
                @views MPI.Bcast!(
                    mesh.tree.coordinates[:, 1:n_cells],
                    mpi_root(),
                    mpi_comm(),
                )
            end
            ...
            # Set domain information
            mesh.tree.center_level_0 =
                MPI.Bcast!(collect(mesh.tree.center_level_0), mpi_root(), mpi_comm())
            mesh.tree.length_level_0 =
                MPI.Bcast!(collect(mesh.tree.length_level_0), mpi_root(), mpi_comm())[1]
            mesh.tree.periodicity =
                Tuple(MPI.Bcast!(collect(mesh.tree.periodicity), mpi_root(), mpi_comm()))
