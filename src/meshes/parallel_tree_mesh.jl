...
@muladd begin
    ...
    function partition!(mesh::ParallelTreeMesh; allow_coarsening = true)
        ...
        n_leaves_per_rank = OffsetArray(
            fill(div(length(leaves), mpi_nranks()), mpi_nranks()),
            0:(mpi_nranks()-1),
        )
        for d = 0:(rem(length(leaves), mpi_nranks())-1)
        ...
        for d = 0:(mpi_nranks()-1)
            ...
            if allow_coarsening &&
               ...
               d < length(n_leaves_per_rank) - 1
                ...
                if length(additional_cells) > 0
                    ...
                    additional_leaves =
                        count(id -> is_leaf(mesh.tree, id), additional_cells)
            ...
            if d < length(n_leaves_per_rank) - 1
                mesh.first_cell_by_rank[d+1] =
                    mesh.first_cell_by_rank[d] + mesh.n_cells_by_rank[d]
            end
        end
