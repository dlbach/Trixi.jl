...
@muladd begin
    ...
    varnames(::typeof(cons2cons), ::CompressibleEulerEquations3D) =
        ("rho", "rho_v1", "rho_v2", "rho_v3", "rho_e")
    varnames(::typeof(cons2prim), ::CompressibleEulerEquations3D) =
        ("rho", "v1", "v2", "v3", "p")
    ...
    """
    function initial_condition_eoc_test_coupled_euler_gravity(
        x,
        t,
        equations::CompressibleEulerEquations3D,
    )
    ...
    """
    @inline function source_terms_eoc_test_coupled_euler_gravity(
        u,
        x,
        t,
        equations::CompressibleEulerEquations3D,
    ...
    @inline function boundary_condition_slip_wall(
        ...
        if v_normal <= 0.0
            ...
            p_star =
                p_local *
                (
                    1 + 0.5 * (equations.gamma - 1) * v_normal / sound_speed
                )^(2 * equations.gamma * equations.inv_gamma_minus_one)
        else # v_normal > 0.0
            A = 2 / ((equations.gamma + 1) * rho_local)
            B = p_local * (equations.gamma - 1) / (equations.gamma + 1)
            p_star =
                p_local +
                0.5 * v_normal / A * (v_normal + sqrt(v_normal^2 + 4 * A * (p_local + B)))
        end
    ...
    @inline function boundary_condition_slip_wall(
        ...
        return boundary_condition_slip_wall(
            u_inner,
            normal_direction,
            direction,
            x,
    ...
    @inline function boundary_condition_slip_wall(
        ...
        if isodd(direction)
            ...
            boundary_flux = boundary_condition_slip_wall(
                u_inner,
                normal_direction,
                x,
                t,
    ...
    """
    @inline function flux_shima_etal(
        u_ll,
        u_rr,
        orientation::Integer,
        equations::CompressibleEulerEquations3D,
    ...
    """
    @inline function flux_kennedy_gruber(
        u_ll,
        u_rr,
        orientation::Integer,
        equations::CompressibleEulerEquations3D,
    ...
    """
    @inline function flux_chandrashekar(
        u_ll,
        u_rr,
        orientation::Integer,
        equations::CompressibleEulerEquations3D,
        ...
        if orientation == 1
            ...
            f4 = f1 * v3_avg
            f5 =
                f1 * 0.5 * (1 / (equations.gamma - 1) / beta_mean - velocity_square_avg) +
                f2 * v1_avg +
                f3 * v2_avg +
                f4 * v3_avg
        elseif orientation == 2
    ...
    """
    @inline function splitting_steger_warming(
        u,
        orientation::Integer,
        equations::CompressibleEulerEquations3D,
    )
    ...
    @inline function splitting_steger_warming(
        ...
        if orientation == 1
            ...
            f5p =
                rho_2gamma * (
                    alpha_p * 0.5 * (v1^2 + v2^2 + v3^2) +
                    a * v1 * (lambda2_p - lambda3_p) +
    ...
    @inline function splitting_steger_warming(
        ...
        if orientation == 1
            ...
            f5m =
                rho_2gamma * (
                    alpha_m * 0.5 * (v1^2 + v2^2 + v3^2) +
                    a * v1 * (lambda2_m - lambda3_m) +
    ...
    @inline function (flux_lmars::FluxLMARS)(
        u_ll,
        u_rr,
        orientation::Integer,
        equations::CompressibleEulerEquations3D,
    ...
    function flux_hllc(
        ...
        elseif orientation == 2 # y-direction
            ...
            vel_R = v2_rr
            ekin_roe =
                (sqrt_rho_ll * v1_ll + sqrt_rho_rr * v1_rr)^2 +
                (sqrt_rho_ll * v3_ll + sqrt_rho_rr * v3_rr)^2
        else # z-direction
            vel_L = v3_ll
            vel_R = v3_rr
    ...
    function flux_hlle(
        ...
        elseif SsR <= 0.0 && SsL < 0.0
            ...
            f1 =
                (SsR * f_ll[1] - SsL * f_rr[1] + SsL * SsR * (u_rr[1] - u_ll[1])) /
                (SsR - SsL)
            f2 =
                (SsR * f_ll[2] - SsL * f_rr[2] + SsL * SsR * (u_rr[2] - u_ll[2])) /
                (SsR - SsL)
            f3 =
                (SsR * f_ll[3] - SsL * f_rr[3] + SsL * SsR * (u_rr[3] - u_ll[3])) /
                (SsR - SsL)
            f4 =
                (SsR * f_ll[4] - SsL * f_rr[4] + SsL * SsR * (u_rr[4] - u_ll[4])) /
                (SsR - SsL)
            f5 =
                (SsR * f_ll[5] - SsL * f_rr[5] + SsL * SsR * (u_rr[5] - u_ll[5])) /
                (SsR - SsL)
        end
    ...
    @inline function entropy2cons(w, equations::CompressibleEulerEquations3D)
        ...
        # eq. (52)
        rho_iota =
            ((gamma - 1) / (-V5)^gamma)^(equations.inv_gamma_minus_one) *
    ...
    @inline function density_pressure(u, equations::CompressibleEulerEquations3D)
        rho, rho_v1, rho_v2, rho_v3, rho_e = u
        rho_times_p =
            (equations.gamma - 1) * (rho * rho_e - 0.5 * (rho_v1^2 + rho_v2^2 + rho_v3^2))
        return rho_times_p
    end
    ...
    @inline entropy(cons, equations::CompressibleEulerEquations3D) =
