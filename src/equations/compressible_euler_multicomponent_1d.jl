...
@muladd begin
    ...
    struct CompressibleEulerMulticomponentEquations1D{NVARS,NCOMP,RealT<:Real} <:
        ...
        function CompressibleEulerMulticomponentEquations1D{NVARS,NCOMP,RealT}(
            gammas::SVector{NCOMP,RealT},
            gas_constants::SVector{NCOMP,RealT},
    ...
    function CompressibleEulerMulticomponentEquations1D(; gammas, gas_constants)
        ...
        return CompressibleEulerMulticomponentEquations1D{NVARS,NCOMP,RealT}(
            __gammas,
            __gas_constants,
        )
    end
    ...
    @inline Base.real(
        ::CompressibleEulerMulticomponentEquations1D{NVARS,NCOMP,RealT},
    ...
    @inline function source_terms_convergence_test(
        ...
        du_rho = SVector{ncomponents(equations),real(equations)}(
            0.0 for i in eachcomponent(equations)
        )
    ...
    @inline function flux_chandrashekar(
        ...
        rhok_mean = SVector{ncomponents(equations),real(equations)}(
            ln_mean(u_ll[i+2], u_rr[i+2]) for i in eachcomponent(equations)
        )
        rhok_avg = SVector{ncomponents(equations),real(equations)}(
            0.5 * (u_ll[i+2] + u_rr[i+2]) for i in eachcomponent(equations)
        )
    ...
    @inline function cons2prim(u, equations::CompressibleEulerMulticomponentEquations1D)
        ...
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            u[i+2] for i in eachcomponent(equations)
        )
    ...
    @inline function prim2cons(prim, equations::CompressibleEulerMulticomponentEquations1D)
        ...
        cons_rho = SVector{ncomponents(equations),RealT}(
            prim[i+2] for i in eachcomponent(equations)
        )
    ...
    @inline function densities(u, v, equations::CompressibleEulerMulticomponentEquations1D)
        ...
        return SVector{ncomponents(equations),real(equations)}(
            u[i+2] * v for i in eachcomponent(equations)
        )
    end
