...
@muladd begin
    ...
    * Trixi.jl's visualization tools will visualize the mean values by default.
    ...
    function AcousticPerturbationEquations2D(
        v_mean_global::NTuple{2,<:Real},
        c_mean_global::Real,
        rho_mean_global::Real,
    )
        return AcousticPerturbationEquations2D(
            SVector(v_mean_global),
            c_mean_global,
            rho_mean_global,
        )
    end
    ...
    varnames(::typeof(cons2prim), ::AcousticPerturbationEquations2D) =
        ("v1_prime", "v2_prime", "p_prime", "v1_mean", "v2_mean", "c_mean", "rho_mean")
    ...
    varnames(::typeof(cons2state), ::AcousticPerturbationEquations2D) =
        ("v1_prime", "v2_prime", "p_prime_scaled")
    varnames(::typeof(cons2mean), ::AcousticPerturbationEquations2D) =
    ...
    function global_mean_vars(equations::AcousticPerturbationEquations2D)
        return equations.v_mean_global[1],
        equations.v_mean_global[2],
    ...
    """
    function source_terms_convergence_test(
        u,
        x,
        t,
        equations::AcousticPerturbationEquations2D,
        ...
        du3 =
            A * omega * co * (2 * c_mean^2 * rho_mean + 2 * c * tmp + 2 * A * tmp * si) /
            c_mean^2
    ...
    """
    function boundary_condition_wall(
        u_inner,
        orientation,
        direction,
        x,
    ...
    function boundary_condition_slip_wall(
        ...
        u_boundary = SVector(
            u_inner[1] - 2.0 * u_normal * normal[1],
    ...
    @inline function flux(
        ...
        f1 =
            normal_direction[1] *
            (v1_mean * v1_prime + v2_mean * v2_prime + c_mean^2 * p_prime_scaled / rho_mean)
        f2 =
            normal_direction[2] *
            (v1_mean * v1_prime + v2_mean * v2_prime + c_mean^2 * p_prime_scaled / rho_mean)
        f3 = (
            normal_direction[1] * (rho_mean * v1_prime + v1_mean * p_prime_scaled) +
    ...
    @inline function max_abs_speed_naive(
        ...
        λ_max =
            max(abs(v_ll), abs(v_rr)) + max(c_mean_ll, c_mean_rr) * norm(normal_direction)
    end
