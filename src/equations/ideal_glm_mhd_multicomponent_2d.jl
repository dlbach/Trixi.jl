...
@muladd begin
    ...
    mutable struct IdealGlmMhdMulticomponentEquations2D{NVARS,NCOMP,RealT<:Real} <:
        ...
        function IdealGlmMhdMulticomponentEquations2D{NVARS,NCOMP,RealT}(
            gammas::SVector{NCOMP,RealT},
            gas_constants::SVector{NCOMP,RealT},
        ) where {NVARS,NCOMP,RealT<:Real}

            NCOMP >= 1 || throw(
                DimensionMismatch(
                    "`gammas` and `gas_constants` have to be filled with at least one value",
                ),
            )
    ...
    function IdealGlmMhdMulticomponentEquations2D(; gammas, gas_constants)
        ...
        return IdealGlmMhdMulticomponentEquations2D{NVARS,NCOMP,RealT}(
            __gammas,
            __gas_constants,
        )
    end
    ...
    @inline Base.real(
        ::IdealGlmMhdMulticomponentEquations2D{NVARS,NCOMP,RealT},
    ) where {NVARS,NCOMP,RealT} = RealT
    ...
    default_analysis_integrals(::IdealGlmMhdMulticomponentEquations2D) =
    ...
    """
    function initial_condition_convergence_test(
        x,
        t,
        ...
        rho = 1
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            2^(i - 1) * (1 - 2) / (1 - 2^ncomponents(equations)) * rho for
            i in eachcomponent(equations)
        )
        v1 = -B_perp * sin(alpha)
        v2 = B_perp * cos(alpha)
    ...
    function initial_condition_weak_blast_wave(
        ...
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            r > 0.5 ? 2^(i - 1) * (1 - 2) / (1 - 2^ncomponents(equations)) * 1.0 :
            2^(i - 1) * (1 - 2) / (1 - 2^ncomponents(equations)) * 1.1691 for
            i in eachcomponent(equations)
        )
    ...
    @inline function flux(
        ...
        if orientation == 1
            ...
            f3 = rho_v1 * v3 - B1 * B3
            f4 =
                (kin_en + gamma * p / (gamma - 1) + 2 * mag_en) * v1 -
            ...
            f3 = rho_v2 * v3 - B2 * B3
            f4 =
                (kin_en + gamma * p / (gamma - 1) + 2 * mag_en) * v2 -
    ...
    function flux_derigs_etal(
        ...
        rhok_mean = SVector{ncomponents(equations),real(equations)}(
            ln_mean(u_ll[i+8], u_rr[i+8]) for i in eachcomponent(equations)
        )
        rhok_avg = SVector{ncomponents(equations),real(equations)}(
            0.5 * (u_ll[i+8] + u_rr[i+8]) for i in eachcomponent(equations)
        )
        ...
        help2 = zero(T_rr)
        if orientation == 1
            f_rho = SVector{ncomponents(equations),real(equations)}(
                rhok_mean[i] * v1_avg for i in eachcomponent(equations)
            )
            for i in eachcomponent(equations)
                help1 += f_rho[i] * cv[i]
                help2 += f_rho[i]
            ...
            f4 =
                (help1 / T_log) - 0.5 * (vel_norm_avg) * (help2) +
                f1 * v1_avg +
                f2 * v2_avg +
                f3 * v3_avg +
                f5 * B1_avg +
                f6 * B2_avg +
                f7 * B3_avg +
                f8 * psi_avg - 0.5 * v1_mag_avg + B1_avg * vel_dot_mag_avg -
                c_h * psi_B1_avg

        else
            f_rho = SVector{ncomponents(equations),real(equations)}(
                rhok_mean[i] * v2_avg for i in eachcomponent(equations)
            )
            for i in eachcomponent(equations)
                help1 += f_rho[i] * cv[i]
                help2 += f_rho[i]
            ...
            f4 =
                ...
                f6 * B2_avg +
                f7 * B3_avg +
                f8 * psi_avg - 0.5 * v2_mag_avg + B2_avg * vel_dot_mag_avg -
                c_h * psi_B2_avg
    ...
    @inline function flux_hindenlang_gassner(
        ...
        if orientation == 1
            ...
            f5 = (
                f1 * (velocity_square_avg + inv_rho_p_mean * inv_gamma_minus_one) +
                0.5 * (
                    +p_ll * v1_rr +
                    p_rr * v1_ll +
    ...
    function cons2prim(u, equations::IdealGlmMhdMulticomponentEquations2D)
        ...
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            u[i+8] for i in eachcomponent(equations)
        )
        rho = density(u, equations)
    ...
    @inline function cons2entropy(u, equations::IdealGlmMhdMulticomponentEquations2D)
        ...
        entrop_rho = SVector{ncomponents(equations),real(equations)}(
            -1.0 * (cv[i] * log(T) - gas_constants[i] * log(u[i+8])) +
            gas_constants[i] +
            cv[i] - (v_square / (2 * T)) for i in eachcomponent(equations)
        )
    ...
    @inline function prim2cons(prim, equations::IdealGlmMhdMulticomponentEquations2D)
        ...
        cons_rho = SVector{ncomponents(equations),real(equations)}(
            prim[i+8] for i in eachcomponent(equations)
        )
        ...
        cons_other =
            SVector{8,real(equations)}(rho_v1, rho_v2, rho_v3, rho_e, B1, B2, B3, psi)
    ...
    @inline function densities(u, v, equations::IdealGlmMhdMulticomponentEquations2D)
        ...
        return SVector{ncomponents(equations),real(equations)}(
            u[i+8] * v for i in eachcomponent(equations)
        )
    end
