...
@muladd begin
    ...
    struct CompressibleEulerMulticomponentEquations2D{NVARS,NCOMP,RealT<:Real} <:
        ...
        function CompressibleEulerMulticomponentEquations2D{NVARS,NCOMP,RealT}(
            gammas::SVector{NCOMP,RealT},
            gas_constants::SVector{NCOMP,RealT},
    ...
    function CompressibleEulerMulticomponentEquations2D(; gammas, gas_constants)
        ...
        return CompressibleEulerMulticomponentEquations2D{NVARS,NCOMP,RealT}(
            __gammas,
            __gas_constants,
        )
    end
    ...
    @inline Base.real(
        ::CompressibleEulerMulticomponentEquations2D{NVARS,NCOMP,RealT},
    ...
    @inline function source_terms_convergence_test(
        ...
        # Here we compute an arbitrary number of different rhos. (one rho is double the next rho while the sum of all rhos is 1
        du_rho = SVector{ncomponents(equations),real(equations)}(
            2^(i - 1) * (1 - 2) / (1 - 2^ncomponents(equations)) * tmp1 for
            i in eachcomponent(equations)
        )
    ...
    @inline function flux_chandrashekar(
        ...
        rhok_mean = SVector{ncomponents(equations),real(equations)}(
            ln_mean(u_ll[i+3], u_rr[i+3]) for i in eachcomponent(equations)
        )
        rhok_avg = SVector{ncomponents(equations),real(equations)}(
            0.5 * (u_ll[i+3] + u_rr[i+3]) for i in eachcomponent(equations)
        )
        ...
        if orientation == 1
            ...
            f2 = (help2) * v2_avg
            f3 =
                (help1) / T_log - 0.5 * (v1_square + v2_square) * (help2) +
                v1_avg * f1 +
                v2_avg * f2
        else
            f_rho = SVector{ncomponents(equations),real(equations)}(
    ...
    @inline function flux_ranocha(
        ...
        if orientation == 1
            ...
            f2 = f_rho_sum * v2_avg
            f3 =
                f_rho_sum * (velocity_square_avg + inv_rho_p_mean * inv_gamma_minus_one) +
    ...
    @inline function cons2prim(u, equations::CompressibleEulerMulticomponentEquations2D)
        ...
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            u[i+3] for i in eachcomponent(equations)
        )
    ...
    @inline function cons2entropy(u, equations::CompressibleEulerMulticomponentEquations2D)
        ...
        entrop_rho = SVector{ncomponents(equations),real(equations)}(
            gas_constant * ((gamma - s) / (gamma - 1.0) - (0.5 * v_square * rho_p)) for
            i in eachcomponent(equations)
        )
    ...
    @inline function prim2cons(prim, equations::CompressibleEulerMulticomponentEquations2D)
        ...
        cons_rho = SVector{ncomponents(equations),real(equations)}(
            prim[i+3] for i in eachcomponent(equations)
        )
    ...
    @inline function density_pressure(
        u,
        equations::CompressibleEulerMulticomponentEquations2D,
    )
        rho_v1, rho_v2, rho_e = u
    ...
    @inline function densities(u, v, equations::CompressibleEulerMulticomponentEquations2D)
        ...
        return SVector{ncomponents(equations),real(equations)}(
            u[i+3] * v for i in eachcomponent(equations)
        )
    end
