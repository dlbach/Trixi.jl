...
@inline function flux_upwind(u_ll, u_rr, normal_direction::AbstractVector, equations::GLMMaxwellEquations2D)
  ...
  flux_component_2 = normal_direction[1] * u_diff[2] - normal_direction[2] * u_diff[1] + c * u_sum[3]
  
  f1 = 0.5 * c * ( normal_direction[1] * flux_component_1 - normal_direction[2] * flux_component_2 )
  f2 = 0.5 * c * ( normal_direction[2] * flux_component_1 + normal_direction[1] * flux_component_2 )
...
function boundary_condition_perfect_conducting_wall(u_inner, orientation, direction, x, t,
  ...
  if iseven(direction)
    return surface_flux_function(u_inner, SVector(-u_inner[1], -u_inner[2], u_inner[3], -u_inner[4] - psi_outer), orientation, equations)
  else
    return surface_flux_function(SVector(-u_inner[1], -u_inner[2], u_inner[3], -u_inner[4] + psi_outer), u_inner, orientation, equations)
  end
end
...
function boundary_condition_perfect_conducting_wall(u_inner, normal_direction::AbstractVector, direction, x, t,
  ...
  c = 299792458.0
  norm_ = copy(normal_direction)/norm(normal_direction)
  psi_outer = 2.0 * equations.c_h * ( norm_[1] * u_inner[1] + norm_[2] * u_inner[2]) / c
  if iseven(direction)
    return surface_flux_function(u_inner, SVector(-u_inner[1], -u_inner[2], u_inner[3], -u_inner[4] - psi_outer), norm_, equations)
  else
    return -surface_flux_function(u_inner, SVector(-u_inner[1], -u_inner[2], u_inner[3], -u_inner[4] + psi_outer), -norm_, equations)
  end
end
...
min_max_speed_naive(u_ll, u_rr, orientation, equations::GLMMaxwellEquations2D) = max(1.0,equations.c_h)*(-299792458.0,299792458.0)

max_abs_speeds(u, equations::GLMMaxwellEquations2D) = (max(1.0,equations.c_h)*299792458.0,max(1.0,equations.c_h)*299792458.0)

max_abs_speed_naive(u_ll, u_rr, orientation, equations::GLMMaxwellEquations2D) = max(1.0,equations.c_h)*299792458.0
