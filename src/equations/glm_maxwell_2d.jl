...
@inline function flux_upwind(u_ll,
    ...
    u_sum = u_ll + u_rr
    u_diff = u_ll - u_rr
    flux_component_1 = equations.c_h *
    ...
    f3 = 0.5 * (normal_direction[1] * u_sum[2] - normal_direction[2] * u_sum[1] +
          c * u_diff[3])
    f4 = 0.5 *
         equations.c_h *
         (equations.c_h *
...
function min_max_speed_naive(u_ll, u_rr, orientation, equations::GLMMaxwellEquations2D)
    max(1.0, equations.c_h) * (-299792458.0, 299792458.0)
end

function max_abs_speeds(u, equations::GLMMaxwellEquations2D)
    (max(1.0, equations.c_h) * 299792458.0, max(1.0, equations.c_h) * 299792458.0)
end

function max_abs_speed_naive(u_ll, u_rr, orientation, equations::GLMMaxwellEquations2D)
