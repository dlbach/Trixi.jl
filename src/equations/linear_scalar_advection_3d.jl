...
@muladd begin
    ...
    function initial_condition_sin(x, t, equation::LinearScalarAdvectionEquation3D)
        ...
        scalar =
            sin(2 * pi * x_trans[1]) * sin(2 * pi * x_trans[2]) * sin(2 * pi * x_trans[3])
        return SVector(scalar)
    end
    ...
    [`boundary_condition_linear_z`](@ref).
    """
    function boundary_condition_linear_z(
        u_inner,
        orientation,
        direction,
        x,
    ...
    @inline energy_total(u, equation::LinearScalarAdvectionEquation3D) =
