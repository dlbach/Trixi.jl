...
@muladd begin
    ...
    mutable struct IdealGlmMhdMulticomponentEquations1D{NVARS,NCOMP,RealT<:Real} <:
        ...
        function IdealGlmMhdMulticomponentEquations1D{NVARS,NCOMP,RealT}(
            gammas::SVector{NCOMP,RealT},
            gas_constants::SVector{NCOMP,RealT},
    ...
    function IdealGlmMhdMulticomponentEquations1D(; gammas, gas_constants)
        ...
        return IdealGlmMhdMulticomponentEquations1D{NVARS,NCOMP,RealT}(
            __gammas,
            __gas_constants,
        )
    end
    ...
    @inline Base.real(
        ::IdealGlmMhdMulticomponentEquations1D{NVARS,NCOMP,RealT},
    ...
    function flux_derigs_etal(
        ...
        rhok_mean = SVector{ncomponents(equations),real(equations)}(
            ln_mean(u_ll[i+7], u_rr[i+7]) for i in eachcomponent(equations)
        )
        rhok_avg = SVector{ncomponents(equations),real(equations)}(
            0.5 * (u_ll[i+7] + u_rr[i+7]) for i in eachcomponent(equations)
        )
        ...
        f4 =
            ...
            f5 * B1_avg +
            f6 * B2_avg +
            f7 * B3_avg - 0.5 * v1_mag_avg + B1_avg * vel_dot_mag_avg
    ...
    function cons2prim(u, equations::IdealGlmMhdMulticomponentEquations1D)
        ...
        prim_rho = SVector{ncomponents(equations),real(equations)}(
            u[i+7] for i in eachcomponent(equations)
        )
        rho = density(u, equations)
    ...
    @inline function cons2entropy(u, equations::IdealGlmMhdMulticomponentEquations1D)
        ...
        entrop_rho = SVector{ncomponents(equations),real(equations)}(
            -1.0 * (cv[i] * log(T) - gas_constants[i] * log(u[i+7])) +
            gas_constants[i] +
    ...
    @inline function prim2cons(prim, equations::IdealGlmMhdMulticomponentEquations1D)
        ...
        cons_rho = SVector{ncomponents(equations),real(equations)}(
            prim[i+7] for i in eachcomponent(equations)
        )
    ...
    @inline function densities(u, v, equations::IdealGlmMhdMulticomponentEquations1D)
        ...
        return SVector{ncomponents(equations),real(equations)}(
            u[i+7] * v for i in eachcomponent(equations)
        )
    end
