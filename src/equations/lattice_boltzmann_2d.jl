...
@muladd begin
    ...
    * 9 ←→ 9
    ...
    function LatticeBoltzmannEquations2D(;
        Ma,
        Re,
    ...
    varnames(::typeof(cons2cons), equations::LatticeBoltzmannEquations2D) =
        ntuple(v -> "pdf" * string(v), nvariables(equations))
    varnames(::typeof(cons2prim), equations::LatticeBoltzmannEquations2D) =
    ...
    varnames(::typeof(cons2macroscopic), ::LatticeBoltzmannEquations2D) =
        ("rho", "v1", "v2", "p")
    ...
    """
    @inline function boundary_condition_noslip_wall(
        u_inner,
        orientation,
        direction,
        x,
    ...
    @inline function velocity(u, equations::LatticeBoltzmannEquations2D)
        ...
        return SVector(dot(v_alpha1, u) / rho, dot(v_alpha2, u) / rho)
    end
    ...
    `u`.
    """
    @inline pressure(rho::Real, equations::LatticeBoltzmannEquations2D) =
        rho * equations.c_s^2
    @inline pressure(u, equations::LatticeBoltzmannEquations2D) =
    ...
    """
    @inline function equilibrium_distribution(
        alpha,
        rho,
        v1,
        v2,
        ...
        return weights[alpha] *
               rho *
               (
                   1 + va_v / cs_squared + va_v^2 / (2 * cs_squared^2) -
    ...
    @inline function equilibrium_distribution(
        ...
        return SVector(
            equilibrium_distribution(1, rho, v1, v2, equations),
