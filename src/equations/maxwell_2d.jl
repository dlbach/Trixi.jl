...
@muladd begin
    ...
    function boundary_condition_irradiation(
        u_inner,
        orientation,
        direction,
        x,
        ...
        if iseven(direction)
            return surface_flux_function(
                u_inner,
                SVector(2.0 * e1 - u_inner[1], 2.0 * e2 - u_inner[2], u_inner[3]),
                orientation,
                equations,
            )
        else
    ...
    function boundary_condition_truncation(
        u_inner,
        orientation,
        direction,
        x,
    ...
    min_max_speed_naive(u_ll, u_rr, orientation, ::MaxwellEquations2D) =
