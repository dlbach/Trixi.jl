...
struct MaxwellEquations2D <: AbstractMaxwellEquations{2,3}
end
...
@inline function flux_upwind(u_ll, u_rr, orientation::Integer, equations::MaxwellEquations2D)
  c = 299792458.0
  u_sum = u_ll + u_rr
  u_diff = u_ll - u_rr
  if orientation == 1
...
@inline function flux_upwind(u_ll, u_rr, normal_direction::AbstractVector, equations::MaxwellEquations2D)
  c = 299792458.0
  u_sum = u_ll + u_rr
  u_diff = u_ll - u_rr
  f1 = -0.5 * c * normal_direction[2] * ( -normal_direction[2] * u_diff[1] + normal_direction[1] * u_diff[2] + c * u_sum[3] )
  f2 = 0.5 * c * normal_direction[1] * ( -normal_direction[2] * u_diff[1] + normal_direction[1] * u_diff[2] + c * u_sum[3] )
  f3 = 0.5 * ( -normal_direction[2] * u_sum[1] + normal_direction[1] * u_sum[2] + c * u_diff[3] )
...
function boundary_condition_perfect_conducting_wall(u_inner, orientation, direction, x, t,
  ...
  if iseven(direction)
    return surface_flux_function(u_inner, SVector(-u_inner[1], -u_inner[2], u_inner[3]), orientation, equations)
  else
    return surface_flux_function(SVector(-u_inner[1], -u_inner[2], u_inner[3]), u_inner, orientation, equations)
  end
end

function boundary_condition_perfect_conducting_wall(u_inner, normal_direction::AbstractVector, x, t,
                                      surface_flux_function, equations::MaxwellEquations2D)  
  return surface_flux_function(u_inner, SVector(-u_inner[1], -u_inner[2], u_inner[3]), normal_direction, equations)
end
...
function boundary_condition_irradiation(u_inner, orientation, direction, x, t,
  ...
  if iseven(direction)
    return surface_flux_function(u_inner, SVector(2.0*e1-u_inner[1], 2.0*e2-u_inner[2], u_inner[3]), orientation, equations)
  else
    return surface_flux_function(SVector(2.0*e1-u_inner[1], 2.0*e2-u_inner[2], u_inner[3]), u_inner, orientation, equations)
  end
end
...
function boundary_condition_irradiation(u_inner, normal_direction::AbstractVector, x, t,
  ...
  e1 = -(k_orth/k_par)*sin(k_orth*x[2])*cos(k_par*x[1]-omega*t)
  e2 = cos(k_orth*x[2])*sin(k_par*x[1]-omega*t)
  return surface_flux_function(u_inner, SVector(2.0*e1-u_inner[1], 2.0*e2-u_inner[2], u_inner[3]), normal_direction, equations)
end
