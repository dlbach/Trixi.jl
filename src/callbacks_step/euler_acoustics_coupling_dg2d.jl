...
@muladd begin
    ...
    function calc_acoustic_sources!(
        ...
        @threaded for k = 1:length(coupled_element_ids)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                vorticity =
                    calc_vorticity_node(u_euler, mesh, equations, dg, cache, i, j, element)

                prim_euler = cons2prim(
                    get_node_vars(u_euler, equations, dg, i, j, element),
                    equations,
                )
                v1 = prim_euler[2]
                v2 = prim_euler[3]
                ...
                acoustic_source_terms[1, i, j, k] -=
                    -vorticity_prime * v2_mean - vorticity_mean[i, j, element] * v2_prime
                acoustic_source_terms[2, i, j, k] -=
                    vorticity_prime * v1_mean + vorticity_mean[i, j, element] * v1_prime
            end
        end
