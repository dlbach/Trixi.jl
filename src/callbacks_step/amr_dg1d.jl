...
@muladd begin
    ...
    function refine!(
        ...
        if isperiodic(mesh.tree)
            @assert ninterfaces(interfaces) == 1 * nelements(dg, cache) (
                "For 1D and periodic domains, the number of interfaces must be the same as the number of elements"
            )
        end
    ...
    function refine_element!(
        ...
        for i in eachnode(dg)
            ...
            for k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, old_element_id) *
                    forward_lower[i, k]
            end
    ...
    function coarsen_elements!(
        ...
        for i in eachnode(dg)
            ...
            for k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, right_id) * reverse_upper[i, k]
            end
