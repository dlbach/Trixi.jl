...
@muladd begin
    ...
    function create_cache_analysis(
        analyzer,
        mesh::TreeMesh{3},
        equations,
        dg::DG,
        ...
        for element in eachelement(dg, cache)
            ...
            for k in eachnode(analyzer), j in eachnode(analyzer), i in eachnode(analyzer)
                u_exact = initial_condition(
                    get_node_coords(x_local, equations, dg, i, j, k),
                    t,
                    equations,
                )
                diff =
    ...
    function integrate_via_indices(
        func::Func,
        u,
        mesh::TreeMesh{3},
        equations,
    ...
    function integrate_via_indices(
        ...
        for element in eachelement(dg, cache)
            for k in eachnode(dg), j in eachnode(dg), i in eachnode(dg)
                volume_jacobian =
                    abs(inv(cache.elements.inverse_jacobian[i, j, k, element]))
                integral +=
                    volume_jacobian *
                    weights[i] *
                    weights[j] *
    ...
    function integrate(
        ...
        integrate_via_indices(
            ...
            cache;
            normalize = normalize,
        ) do u, i, j, k, element, equations, dg
    ...
    function integrate(
        ...
        integrate_via_indices(
            ...
            return func(
                u_local,
                (gradients_1_local, gradients_2_local, gradients_3_local),
                equations_parabolic,
            )
        end
    end
    ...
    function analyze(
        ...
        integrate_via_indices(
            ...
            cache,
            du,
        ) do u, i, j, k, element, equations, dg, du
    ...
    function analyze(
        ...
        integrate_via_indices(
            ...
            cache,
            cache,
            dg.basis.derivative_matrix,
        ) do u, i, j, k, element, equations, dg, cache, derivative_matrix
            divb = zero(eltype(u))
            for l in eachnode(dg)
                divb += (
                    derivative_matrix[i, l] * u[6, l, j, k, element] +
    ...
    function analyze(
        ...
        integrate_via_indices(
            ...
            for l in eachnode(dg)
                divb += (
                    derivative_matrix[i, l] * (
                        Ja11 * u[6, l, j, k, element] +
                        Ja12 * u[7, l, j, k, element] +
                        Ja13 * u[8, l, j, k, element]
                    ) +
                    derivative_matrix[j, l] * (
                        Ja21 * u[6, i, l, k, element] +
                        Ja22 * u[7, i, l, k, element] +
                        Ja23 * u[8, i, l, k, element]
                    ) +
                    derivative_matrix[k, l] * (
                        Ja31 * u[6, i, j, l, element] +
