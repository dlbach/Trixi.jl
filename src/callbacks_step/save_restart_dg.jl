...
@muladd begin
    ...
    function load_restart_file(
        ...
        h5open(restart_file, "r") do file
            ...
            if read(attributes(file)["polydeg"]) != polydeg(dg)
                error(
                    "restart mismatch: polynomial degree in solver differs from value in restart file",
                )
            end
            if read(attributes(file)["n_elements"]) != nelements(dg, cache)
                error(
                    "restart mismatch: number of elements in solver differs from value in restart file",
                )
            end
            ...
            for v in eachvariable(equations)
                ...
                if (name = read(attributes(var)["name"])) !=
                   varnames(cons2cons, equations)[v]
                    error(
                        "mismatch: variables_$v should be '$(varnames(cons2cons, equations)[v])', but found '$name'",
                    )
                end
    ...
    function save_restart_file_parallel(
        u,
        time,
        dt,
        timestep,
    ...
    function save_restart_file_on_root(
        u,
        time,
        dt,
        timestep,
    ...
    function load_restart_file_on_root(
        mesh::Union{ParallelTreeMesh,ParallelP4estMesh},
        equations,
        dg::DG,
        cache,
