...
@muladd begin
    ...
    function initialize_mean_values(
        mesh::TreeMesh{2},
        ...
        v_mean = zeros(
            uEltype,
            (ndims(equations), nnodes(dg), nnodes(dg), nelements(cache.elements)),
        )
    ...
    function calc_mean_values!(
        ...
        @threaded for element in eachelement(dg, cache)
            for j in eachnode(dg), i in eachnode(dg)
                ...
                u_node_prim =
                    cons2prim(get_node_vars(u, equations, dg, i, j, element), equations)
                u_prev_node_prim = cons2prim(
                    get_node_vars(u_prev, equations, dg, i, j, element),
                    equations,
                )
                ...
                vorticity_mean[i, j, element] +=
                    integration_constant * (vorticity_prev_node + vorticity)
