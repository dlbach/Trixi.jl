...
@muladd begin
        ...
        if get(io, :compact, false)
            ...
            if analysis_callback.save_analysis
                ...
                push!(
                    setup,
                    "│ output directory" =>
                        abspath(normpath(analysis_callback.output_directory)),
                )
            end
    ...
    function AnalysisCallback(
        ...
        cache_analysis =
            create_cache_analysis(analyzer, mesh, equations, solver, cache, RealT, uEltype)
    ...
    function initialize!(
        ...
        t,
        integrator,
    ) where {Condition,Affect!<:AnalysisCallback}
        semi = integrator.p
    ...
    function (analysis_callback::AnalysisCallback)(integrator)
        ...
        runtime_since_last_analysis =
            1.0e-9 * (time_ns() - analysis_callback.start_time_last_analysis)
        ...
        ncalls_rhs_since_last_analysis =
            (ncalls(semi.performance_counter) - analysis_callback.ncalls_rhs_last_analysis)
        performance_index =
            runtime_since_last_analysis * mpi_nranks() /
            (ndofsglobal(mesh, solver, cache) * ncalls_rhs_since_last_analysis)
        ...
        @trixi_timeit timer() "analyze solution" begin
            ...
            mpi_println(
                ...
                "               " *
                " run time:       " *
                @sprintf("%10.8e s", runtime_absolute),
            )
            mpi_println(
                " Δt:             " *
                @sprintf("%10.8e", dt) *
            ...
            mpi_println(
                ...
                "               " *
                " time/DOF/rhs!:  " *
                @sprintf("%10.8e s", runtime_relative),
            )
            mpi_println(
                "                 " *
                "              " *
                "               " *
                " PID:            " *
                @sprintf("%10.8e s", performance_index),
            )
            mpi_println(
                " #DOF:           " *
                @sprintf("% 14d", ndofs(semi)) *
                "               " *
                " alloc'd memory: " *
                @sprintf("%14.3f MiB", memory_use),
            )
            mpi_println(
                " #elements:      " * @sprintf("% 14d", nelements(mesh, solver, cache)),
            )
            ...
            if mpi_isroot() && analysis_callback.save_analysis
                io = open(
                    joinpath(
                        analysis_callback.output_directory,
                        analysis_callback.analysis_filename,
                    ),
                    "a",
                )
                @printf(io, "% 9d", iter)
    ...
    function (analysis_callback::AnalysisCallback)(io, du, u, u_ode, t, semi)
        ...
        if any(
            q in analysis_errors for
            q in (:l2_error, :linf_error, :conservation_error, :residual)
        ) && mpi_isroot()
            print(" Variable:    ")
        ...
        if :l2_error_primitive in analysis_errors ||
           :linf_error_primitive in analysis_errors
            l2_error_prim, linf_error_prim =
                calc_error_norms(cons2prim, u_ode, t, analyzer, semi, cache_analysis)
    ...
    function print_amr_information(callbacks, mesh, solver, cache)
        ...
        for level = max_level:-1:min_level+1
            mpi_println(
                " ├── level $level:    " * @sprintf("% 14d", count(==(level), levels)),
            )
        end
        mpi_println(
            " └── level $min_level:    " * @sprintf("% 14d", count(==(min_level), levels)),
        )
    ...
    function print_amr_information(callbacks, mesh::P4estMesh, solver, cache)
        ...
        for level = max_level:-1:min_level+1
            mpi_println(
                " ├── level $level:    " * @sprintf("% 14d", elements_per_level[level+1]),
            )
        end
        mpi_println(
            " └── level $min_level:    " *
    ...
    function analyze_integrals(
        analysis_integrals::NTuple{N,Any},
        io,
        du,
        u,
        t,
    ...
    function (cb::DiscreteCallback{Condition,Affect!})(
        sol,
    ) where {Condition,Affect!<:AnalysisCallback}
        analysis_callback = cb.affect!
        semi = sol.prob.p
        ...
        l2_error, linf_error =
            calc_error_norms(sol.u[end], sol.t[end], analyzer, semi, cache_analysis)
