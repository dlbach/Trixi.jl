...
@muladd begin
    ...
    function rebalance_solver!(
        u_ode::AbstractVector,
        mesh::TreeMesh{2},
        equations,
        dg::DGSEM,
        cache,
        old_mpi_ranks_per_cell,
    )
        ...
        GC.@preserve old_u_ode begin # OBS! If we don't GC.@preserve old_u_ode, it might be GC'ed
            ...
            @trixi_timeit timer() "reinitialize data structures" reinitialize_containers!(
                mesh,
                equations,
                dg,
                cache,
            ...
            @trixi_timeit timer() "exchange data" begin
                ...
                for element in eachelement(dg, cache)
                    ...
                    if cell_id in old_cell_ids
                        ...
                        request =
                            MPI.Irecv!(@view(u[:, .., element]), src, cell_id, mpi_comm())
                        push!(requests, request)
                    end
                end
    ...
    function refine!(
        ...
        if mesh isa TreeMesh &&
           isperiodic(mesh.tree) &&
           nmortars(cache.mortars) == 0 &&
           !mpi_isparallel()
    ...
    function refine_element!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for l in eachnode(dg), k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, l, old_element_id) *
                    forward_lower[i, k] *
                    forward_lower[j, l]
            end
    ...
    function coarsen_elements!(
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for l in eachnode(dg), k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, l, lower_left_id) *
                    reverse_lower[i, k] *
                    reverse_lower[j, l]
            end
            ...
            for l in eachnode(dg), k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, l, lower_right_id) *
                    reverse_upper[i, k] *
                    reverse_lower[j, l]
            end
            ...
            for l in eachnode(dg), k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, l, upper_left_id) *
                    reverse_lower[i, k] *
                    reverse_upper[j, l]
            end
            ...
            for l in eachnode(dg), k in eachnode(dg)
                acc +=
                    get_node_vars(old_u, equations, dg, k, l, upper_right_id) *
                    reverse_upper[i, k] *
                    reverse_upper[j, l]
            end
