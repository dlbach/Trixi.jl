...
@muladd begin
    ...
    function save_solution_file_parallel(
        data,
        time,
        dt,
        timestep,
        ...
        cum_element_counts =
            append!(zeros(eltype(element_counts), 1), cumsum(element_counts))
        ...
        h5open(filename, "w", mpi_comm()) do file
            ...
            for (v, (key, element_variable)) in enumerate(element_variables)
                ...
                # Write data of each process in slices (ranks start with 0)
                slice =
                    (cum_element_counts[mpi_rank()+1]+1):cum_element_counts[mpi_rank()+2]
                # Add to file
    ...
    function save_solution_file_on_root(
        data,
        time,
        dt,
        timestep,
        ...
        h5open(filename, "w") do file
            ...
            for (v, (key, element_variable)) in enumerate(element_variables)
                ...
                MPI.Gatherv!(
                    element_variable,
                    MPI.VBuffer(recv, element_counts),
                    mpi_root(),
                    mpi_comm(),
                )
