        ...
        GC.@preserve old_u_ode begin # OBS! If we don't GC.@preserve old_u_ode, it might be GC'ed
            ...
            @trixi_timeit timer() "exchange data" begin
                ...
                for old_element_id = 1:old_n_elements
                    ...
                    global_quad_id =
                        old_global_first_quadrant[mpi_rank()+1] + old_element_id - 1
                    if !(
                        global_first_quadrant[mpi_rank()+1] <=
                        global_quad_id <
                ...
                for element in eachelement(dg, cache)
                    ...
                    if old_global_first_quadrant[mpi_rank()+1] <=
                       global_quad_id <
                       old_global_first_quadrant[mpi_rank()+2]
                        # Quad ids are 0-based, element ids are 1-based, hence add 1
                        old_element_id =
                            global_quad_id - old_global_first_quadrant[mpi_rank()+1] + 1
