...
@muladd begin
    ...
    function AliveCallback(; analysis_interval = 0, alive_interval = analysis_interval ÷ 10)

        alive_callback = AliveCallback(0.0, alive_interval, analysis_interval)

        DiscreteCallback(
            alive_callback,
            alive_callback, # the first one is the condition, the second the affect!
    ...
    function initialize!(
        ...
        t,
        integrator,
    ) where {Condition,Affect!<:AliveCallback}
    ...
    function (alive_callback::AliveCallback)(integrator)
        ...
        if isfinished(integrator) && mpi_isroot()
            println("─"^100)
            println(
                "Trixi.jl simulation finished.  Final time: ",
                integrator.t,
                "  Time steps: ",
        ...
        elseif mpi_isroot()
            ...
            @printf(
                "#timesteps: %6d │ Δt: %.4e │ sim. time: %.4e │ run time: %.4e s\n",
                integrator.stats.naccept,
                integrator.dt,
                integrator.t,
                runtime_absolute
