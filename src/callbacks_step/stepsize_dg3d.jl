...
@muladd begin
    ...
    function max_dt(
        ...
        for element in eachelement(dg, cache)
            ...
            max_scaled_speed =
                max(max_scaled_speed, inv_jacobian * (max_λ1 + max_λ2 + max_λ3))
        end
    ...
    function max_dt(u, t, mesh::TreeMesh{3}, constant_speed::True, equations, dg::DG, cache)
        # to avoid a division by zero if the speed vanishes everywhere,
        # e.g. for steady-state linear advection
        max_scaled_speed = nextfloat(zero(t))
