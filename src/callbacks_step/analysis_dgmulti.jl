...
@muladd begin
    ...
    function compute_local_divergence!(
        local_divergence,
        element,
        vector_field,
        mesh,
    ...
    function integrate(
        ...
        local_gradient_quadrature_values =
            ntuple(_ -> similar(cache_parabolic.local_u_values_threaded), 3)

        integral = zero(eltype(u))
        for e in eachelement(mesh, dg)
            u_quadrature_values =
                cache_parabolic.local_u_values_threaded[Threads.threadid()]
            gradient_x_quadrature_values =
                local_gradient_quadrature_values[1][Threads.threadid()]
            gradient_y_quadrature_values =
                local_gradient_quadrature_values[2][Threads.threadid()]
            gradient_z_quadrature_values =
                local_gradient_quadrature_values[3][Threads.threadid()]
            ...
            for i in eachindex(u_quadrature_values)
                gradients_i = SVector(
                    gradient_x_quadrature_values[i],
                    gradient_y_quadrature_values[i],
                    gradient_z_quadrature_values[i],
                )
                integral +=
                    mesh.md.wJq[i, e] * func(u_quadrature_values[i], gradients_i, equations)
            end
        end
        return integral
    end
    ...
    function ndofsglobal(mesh::DGMultiMesh, solver::DGMulti, cache)
        if mpi_isparallel()
            error(
                "`ndofsglobal` is not implemented for `DGMultiMesh` when used in parallel with MPI",
            )
        else
