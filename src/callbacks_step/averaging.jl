...
@muladd begin
    ...
    function AveragingCallback(
        semi::SemidiscretizationHyperbolic{<:Any,<:CompressibleEulerEquations2D},
        tspan;
        ...
        averaging_callback =
            AveragingCallback(tspan, mean_values, cache, output_directory, filename)
    ...
    function initialize!(
        ...
        t,
        integrator,
    ) where {Condition,Affect!<:AveragingCallback}
        averaging_callback = cb.affect!
        semi = integrator.p
        mesh, equations, solver, cache = mesh_equations_solver_cache(semi)
        u = wrap_array(u_ode, mesh, equations, solver, cache)

        @trixi_timeit timer() "averaging" initialize_cache!(
    ...
    function (averaging_callback::AveragingCallback)(integrator)
        ...
        @trixi_timeit timer() "averaging" calc_mean_values!(
            mean_values,
            averaging_callback.cache,
            u,
            u_prev,
