...
@muladd begin
    ...
    """
    mutable struct TimeSeriesCallback{
        RealT<:Real,
        uEltype<:Real,
        SolutionVariables,
        VariableNames,
    ...
    function Base.show(io::IO, cb::DiscreteCallback{<:Any,<:TimeSeriesCallback})
        ...
        @unpack interval, solution_variables, output_directory, filename =
    ...
    function TimeSeriesCallback(
        ...
        if ndims(point_coordinates) != 2 || size(point_coordinates, 2) != ndims(mesh)
            throw(
                ArgumentError(
                    "`point_coordinates` must be a matrix of size n_points × ndims",
                ),
            )
        end
        ...
        point_data = Vector{uEltype}[Vector{uEltype}() for _ = 1:n_points]
        time = Vector{RealT}()
        step = Vector{Int}()
        time_series_cache =
            create_cache_time_series(point_coordinates_, mesh, solver, cache)

        time_series_callback = TimeSeriesCallback(
            interval,
            solution_variables,
            variable_names,
            output_directory,
    ...
    function TimeSeriesCallback(
        ...
        point_coordinates_ =
            Matrix{eltype(eltype(point_coordinates))}(undef, n_points, ndims(mesh))
