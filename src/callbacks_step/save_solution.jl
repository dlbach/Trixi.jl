...
@muladd begin
        ...
        if get(io, :compact, false)
            ...
            setup = [
                ...
                "save initial solution" =>
                    save_solution_callback.save_initial_solution ? "yes" : "no",
                "save final solution" =>
                    save_solution_callback.save_final_solution ? "yes" : "no",
                "output directory" =>
                    abspath(normpath(save_solution_callback.output_directory)),
            ]
    ...
    function SaveSolutionCallback(;
        ...
        if !isnothing(dt) && interval > 0
            throw(
                ArgumentError(
                    "You can either set the number of steps between output (using `interval`) or the time between outputs (using `dt`) but not both simultaneously",
                ),
            )
        end
        ...
        solution_callback = SaveSolutionCallback(
            interval_or_dt,
    ...
    function initialize_save_cb!(solution_callback::SaveSolutionCallback, u, t, integrator)
        ...
        @trixi_timeit timer() "I/O" begin
            if mesh.unsaved_changes
                mesh.current_filename =
                    save_mesh_file(mesh, solution_callback.output_directory)
    ...
    function (solution_callback::SaveSolutionCallback)(u, t, integrator)
        ...
        return interval_or_dt > 0 && (
            (
                (integrator.stats.naccept % interval_or_dt == 0) &&
    ...
    function (solution_callback::SaveSolutionCallback)(integrator)
        ...
        @trixi_timeit timer() "I/O" begin
            @trixi_timeit timer() "save mesh" if mesh.unsaved_changes
                mesh.current_filename =
                    save_mesh_file(mesh, solution_callback.output_directory, iter)
                mesh.unsaved_changes = false
            end
            ...
            @trixi_timeit timer() "save solution" save_solution_file(
                u_ode,
                t,
                dt,
                iter,
