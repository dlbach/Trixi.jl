...
@muladd begin
    ...
    function create_cache_time_series(point_coordinates, mesh::TreeMesh{2}, dg, cache)
        ...
        interpolating_polynomials =
            calc_interpolating_polynomials(point_coordinates, element_ids, mesh, dg, cache)
    ...
    function get_elements_by_coordinates!(
        element_ids,
        coordinates,
        mesh::TreeMesh,
        dg,
        ...
        for element in eachelement(dg, cache)
            ...
            for index = 1:length(element_ids)
    ...
    function calc_interpolating_polynomials!(
        ...
        for index = 1:length(element_ids)
            ...
            for d = 1:ndims(mesh)
                interpolating_polynomials[:, d, index] .=
                    lagrange_interpolating_polynomials(unit_coordinates[d], nodes, wbary)
            end
        end
    ...
    function calc_interpolating_polynomials(
        ...
        interpolating_polynomials =
            Array{real(dg),3}(undef, nnodes(dg), ndims(mesh), length(element_ids))
    ...
    function record_state_at_points!(
        point_data,
        u,
        solution_variables,
        ...
        for index = 1:length(element_ids)
            ...
            for j in eachnode(dg), i in eachnode(dg)
                u_node = solution_variables(
                    get_node_vars(u, equations, dg, i, j, element_id),
                    equations,
                )

                for v = 1:length(u_node)
                    data[old_length+v] += (
                        u_node[v] *
