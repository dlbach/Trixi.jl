...
@muladd begin
    ...
    function AMRCallback(
        ...
        if !(interval isa Integer && interval >= 0)
            throw(
                ArgumentError(
                    "`interval` must be a non-negative integer (provided `interval = $interval`)",
                ),
            )
        end
    ...
    function initialize!(
        ...
        t,
        integrator,
    ) where {Condition,Affect!<:AMRCallback}
        amr_callback = cb.affect!
        semi = integrator.p
    ...
    function (amr_callback::AMRCallback)(integrator; kwargs...)
        ...
        @trixi_timeit timer() "AMR" begin
            has_changed =
                amr_callback(u_ode, semi, integrator.t, integrator.iter; kwargs...)
            if has_changed
    ...
    function (amr_callback::AMRCallback)(
        u_ode::AbstractVector,
        mesh::TreeMesh,
        equations,
        dg::DG,
        ...
        lambda = @trixi_timeit timer() "indicator" controller(
            u,
            mesh,
            equations,
            dg,
        ...
        @boundscheck begin
            @assert axes(lambda) == axes(leaf_cell_ids) (
                "Indicator (axes = $(axes(lambda))) and leaf cell (axes = $(axes(leaf_cell_ids))) arrays have different axes"
            )
        end
        ...
        @trixi_timeit timer() "refine" if !only_coarsen && !isempty(to_refine)
            ...
            refined_original_cells =
                @trixi_timeit timer() "mesh" refine!(mesh.tree, to_refine)

            # Find all indices of elements whose cell ids are in refined_original_cells
            elements_to_refine =
                findall(in(refined_original_cells), cache.elements.cell_ids)

            # refine solver
            @trixi_timeit timer() "solver" refine!(
                u_ode,
                adaptor,
                mesh,
                equations,
            ...
            for (p_u_ode, p_mesh, p_equations, p_dg, p_cache) in passive_args
                @trixi_timeit timer() "passive solver" refine!(
                    p_u_ode,
                    adaptor,
                    p_mesh,
                    p_equations,
        ...
        @trixi_timeit timer() "coarsen" if !only_refine && !isempty(to_coarsen)
            ...
            to_coarsen =
                collect(1:length(parents_to_coarsen))[parents_to_coarsen.==2^ndims(mesh)]

            # Finally, coarsen mesh
            coarsened_original_cells =
                @trixi_timeit timer() "mesh" coarsen!(mesh.tree, to_coarsen)
            ...
            removed_child_cells = zeros(
                Int,
                n_children_per_cell(mesh.tree) * length(coarsened_original_cells),
            )
            for (index, coarse_cell_id) in enumerate(coarsened_original_cells)
                for child = 1:n_children_per_cell(mesh.tree)
                    removed_child_cells[n_children_per_cell(mesh.tree)*(index-1)+child] =
                        coarse_cell_id + child
                end
            end
            ...
            # coarsen solver
            @trixi_timeit timer() "solver" coarsen!(
                u_ode,
                adaptor,
                mesh,
                equations,
            ...
            for (p_u_ode, p_mesh, p_equations, p_dg, p_cache) in passive_args
                @trixi_timeit timer() "passive solver" coarsen!(
                    p_u_ode,
                    adaptor,
                    p_mesh,
                    p_equations,
    ...
    # 2D
    cfunction(::typeof(copy_to_quad_iter_volume), ::Val{2}) = @cfunction(
        copy_to_quad_iter_volume,
        Cvoid,
        (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(copy_to_quad_iter_volume), ::Val{3}) = @cfunction(
        copy_to_quad_iter_volume,
        Cvoid,
    ...
    function (amr_callback::AMRCallback)(
        ...
        if has_changed # TODO: Taal decide, where shall we set this?
            ...
            if mpi_isparallel() && amr_callback.dynamic_load_balancing
                @trixi_timeit timer() "dynamic load balancing" begin
                    global_first_quadrant = unsafe_wrap(
                        Array,
    ...
    function reinitialize_boundaries!(
    ...
    function ControllerThreeLevel(
        semi,
        indicator;
        base_level = 1,
        med_level = base_level,
        med_threshold = 0.0,
    ...
    create_cache(indicator_type::Type{ControllerThreeLevel}, semi) =
    ...
    # 2D
    cfunction(::typeof(extract_levels_iter_volume), ::Val{2}) = @cfunction(
        extract_levels_iter_volume,
        Cvoid,
        (Ptr{p4est_iter_volume_info_t}, Ptr{Cvoid})
    )
    # 3D
    cfunction(::typeof(extract_levels_iter_volume), ::Val{3}) = @cfunction(
        extract_levels_iter_volume,
        Cvoid,
        (Ptr{p8est_iter_volume_info_t}, Ptr{Cvoid})
    )
    ...
    function (controller::ControllerThreeLevel)(
        u::AbstractArray{<:Any},
        mesh,
        equations,
        dg::DG,
    ...
    """
    struct ControllerThreeLevelCombined{
        RealT<:Real,
        IndicatorPrimary,
        IndicatorSecondary,
        Cache,
    ...
    function ControllerThreeLevelCombined(
        semi,
        indicator_primary,
        indicator_secondary;
        base_level = 1,
        ...
        med_threshold, max_threshold, max_threshold_secondary =
            promote(med_threshold, max_threshold, max_threshold_secondary)
        cache = create_cache(ControllerThreeLevelCombined, semi)
        ControllerThreeLevelCombined{
            typeof(max_threshold),
            typeof(indicator_primary),
            typeof(indicator_secondary),
            typeof(cache),
        }(
            base_level,
            med_level,
    ...
    create_cache(indicator_type::Type{ControllerThreeLevelCombined}, semi) =
        create_cache(indicator_type, mesh_equations_solver_cache(semi)...)
    ...
    function get_element_variables!(
        ...
        get_element_variables!(
            element_variables,
            controller.indicator_primary,
            amr_callback,
        )
    end


    function (controller::ControllerThreeLevelCombined)(
        u::AbstractArray{<:Any},
        mesh,
        equations,
        dg::DG,
