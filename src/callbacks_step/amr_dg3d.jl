...
@muladd begin
    ...
    function refine!(
        ...
        GC.@preserve old_u_ode begin # OBS! If we don't GC.@preserve old_u_ode, it might be GC'ed
            ...
            element_id = 1
            for old_element_id = 1:old_n_elements
        ...
        if mesh isa TreeMesh && isperiodic(mesh.tree) && nmortars(cache.mortars) == 0
            @assert ninterfaces(cache.interfaces) == ndims(mesh) * nelements(dg, cache) (
                "For $(ndims(mesh))D and periodic domains and conforming elements, the number of interfaces must be $(ndims(mesh)) times the number of elements"
            )
        end
        ...
        GC.@preserve old_u_ode begin # OBS! If we don't GC.@preserve old_u_ode, it might be GC'ed
            ...
            for old_element_id = 1:old_n_elements
                ...
                if to_be_removed[old_element_id]
                    ...
                    @assert all(
                        to_be_removed[old_element_id:(old_element_id+2^ndims(mesh)-1)],
                    ) "bad cell/element order"
    ...
    function coarsen_elements!(
        ...
        multiply_dimensionwise!(
            ...
            reverse_lower,
            reverse_lower,
            view(old_u, :, :, :, :, bottom_lower_left_id),
            u_tmp1,
            u_tmp2,
        )
        ...
        add_multiply_dimensionwise!(
            ...
            reverse_lower,
            reverse_lower,
            view(old_u, :, :, :, :, bottom_lower_right_id),
            u_tmp1,
            u_tmp2,
        )
        ...
        add_multiply_dimensionwise!(
            ...
            reverse_upper,
            reverse_lower,
            view(old_u, :, :, :, :, bottom_upper_left_id),
            u_tmp1,
            u_tmp2,
        )
        ...
        add_multiply_dimensionwise!(
            ...
            reverse_upper,
            reverse_lower,
            view(old_u, :, :, :, :, bottom_upper_right_id),
            u_tmp1,
            u_tmp2,
        )
