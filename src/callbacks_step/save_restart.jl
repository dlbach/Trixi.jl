...
@muladd begin
    ...
    function Base.show(
        ...
        if get(io, :compact, false)
            ...
            setup = [
                ...
                "save final solution" =>
                    save_restart_callback.save_final_restart ? "yes" : "no",
                "output directory" =>
                    abspath(normpath(save_restart_callback.output_directory)),
            ]
    ...
    function SaveRestartCallback(;
        ...
        restart_callback =
            SaveRestartCallback(interval, save_final_restart, output_directory)

        DiscreteCallback(
            restart_callback,
            restart_callback, # the first one is the condition, the second the affect!
    ...
    function initialize!(
        ...
        t,
        integrator,
    ) where {Condition,Affect!<:SaveRestartCallback}
        restart_callback = cb.affect!
        ...
        @trixi_timeit timer() "I/O" begin
            if mesh.unsaved_changes
                mesh.current_filename =
                    save_mesh_file(mesh, restart_callback.output_directory)
    ...
    function (restart_callback::SaveRestartCallback)(integrator)
        ...
        @trixi_timeit timer() "I/O" begin
            if mesh.unsaved_changes
                mesh.current_filename =
                    save_mesh_file(mesh, restart_callback.output_directory, iter)
                mesh.unsaved_changes = false
            end
    ...
    @inline function save_restart_file(
        u_ode,
        t,
        dt,
        iter,
