...
@muladd begin
    ...
    """
    function EulerAcousticsCouplingCallback(
        ode_euler,
        averaging_callback::DiscreteCallback{<:Any,<:AveragingCallback},
        alg,
        cfl_acoustics::Real,
        ...
        return EulerAcousticsCouplingCallback(
            ode_euler,
            mean_values,
            alg,
            cfl_acoustics,
    ...
    function EulerAcousticsCouplingCallback(
        ...
        euler_acoustics_coupling = EulerAcousticsCouplingCallback{
            typeof(cfl_acoustics),
            typeof(mean_values),
            typeof(integrator_euler),
        }(
            StepsizeCallback(cfl_acoustics),
    ...
    ) where {Condition,Affect!<:EulerAcousticsCouplingCallback}
        euler_acoustics_coupling = cb.affect!
    ...
    function (euler_acoustics_coupling::EulerAcousticsCouplingCallback)(
        integrator_acoustics,
    )
        @unpack stepsize_callback_acoustics, stepsize_callback_euler, integrator_euler =
            euler_acoustics_coupling
        ...
        if !isfinished(integrator_euler)
            ...
            if !(
                SciMLBase.successful_retcode(return_code) ||
                return_code != SciMLBase.ReturnCode.Default
            )
                error(
                    "Error during compressible Euler time integration. Received return code $(return_code)",
                )
            end
        end
