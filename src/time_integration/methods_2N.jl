...
@muladd begin
    ...
    mutable struct SimpleIntegrator2N{
        RealT<:Real,
        uType,
        Params,
        Sol,
    ...
    function solve(
        ...
        callback = nothing,
        kwargs...,
    ) where {T<:SimpleAlgorithm2N}
        u = copy(ode.u0)
        du = similar(u)
        u_tmp = similar(u)
        t = first(ode.tspan)
        iter = 0
        integrator = SimpleIntegrator2N(
            u,
            du,
            u_tmp,
            t,
            ...
            ode.f,
            alg,
            SimpleIntegrator2NOptions(callback, ode.tspan; kwargs...),
            false,
        )
            ...
            for stage in eachindex(alg.c)
                ...
                @trixi_timeit timer() "Runge-Kutta step" begin
                    @threaded for i in eachindex(integrator.u)
                        integrator.u_tmp[i] =
                            integrator.du[i] - integrator.u_tmp[i] * a_stage
