...
function Trixi.boundary_condition_perfect_conducting_wall(
    ...
    if iseven(direction)
        if orientation == 1
            return surface_flux_function(
                u_inner,
                SVector(u_inner[1], -u_inner[2], u_inner[3], -u_inner[4]),
                orientation,
                equations,
            )
        else
...
function Trixi.boundary_condition_irradiation(u_inner, orientation, direction,
                                        x, t,
                                        surface_flux_function,
                                        equations::GlmMaxwellEquations2D)
    c = equations.speed_of_light
    ...
    if iseven(direction)
        if orientation == 1
            return surface_flux_function(u_inner,
                                        SVector(u_inner[1], 2 * e2 - u_inner[2],
                                                u_inner[3], -u_inner[4]),
                                        orientation, equations)
        else
            return surface_flux_function(u_inner, SVector(2 * e1 - u_inner[1], u_inner[2],
                                                u_inner[3], -u_inner[4]),
                                        orientation, equations)
        end
    else
        if orientation == 1
            return surface_flux_function(SVector(u_inner[1], 2 * e2 - u_inner[2],
                                                u_inner[3], -u_inner[4]), u_inner,
                                        orientation, equations)
        else
            return surface_flux_function(SVector(2 * e1 - u_inner[1], u_inner[2],
                                                u_inner[3], -u_inner[4]), u_inner,
                                        orientation, equations)
        end        
    end
...
function source_term_function(u, x, t, equations::GlmMaxwellEquations2D)
    return SVector(0.0, 0.0, 0.0, -2*pi*sin(pi*x[1])*sin(pi*x[2]) * equations.c_e)
    #return SVector(0.0, 0.0, 0.0, 0.0)
end

equation = GlmMaxwellEquations2D(1e0,1e0,1e0)
