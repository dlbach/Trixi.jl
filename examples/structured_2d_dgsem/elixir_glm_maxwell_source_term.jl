...
function boundary_condition_irradiation(u_inner, orientation_or_normal_direction, direction, x, t,
  ...
  if iseven(direction)
    return surface_flux_function(u_inner, SVector(2.0*x[1]-u_inner[1], 2.0*x[2]-u_inner[2], u_inner[3], -u_inner[4]), orientation_or_normal_direction, equations)
  else
    return surface_flux_function(SVector(2.0*x[1]-u_inner[1], 2.0*x[2]-u_inner[2], u_inner[3], -u_inner[4]), u_inner, orientation_or_normal_direction, equations)
  end
end
